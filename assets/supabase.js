...
// ---- Window export shim for inline scripts ----
try {
  if (typeof window !== 'undefined' && !window.__SUPABASE_SHIM__) {
    window.__SUPABASE_SHIM__ = true;
    const api = { SUPABASE_URL, SUPABASE_ANON, rpc, insert, select, update, nowISO };
    for (const [k, v] of Object.entries(api)) {
      if (!(k in window)) window[k] = v;
    }
    console.log('[supabase.js] helpers exposed on window for inline pages');
  }
} catch (e) { /* no-op */ }