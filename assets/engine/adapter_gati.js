...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function row(o){
    ...
    const btn = document.createElement('button'); btn.textContent = 'DORËZO'; btn.style.padding='10px 12px'; btn.style.borderRadius='10px'; btn.style.border='1px solid #456'; btn.style.background='#18222e'; btn.style.color='#fff';
    btn.onclick = async ()=>{ await Engine.saveOrder(Object.assign(o,{status:Engine.STATUS.DORZIM, picked_at:new Date().toISOString()})); load(); };
  ...
  async function load(){
    ...
    try{
      const items = (await Engine.list(Engine.STATUS.GATI)) || [];
      container.innerHTML = '';
      if(!items.length){ container.innerHTML = '<div style="opacity:.7;padding:10px;border:1px solid #223;border-radius:12px">S’ka porosi GATI.</div>'; return; }
