...
export function pushArkaLocal(record) {
  // Kept as a tiny wrapper for backwards compatibility.
  // Writes to the unified local store (ARKA_MOVES) and mirrors legacy keys.
  if (typeof window === 'undefined') return;
  try {
    const list = Array.isArray(getLocalMoves()) ? getLocalMoves() : [];
    const next = [record, ...list];
    setLocalMoves(next.slice(0, 500));
  } catch {
    // ignore
  }
}
