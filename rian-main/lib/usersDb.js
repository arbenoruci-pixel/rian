...
export async function upsertUser(user) {
  ...
  if (!payload.name || !payload.pin) {
    return { ok: false, error: new Error('NAME and PIN are required') };
  }
  ...
  const { data, error } = payload.id
    ? await query.update(payload).eq('id', payload.id).select().maybeSingle()
    : await query.insert([{ name: payload.name, role: payload.role, pin: payload.pin, is_active: payload.is_active }]).select().maybeSingle();
