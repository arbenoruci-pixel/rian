/* TEPIHA SW Navigation Flight Recorder V34.9 — auto update activation + black-screen safe reload, no business storage deletion. */
/* eslint-disable no-restricted-globals */
(function () {
  'use strict';

  var NAV_DIAG_CACHE = 'tepiha-sw-navigation-diag-v1';
  var NAV_SHELL_CACHE = 'tepiha-sw-navigation-shell-v35';
  var NAV_ASSET_CACHE = 'tepiha-sw-navigation-assets-v35';
  ...
  var APP_EPOCH = 'RESET-2026-06-20-PRANIMI-CODE-RESERVATION-V37';
  var APP_VERSION = '2.0.50-pranimi-code-reservation-v37';
  var SW_NAV_DIAG_VERSION = 'sw-navigation-diag-v34.9-auto-update-black-screen-guard';
  var SAFE_SCREEN_VERSION = 'safe-screen-v34.9-auto-update-black-screen-guard';
  ...
  var AUTO_UPDATE_PREPARE_MESSAGE = 'TEPIHA_PWA_AUTO_UPDATE_PREPARE_RELOAD';
  var AUTO_UPDATE_RELOAD_MESSAGE = 'TEPIHA_PWA_AUTO_UPDATE_RELOAD_NOW';
  var AUTO_UPDATE_CLIENT_RELOAD_DELAY_MS = 900;

  function isAutoUpdateNavigation(request) {
    try {
      var u = new URL(request && request.url ? request.url : '/', self.location.origin);
      return u.searchParams.has(AUTO_UPDATE_RELOAD_PARAM);
  ...
  function autoUpdateNavigationUrl(rawUrl) {
    try {
      ...
      return u.href;
    } catch (_) { return absoluteUrl('/?' + AUTO_UPDATE_RELOAD_PARAM + '=' + Date.now()); }
  }
  ...
  async function notifyClientsAndNavigateForAutoUpdate(reason) {
    try {
      if (!self.clients || !self.clients.matchAll) return { count: 0, skipped: true };
      var clients = await self.clients.matchAll({ type: 'window', includeUncontrolled: true });
      if (!Array.isArray(clients) || !clients.length) return { count: 0, skipped: false };
      var payload = {
        type: AUTO_UPDATE_PREPARE_MESSAGE,
        reason: safeString(reason || 'service_worker_activate'),
      ...
      for (var i = 0; i < clients.length; i += 1) {
        try { clients[i].postMessage(payload); } catch (_) {}
      }
      ...
      for (var j = 0; j < clients.length; j += 1) {
        try {
          ...
          var nextUrl = autoUpdateNavigationUrl(client && client.url);
          if (nextUrl && client && typeof client.navigate === 'function') {
            await client.navigate(nextUrl);
            navigated += 1;
          } else if (client && client.postMessage) {
            client.postMessage(Object.assign({}, payload, { type: AUTO_UPDATE_RELOAD_MESSAGE }));
          }
        } catch (_) {}
      }
      return { count: clients.length, navigated: navigated, skipped: false };
    } catch (_) { return { count: 0, skipped: true, error: true }; }
  }
  ...
  async function handleNavigation(event) {
    ...
    var autoUpdateNav = isAutoUpdateNavigation(request);
    if (autoUpdateNav) {
      entry.autoUpdateNavigation = true;
      addAttempt(entry, 'auto_update_navigation', 'network_first_forced', { reloadParam: AUTO_UPDATE_RELOAD_PARAM });
    }

    var earlyCachedShell = autoUpdateNav ? null : await matchCachedShellCandidates(request, entry);
    if (earlyCachedShell) {
      entry.durationMs = Date.now() - startedAt;
  ...
  self.addEventListener('activate', function (event) {
    try {
      event.waitUntil((async function () {
        ...
        await notifyClientsAndNavigateForAutoUpdate('sw_navigation_diag_activate_auto_update_v3');
      })());
    } catch (_) {}
  });
  ...
  self.addEventListener('message', function (event) {
    try {
      ...
      if (safeString(data && data.type) === 'SKIP_WAITING') {
        try { event.waitUntil(Promise.resolve(self.skipWaiting()).catch(function () {})); } catch (_) { try { self.skipWaiting(); } catch (__) {} }
      }
    } catch (_) {}
  });
