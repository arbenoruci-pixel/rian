...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name, approved_by_pin, approved_by_role }){
  ...
  try {
    ...
    if (!up.data || up.data.length === 0) {
      // If nothing updated, it's either (a) row missing, (b) already APPLIED, or (c) truly blocked by RLS.
      // Check existence to give a clear reason and to make retries idempotent.
      ...
      if (chk?.data && chk.data.length > 0 && chk.data[0]?.status === 'APPLIED') {
        return { ok: true, already: true };
      }
      if (chk?.data && chk.data.length === 0) throw new Error('PENDING_NOT_FOUND');
