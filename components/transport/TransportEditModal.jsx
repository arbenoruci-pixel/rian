...
export default function TransportEditModal({ open, item, onClose, onSaved }) {
  ...
  const code = item?.code_str || data?.code_str || data?.order_code || data?.order_tcode || data?.official_order_code || item?.order_code || item?.client_tcode || data?.client_tcode || data?.client?.tcode || data?.client?.code || item?.code || '';
  ...
  async function save() {
    ...
    try {
      ...
      await updateTransportOrderById(item.id, {
        data: d,
        client_name: nm || null,
