...
export default function ReadyBonusLiveCard({ actor = null, style = null }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      inFlight = true;
      try {
        const data = await getBaseReadyBonusSummary({ actorPin: pin, workerPin: manager ? 'ALL' : pin, allowCache: true });
  ...
  const rate = Number(config?.rate_m2 || 0.10) || 0.10;

  return (
    <div data-ready-bonus-live-card="2" style={{ marginBottom:14,padding:12,borderRadius:16,border:'1px solid rgba(250,204,21,.34)',background:'linear-gradient(135deg,rgba(113,63,18,.24),rgba(15,23,42,.88))',display:'grid',gap:9,...(style||{}) }}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10}}>
        <div>
          <div style={{fontSize:10,fontWeight:1000,letterSpacing:'.12em',color:'#fde68a'}}>BONUSI {windowHours}H • VETËM BAZA</div>
          <div style={{marginTop:3,fontSize:19,lineHeight:1,fontWeight:1000}}>{manager ? 'KREJT PUNËTORËT' : 'BONUSI IM LIVE'}</div>
          <div style={{marginTop:5,fontSize:10,color:'#cbd5e1',fontWeight:800}}>{rate.toFixed(2)}€ / m² • afati dhe mundësitë lexohen direkt nga DB • rifreskohet automatikisht</div>
        </div>
        <Link href="/arka/bonuset" style={{textDecoration:'none',padding:'9px 11px',borderRadius:11,background:'#ca8a04',color:'#fff',fontSize:10,fontWeight:1000,whiteSpace:'nowrap'}}>HAPE</Link>
      </div>

      {summary ? <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:6}}>
        <div style={{padding:8,borderRadius:11,background:'rgba(2,6,23,.55)'}}><div style={{fontSize:8,color:'#94a3b8',fontWeight:1000}}>SOT</div><div style={{marginTop:3,fontSize:17,fontWeight:1000}}>{euro(totals?.today_earned)}</div><div style={{fontSize:8,color:'#94a3b8',fontWeight:800}}>{m2(totals?.today_m2)}</div></div>
        <div style={{padding:8,borderRadius:11,background:'rgba(2,6,23,.55)'}}><div style={{fontSize:8,color:'#94a3b8',fontWeight:1000}}>MUAJI</div><div style={{marginTop:3,fontSize:17,fontWeight:1000}}>{euro(totals?.month_earned)}</div><div style={{fontSize:8,color:'#94a3b8',fontWeight:800}}>{totals?.month_orders || 0} porosi</div></div>
...
// READY_BONUS_LIVE_CARD_V2 — labels use the live DB config instead of a hardcoded window.
