...
export default function ApprovalsPopup() {
  ...
  useEffect(() => {
    ...
    const currentUserKey = `${String(user?.pin || '')}|${String(user?.role || '')}`;
    const shouldForce = lastUserKeyRef.current !== currentUserKey;
    lastUserKeyRef.current = currentUserKey;

    const run = () => {
      loadOnce(shouldForce ? user : undefined);
    };
    ...
    const onFocus = () => scheduleLoad(2200);
    const onOnline = () => scheduleLoad(2600);
    const onVisible = () => {
      try {
        if (document.visibilityState === 'visible') scheduleLoad(2200);
      } catch {}
    };
