...
export default function PastrimiPaymentPurposeWizard({ open, code, clientName, due, cashGiven, busy, onChoose, onBack }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="pastrimi-payment-purpose-title" data-pastrimi-payment-purpose-wizard="1" style={{position:'fixed',inset:0,zIndex:120,background:'rgba(2,6,23,.90)',display:'grid',placeItems:'center',padding:12}}>
      <div style={{width:'min(560px,100%)',border:'1px solid rgba(96,165,250,.38)',borderRadius:22,background:'linear-gradient(180deg,#111827,#020617)',color:'#fff',padding:16,boxShadow:'0 24px 90px rgba(0,0,0,.72)'}}>
        ...
        <div style={{display:'grid',gap:10,marginTop:14}}>
          <button type="button" disabled={busy || pickupBlocked} onClick={() => onChoose?.(PASTRIMI_PAYMENT_PURPOSE.PICKUP_NOW)} style={{minHeight:72,borderRadius:15,border:'1px solid rgba(74,222,128,.48)',background:pickupBlocked?'#334155':'linear-gradient(135deg,#166534,#15803d)',color:'#fff',fontWeight:1000,fontSize:16,textAlign:'left',padding:'10px 13px',opacity:busy?.7:1}}>
          ...
          <button type="button" disabled={busy} onClick={() => onChoose?.(PASTRIMI_PAYMENT_PURPOSE.PREPAY)} style={{minHeight:72,borderRadius:15,border:'1px solid rgba(96,165,250,.48)',background:'linear-gradient(135deg,#1e3a8a,#1d4ed8)',color:'#fff',fontWeight:1000,fontSize:16,textAlign:'left',padding:'10px 13px',opacity:busy?.7:1}}>
        ...
        <button type="button" disabled={busy} onClick={onBack} style={{width:'100%',minHeight:46,marginTop:12,borderRadius:13,border:'1px solid rgba(148,163,184,.28)',background:'rgba(15,23,42,.75)',color:'#e2e8f0',fontWeight:950}}>← KTHEHU TE SHUMA</button>
      </div>
    </div>
  );
}
