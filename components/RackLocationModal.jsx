...
export default function RackLocationModal({
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: '#0b0b0b', zIndex: 10001, display: 'flex', flexDirection: 'column' }}
    >
      <div
        style={{
          padding: 14,
      ...
      >
        <div>
          <div style={{ fontWeight: 900, fontSize: 18 }}>
            {title}{orderCode ? ` (KODI: ${orderCode})` : ''}
          </div>
      ...
      <div style={{ padding: '16px 14px', overflow: 'auto', flex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
          {RACK_SPOTS.map((s) => {
            ...
            const otherOwners = owners.filter((x) => String(x.orderId) !== String(orderId));
        ...
        {placeErr && (
          <div style={{ marginTop: 10, color: '#ef4444', fontWeight: 800, fontSize: 13, background: 'rgba(239, 68, 68, 0.1)', padding: 8, borderRadius: 8 }}>
            {placeErr}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: 14, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 10 }}>
        ...
        <button className="btn primary" onClick={onSave} disabled={busy} style={{ flex: 2, fontWeight: 900 }}>
          {busy ? 'DUKE RUAJTUR...' : saveLabel}
        </button>
      </div>
    </div>
  );
