...
-- A second client-side retry loop happened after the first audited incident:
-- T1289 completed, T1290..T1298 were reserved without any lifecycle row, and
-- T1299 completed. Keep this repair exact so no older ambiguous code is reused.
...
create temporary table dispatch_tcode_cleanup_v2_expected (
  code text primary key,
  code_n bigint not null unique
) on commit drop;

insert into dispatch_tcode_cleanup_v2_expected(code, code_n)
select 'T' || n::text, n
from generate_series(1290::bigint, 1298::bigint) as n;

create table if not exists public.backup_dispatch_tcode_stranded_cleanup_20260830_v2 (
  code text primary key,
  owner_id text,
  status text,
...
alter table public.backup_dispatch_tcode_stranded_cleanup_20260830_v2
  enable row level security;

revoke all on table public.backup_dispatch_tcode_stranded_cleanup_20260830_v2
...
begin
  ...
  from dispatch_tcode_cleanup_v2_expected;

  if v_expected_count <> 9 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_EXPECTED_SET_CHANGED:%', v_expected_count;
  end if;

  select count(*) into v_pool_count
  from dispatch_tcode_cleanup_v2_expected e
  join public.transport_code_pool p
    on upper(btrim(p.code)) = e.code;

  if v_pool_count <> 9 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_POOL_SET_CHANGED:%', v_pool_count;
  end if;
  ...
  where p.created_at is null
     or p.created_at < '2026-08-30 21:59:47.095842+00'::timestamptz
     or p.created_at > '2026-08-30 22:01:49.970515+00'::timestamptz
     or not (
  ...
  if v_bad_state_codes is not null then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_STATE_OR_TIME_CHANGED:%', v_bad_state_codes;
  end if;
  ...
  where exists (
    ...
    -- Any lease history is a blocker, including terminal leases.
    or exists (
      select 1
      ...
      where upper(btrim(coalesce(k.code_str, ''))) = e.code
         ...
         or exists (
           ...
           from jsonb_each_text(
             case when jsonb_typeof(k.data) = 'object' then k.data else '{}'::jsonb end
  ...
  if v_blocked_codes is not null then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_REFERENCE_BLOCKERS:%', v_blocked_codes;
  end if;

  select count(*) into v_protected_count
  from (values
    ('T1289'::text, 1289::bigint),
    ('T1299'::text, 1299::bigint)
  ...
  if v_protected_count <> 2 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_PROTECTED_SET_CHANGED:%', v_protected_count;
  end if;
end;
$$;

insert into public.backup_dispatch_tcode_stranded_cleanup_20260830_v2(
  code,
  owner_id,
  status,
  created_at
...
begin
  ...
  from dispatch_tcode_cleanup_v2_expected e
  join public.backup_dispatch_tcode_stranded_cleanup_20260830_v2 b
    on upper(btrim(b.code)) = e.code;

  if v_backup_count <> 9 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_BACKUP_INCOMPLETE:%', v_backup_count;
  end if;
  ...
  where upper(btrim(p.code)) = e.code
    ...
    and p.created_at between
      '2026-08-30 21:59:47.095842+00'::timestamptz
      and '2026-08-30 22:01:49.970515+00'::timestamptz;

  get diagnostics v_updated = row_count;

  if v_updated <> v_to_update then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_UPDATE_COUNT_CHANGED:%/%', v_updated, v_to_update;
  end if;
  ...
  if v_postcheck_count <> 9 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_POSTCHECK_FAILED:%', v_postcheck_count;
  end if;
  ...
  if v_protected_count <> 2 then
    raise exception 'DISPATCH_TCODE_CLEANUP_V2_PROTECTED_POSTCHECK_FAILED:%', v_protected_count;
  end if;
end;
$$;
