...
-- V2 wrote DELIVERED_WITH_PAYMENT before FIFO allocation. Only repair events
-- proven to be fully settled by confirmed allocations; ambiguous/partial rows
-- remain untouched for manual review.
with correctable as (
  select
    e.id,
    round(coalesce(sum(case when b.status = 'CONFIRMED' then a.amount else 0 end), 0), 2) as settled_cash,
    round(r.outstanding_amount, 2) as remaining_debt,
    round(r.original_amount - r.opening_paid_amount, 2) as collectible_amount
  from public.transport_delivery_events e
  join public.transport_receivables r on r.id = e.receivable_id
  left join public.transport_payment_allocations a on a.receivable_id = r.id
  left join public.transport_payment_batches b on b.id = a.batch_id
  where e.event_type = 'DELIVERED_WITH_PAYMENT'
    and e.debt_created > 0
    and r.status = 'PAID'
    and r.outstanding_amount = 0
  group by e.id, r.outstanding_amount, r.original_amount, r.opening_paid_amount
  having round(coalesce(sum(case when b.status = 'CONFIRMED' then a.amount else 0 end), 0), 2)
    = round(r.original_amount - r.opening_paid_amount, 2)
)
...
select
  ...
  to_jsonb(e)
from public.transport_delivery_events e
join correctable c on c.id = e.id
...
set cash_received = c.settled_cash,
    debt_created = c.remaining_debt
from correctable c
where e.id = c.id;
