...
export function isKnownRackSlotKey(value) {
  const key = normalizeRackSlotKey(value);
  if (!key) return false;
  return RACK_SPOTS.includes(key) || OVERFLOW_SPOTS.includes(key);
}

function extractRackSlotCandidates(value) {
  const raw = String(value || '');
  const out = [];
  const overflowKey = parseOverflowLocation(raw);
  if (overflowKey) out.push(overflowKey);

  const plain = stripMarks(raw).toUpperCase();
  const normalMatches = plain.matchAll(/(?:^|[^A-Z0-9])([AB])\s*-?\s*(\d{1,2})(?=$|[^A-Z0-9])/g);
  for (const match of normalMatches) {
    const slot = `${match[1]}${Number(match[2])}`;
    if (isKnownRackSlotKey(slot)) out.push(slot);
  }

  const overflowMatches = plain.matchAll(/FURRA\s*(POSHT|NALT|NALTE|LART)[^A-Z0-9]*A\s*-?\s*(\d{1,2})/g);
  for (const match of overflowMatches) {
    const room = match[1] === 'POSHT' ? 'FURRA_POSHT' : 'FURRA_NALT';
    const slot = buildOverflowLocation(room, `A${Number(match[2])}`);
    if (slot && isKnownRackSlotKey(slot)) out.push(slot);
  }

  return Array.from(new Set(out));
}

export function hasConcreteRackLocation(value) {
  return normalizeRackSlots(value).length > 0;
}
...
export function normalizeRackSlots(value) {
  ...
  for (const item of raw || []) {
    ...
    if (cleaned && isKnownRackSlotKey(cleaned) && !out.includes(cleaned)) {
      out.push(cleaned);
      continue;
    }
    for (const candidate of extractRackSlotCandidates(item)) {
