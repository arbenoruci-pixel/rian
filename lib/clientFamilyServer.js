...
export async function familyAction(body, { supabase, authUser, secret, now } = {}) {
  ...
  if (['PUBLIC_INFO', 'PUBLIC_ADD'].includes(action)) {
    const token = verifyFamilyToken(body.token, secret, now);
  ...
  if (action === 'SIGN_LINK') {
    ...
    if (!family) throw new FamilyError('FAMILY_NOT_AVAILABLE', 503);
    return { token: signFamilyToken({ source: body.source, orderId: order.id, clientId: order.client_id }, secret, now) };
  }
