...
import { cleanClientLocation, orderClientLocation } from './clientLocation.js';
...
export async function familyAction(body, { supabase, authUser, secret, now } = {}) {
  ...
  if (['PUBLIC_INFO', 'PUBLIC_ADD', 'PUBLIC_LOCATION'].includes(action)) {
    ...
    if (action === 'PUBLIC_LOCATION') {
      if (!uuid(body.requestId)) throw new FamilyError('FAMILY_LOCATION_INVALID');
      let location;
      try { location = cleanClientLocation(body.location); } catch { throw new FamilyError('FAMILY_LOCATION_INVALID'); }
      const row = { request_id: body.requestId, source: token.source, order_id: token.orderId, client_id: token.clientId, ...location };
      const { error } = await supabase.from('client_family_locations').upsert(row, { onConflict: 'request_id', ignoreDuplicates: true });
      if (error) throw new FamilyError('FAMILY_LOCATION_SAVE_FAILED', 503);
      const { data: saved, error: readError } = await supabase.from('client_family_locations').select('*').eq('request_id', body.requestId).maybeSingle();
      if (readError || !saved) throw new FamilyError('FAMILY_LOCATION_SAVE_FAILED', 503);
      if (Object.entries(row).some(([key, value]) => saved[key] !== value)) throw new FamilyError('FAMILY_LOCATION_CONFLICT', 409);
      return { saved: true };
    }
  ...
  if (action === 'GET_LOCATION') {
    ...
    const { data, error } = await supabase.from('client_family_locations')
      .select('address,latitude,longitude,created_at,order_id')
      .eq('source', body.source).eq('client_id', order.client_id).eq('order_id', String(order.id))
      .order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (error) throw new FamilyError('FAMILY_LOCATION_READ_FAILED', 503);
    return { location: data || orderClientLocation(order) };
  }
