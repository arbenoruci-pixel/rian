// ARKA RPC-only submit contract: rpcOnly: true
...
import { matchesPayrollRecipient } from '@/lib/payrollMonthClose';
// DISPATCH_OWN_HANDOFF_BONUS_BYPASS_V2: cash handoff is independent from worker-only bonus eligibility.
// BASE_READY_48H_BONUS_V1:FINANCE
...
function verifyHandoffSubmitResponse(result = {}, paymentIds = [], options = {}) {
  ...
  let items = Array.isArray(handoff?.cash_handoff_items) ? handoff.cash_handoff_items : [];

  // HANDOFF_RPC_VERIFICATION_FALLBACK_V1
  if (!items.length) {
...
export async function submitWorkerCashToDispatch({ actor, note = '', amountOverride = null }) {
  ...
  try {
    ...
    const [mealRows, readyBonusRows] = await Promise.all([
      ...
      listOpenBaseReadyBonusPayments(pin, { allowNonWorker: true }),
    ]);
...
export async function listWorkerDebtRows(pin, limit = 200) {
  ...
  ].filter(row => ![row?.status, row?.type].some(value => clean(value).toUpperCase() === 'ADVANCE')
    || matchesPayrollRecipient(row, { pin: cleanPin }));
