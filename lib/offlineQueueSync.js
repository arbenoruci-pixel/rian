...
import { getAllOrdersLocal, removeOrderLocal } from "@/lib/offlineStore";
...
async function purgeLocalByCode(code){
  const c = String(code || '').trim();
  if(!c) return;
  try{
    const rows = await getAllOrdersLocal();
    for(const o of (rows||[])){
      const oc = String(o?.code ?? o?.code_n ?? o?.data?.code ?? '').trim();
      if(!oc) continue;
      if(oc === c && (o?._synced === false || o?._local)){
        // eslint-disable-next-line no-await-in-loop
        await removeOrderLocal(o.id);
      }
    }
  }catch{}
}
      ...
      try {
        ...
        try {
          const res = await fetch("/api/offline-sync", {
            ...
            body: JSON.stringify({ type: "insert_order", payload: order }),
          });
        ...
        if (!ok) {
          ...
          ok = true; // if saveOrderToDb didn't throw, treat as ok
        }

        // If synced, purge any local draft copies for the same code to prevent duplicates in lists
        if (ok) {
          await purgeLocalByCode(order?.code);
        }
