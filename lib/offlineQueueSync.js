  ...
  try {
    ...
    for (const item of q) {
      try {
          ...
          if (!ok) {
            ...
            try {
              localStorage.setItem(
                ...
                JSON.stringify({ ts: Date.now(), type: 'offline_pranimi', message: j?.error || 'ROUTE_OK_FALSE', meta: j || null })
              );
            } catch {}
          }
        } catch (e) {
        ...
        if (!ok) {
          const r = await saveOrderToDb(order, "OFFLINE_SYNC");
          // If saveOrderToDb could not reach DB, it returns _offline:true.
          // In that case we MUST keep the queue item so it can retry later.
          if (r && r._offline) {
            keep.push(item);
            continue;
          }
        ...
        try {
          localStorage.setItem(
            'tepiha_last_sync_error',
            JSON.stringify({ ts: Date.now(), type: 'offline_pranimi', message: String(e?.message || e), meta: null })
          );
        } catch {}
        keep.push(item);
      }
