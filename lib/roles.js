...
// Keep UI capabilities and server authorization on the same role contract.
export function canTrackReadyNotifications(role) {
  return [...ADMIN_ROLES, 'MASTER', 'PUNTOR', 'PUNETOR', 'WORKER', 'BAZIST', 'BASE'].includes(normalizeRole(role));
}

export function canManageCustomerCare(role) {
  return [...ADMIN_ROLES, 'MASTER'].includes(normalizeRole(role));
}

export function canUseCustomerCare(role, orderId = '') {
  return canManageCustomerCare(role) || (normalizeRole(role) === 'TRANSPORT' && !!orderId);
}
