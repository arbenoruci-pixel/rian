...
const ACTIVE_DELIVERY_STATUSES = new Set(['ACCEPTED', 'PENDING_DISPATCH_APPROVAL']);
const MONEY_RE = /(\d+(?:[.,]\d+)?)/;
...
export function buildWorkerArkaSummary({
  ...
  const toHandoverToday = Math.max(0, collectedTotal + timaTotal - expenseTotal - mealSelfTotal - hybridCommissionCollected);
  const toHandoverWithPending = Math.max(0, collectedTotal + pendingTotal + timaTotal - expenseTotal - mealSelfTotal - hybridCommissionWithPending);
