...
const ACTIVE_DELIVERY_STATUSES = new Set(['ACCEPTED', 'ACCEPTED_BY_DISPATCH']);
const MONEY_RE = /(\d+(?:[.,]\d+)?)/;
...
function workerKeyMatch(row, pin) {
  ...
  if (type === 'MEAL_PAYMENT') return [row?.created_by_pin, row?.worker_pin].some((value) => cleanText(value) === p);
  if (type === 'MEAL_COVERED') return cleanText(row?.created_by_pin) === p;
...
export function buildWorkerArkaSummary({
  ...
  const lastAcceptedAt = allHandoffs
    .filter((row) => ACTIVE_DELIVERY_STATUSES.has(safeUpper(row?.status)))
