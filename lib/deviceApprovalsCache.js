...
export function cacheApprovedLogin({ pin, role, deviceId, actor }) {
  if (!isBrowser()) return;
  // A retired PIN is never an authentication alias, even if an old PWA once
  // cached a device approval for it.
  if (getRetiredStaffPinAlias(pin)) {
    purgeRetiredStaffPinCaches(localStorage, pin);
    return;
  }
  try {
