...
export function getDispatchOutbox() {
  if (!outbox) outbox = createDispatchOutbox({
    ...
    onChange: changed,
    onIssue: ({ code, id, stage }) => reportDispatchDiagnostic('queue_issue', { code, requestId: id, storageStage: stage }),
    onCommitted: (item, result) => {
      reportDispatchDiagnostic('confirmed', { requestId: item.id, orderId: result.data.id, attempts: item.attempts + 1 });
      createDispatchCreateIntentJournal().clear(item.id);
      window.dispatchEvent(new CustomEvent('tepiha:dispatch-order-committed', {
        detail: { id: item.id, actorId: item.actorId, order: result.data },
      }));
    },
  });
  return outbox;
...
export function wakeDispatchOutbox({ force = false } = {}) {
  try {
    void getDispatchOutbox().drain({ force }).catch(() => {
...
export function installDispatchOutbox() {
  ...
  installed = true;
  let resumeTimer = null, wasHidden = document.hidden;
  const reconnect = () => {
    if (navigator.onLine === false || document.hidden || resumeTimer !== null) return;
    // online/pageshow/visibility can arrive together. Retry pending work once;
    // ordinary focus and polling retain the existing backoff.
    resumeTimer = window.setTimeout(() => {
      resumeTimer = null;
      if (navigator.onLine !== false && !document.hidden) wakeDispatchOutbox({ force: true });
    }, 0);
  };
  ...
  window.addEventListener('pageshow', event => { if (event.persisted) reconnect(); else wakeDispatchOutbox(); });
