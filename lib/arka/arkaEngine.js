...
async function readCommittedBaseBatchRetryActor(sb, payload = {}, actorPin = '') {
  const linkedDebts = Array.isArray(payload.linkedDebts || payload.linked_debts)
    ? (payload.linkedDebts || payload.linked_debts)
    : [];
  const idempotencyKey = cleanText(payload.idempotencyKey || payload.idempotency_key, '');
  if (!linkedDebts.length || !idempotencyKey || !actorPin) return null;
  try {
    const { data, error } = await sb
      .from('base_payment_batches')
      .select('created_by_pin,created_by_name,created_by_role')
      .eq('idempotency_key', idempotencyKey)
      .maybeSingle();
    if (error) throw error;
    if (normalizePin(data?.created_by_pin) !== normalizePin(actorPin)) return null;
    return {
      id: null,
      pin: normalizePin(data.created_by_pin),
      name: cleanText(data.created_by_name, actorPin),
      role: cleanText(data.created_by_role, ''),
    };
  } catch (error) {
    if (isMissingColumnOrFunctionError(error)) return null;
    throw error;
  }
}
...
async function assertActorAllowed(sb, payload = {}, action = '') {
  ...
  if (!user?.pin || (Object.prototype.hasOwnProperty.call(user, 'is_active') && user.is_active === false)) {
    if (action === ARKA_ACTION.BASE_ORDER_PAYMENT) {
      const retryActor = await readCommittedBaseBatchRetryActor(sb, payload, actorPin);
      if (retryActor) return retryActor;
    }
...
function normalizeTransportPaymentFullStatus(value = '') {
    const status = cleanText(value, '').toLowerCase();
  ...
  function buildTransportPatch(row = {}, amount, opts = {}) {
    // TRANSPORT_PAYMENT_CONFIRM_CLOSE_V1:ENGINE — delivery cash and terminal status are committed by one server transaction.
    const { duplicate = false } = opts || {};
    const data = asObject(row?.data);
    const pay = asObject(data?.pay);
...
async function activateBaseReadyBonusAfterPaymentV2(sb, { order, payment, actor } = {}) {
...
// BASE_PAYMENT_48H_BONUS_V2:ENGINE — bonus activation is attached to the verified base payment.

function normalizeBaseClientDebtSnapshot(payload = {}) {
  const source = Array.isArray(payload.linkedDebts || payload.linked_debts)
    ? (payload.linkedDebts || payload.linked_debts)
    : [];
  const byId = new Map();
  for (const raw of source) {
    const orderId = normalizeDbId(raw?.orderId || raw?.order_id || raw?.id);
    const debt = money(raw?.debt ?? raw?.expectedDebt ?? raw?.expected_debt);
    if (!orderId || debt <= 0) continue;
    byId.set(String(orderId), { orderId, debt });
  }
  return Array.from(byId.values()).sort((a, b) => Number(a.orderId) - Number(b.orderId));
}
...
async function baseOrderPayment(sb, payload = {}) {
  ...
  const paymentOutcome = upper(payload.paymentOutcome || payload.payment_outcome || '');
  const expectedDebt = money(payload.expectedDebt ?? payload.expected_debt);
  const statusOnFullPayment = normalizeBasePaymentFullStatus(
    payload.statusOnFullPayment || payload.status_on_full_payment || payload.fullPaymentStatus || payload.full_payment_status
  );
  if (!['PREPAY_STAYS_PASTRIMI', 'CLIENT_PICKED_UP_TO_DORZIM', 'DELIVERY_TO_DORZIM'].includes(paymentOutcome)) {
    throw new Error('PAYMENT_OUTCOME_REQUIRED');
  }
  if (expectedDebt <= 0) throw new Error('EXPECTED_DEBT_REQUIRED');
  const linkedDebts = normalizeBaseClientDebtSnapshot(payload);
  const canonicalClientId = cleanText(payload.clientId || payload.client_id, '');
  if (linkedDebts.length && !canonicalClientId) throw new Error('CANONICAL_CLIENT_ID_REQUIRED');
  const useClientSettlement = linkedDebts.length > 0 && Boolean(canonicalClientId);
  const linkedTotal = money(linkedDebts.reduce((sum, item) => sum + money(item.debt), 0));
  if (useClientSettlement && Math.abs(linkedTotal - expectedDebt) > 0.009) {
    throw new Error('BASE_CLIENT_EXPECTED_TOTAL_MISMATCH');
  }
  const lockKey = `base_cash:${linkedDebts.map((item) => item.orderId).join('-') || orderId}:${idempotentMoneyKey(amount)}:${actor.pin}`;

  return withRuntimeLock(lockKey, async () => {
    if (useClientSettlement) {
      const cashGiven = money(payload.cashGiven ?? payload.cash_given ?? amount);
      const changeAmount = money(payload.changeAmount ?? payload.change_amount ?? Math.max(0, cashGiven - amount));
      const { data: rpcResult, error: rpcError } = await sb.rpc('record_base_client_cash_payment_atomic_v1', {
        p_anchor_order_id: orderId,
        p_amount: amount,
        p_cash_given: cashGiven,
        p_change_amount: changeAmount,
        p_expected_total_debt: expectedDebt,
        p_expected_order_debts: linkedDebts.map((item) => ({ orderId: item.orderId, debt: item.debt })),
        p_expected_client_id: canonicalClientId || null,
        p_actor_pin: actor.pin,
        p_actor_name: actor.name || null,
        p_actor_role: actor.role || null,
        p_order_code: cleanText(payload.orderCode || payload.order_code || payload.code, '') || null,
        p_client_name: cleanText(payload.clientName || payload.client_name, '') || null,
        p_client_phone: cleanText(payload.clientPhone || payload.client_phone, '') || null,
        p_note: cleanText(payload.note, `PAGESA E KLIENTIT ${amount}€`),
        p_payment_outcome: paymentOutcome,
        p_status_on_full_payment: statusOnFullPayment,
        p_idempotency_key: idempotencyKey,
      });
      if (rpcError) throw rpcError;
      if (!rpcResult?.ok || !rpcResult?.payment?.id || !rpcResult?.order?.id || !rpcResult?.batch?.id) {
        throw new Error(rpcResult?.error || 'BASE_CLIENT_ATOMIC_PAYMENT_VERIFY_FAILED');
      }

      const payments = Array.isArray(rpcResult.payments) ? rpcResult.payments : [rpcResult.payment];
      const updatedOrders = Array.isArray(rpcResult.orders) ? rpcResult.orders : [rpcResult.order];
      const allocations = Array.isArray(rpcResult.allocations) ? rpcResult.allocations : [];
      const paymentByOrder = new Map(payments.map((payment) => [String(payment?.order_id || ''), payment]));
      const orderById = new Map(updatedOrders.map((order) => [String(order?.id || ''), order]));
      const readyBonusResults = [];
      for (const allocation of allocations) {
        if (money(allocation?.debt_after) > 0) continue;
        const settledOrder = orderById.get(String(allocation?.order_id || ''));
        const settledPayment = paymentByOrder.get(String(allocation?.order_id || ''));
        if (!settledOrder?.id || !settledPayment?.id) continue;
        try {
          const result = await activateBaseReadyBonusAfterPaymentV2(sb, { order: settledOrder, payment: settledPayment, actor });
          readyBonusResults.push({ orderId: settledOrder.id, ok: true, result });
        } catch (error) {
          readyBonusResults.push({ orderId: settledOrder.id, ok: false, error: String(error?.message || error) });
        }
      }
      const anchorBonus = readyBonusResults.find((item) => String(item.orderId) === String(orderId));
      return {
        ok: true,
        action: ARKA_ACTION.BASE_ORDER_PAYMENT,
        duplicate: rpcResult.duplicate === true,
        existing: rpcResult.duplicate === true,
        payment: rpcResult.payment,
        row: rpcResult.payment,
        payments,
        verifiedPayment: rpcResult.payment,
        paymentVerified: true,
        order: rpcResult.order,
        orders: updatedOrders,
        allocations,
        batch: rpcResult.batch,
        idempotencyKey,
        paymentOutcome,
        readyBonusResults,
        readyBonusResult: anchorBonus?.result || null,
        readyBonus: anchorBonus?.result?.bonus || null,
        readyBonusActivated: anchorBonus?.result?.activated === true,
        readyBonusActivationError: anchorBonus?.ok === false ? anchorBonus.error : null,
        needsBonusRepair: readyBonusResults.some((item) => item.ok === false),
      };
    }

    const { data: rpcResult, error: rpcError } = await sb.rpc('record_base_order_cash_payment_atomic_v1', {
      p_order_id: orderId,
      p_amount: amount,
      p_expected_debt: expectedDebt,
      p_actor_pin: actor.pin,
      p_actor_name: actor.name || null,
      p_actor_role: actor.role || null,
      p_order_code: cleanText(payload.orderCode || payload.order_code || payload.code, '') || null,
      p_client_name: cleanText(payload.clientName || payload.client_name, '') || null,
      p_client_phone: cleanText(payload.clientPhone || payload.client_phone, '') || null,
      p_note: cleanText(payload.note, `PAGESA ${amount}€`),
      p_payment_outcome: paymentOutcome,
      p_status_on_full_payment: statusOnFullPayment,
      p_idempotency_key: idempotencyKey,
    });
    if (rpcError) throw rpcError;
    if (!rpcResult?.ok || !rpcResult?.payment?.id || !rpcResult?.order?.id) {
      throw new Error(rpcResult?.error || 'BASE_ATOMIC_PAYMENT_VERIFY_FAILED');
    }
    const verifiedPayment = rpcResult.payment;
    const updatedOrder = rpcResult.order;
    const reusedExistingPayment = rpcResult.duplicate === true;
...
async function expenseRequest(sb, payload = {}) {
  ...
  const requestedExpenseDateKey = cleanText(payload.expenseDateKey || payload.expense_date_key, '');
  ...
  let expenseDateKey = '';
  if (paymentType === ARKA_PAYMENT_TYPE.EXPENSE && requestedExpenseDateKey) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(requestedExpenseDateKey)) throw new Error('EXPENSE_DATE_INVALID');
    const todayKey = mealTodayBoundsIso(new Date()).dateKey;
    const [todayY, todayM, todayD] = todayKey.split('-').map((value) => Number(value));
    const yesterdayKey = new Date(Date.UTC(todayY, todayM - 1, todayD - 1, 12, 0, 0)).toISOString().slice(0, 10);
    if (![todayKey, yesterdayKey].includes(requestedExpenseDateKey)) throw new Error('EXPENSE_DATE_NOT_ALLOWED');
    expenseDateKey = requestedExpenseDateKey;
    if (expenseDateKey === yesterdayKey) {
      const [year, month, day] = expenseDateKey.split('-').map((value) => Number(value));
      createdAt = new Date(mealZonedLocalToUtcMs(ARKA_MEAL_TIME_ZONE, year, month, day, 12, 0, 0)).toISOString();
    }
  }
  const handoffNote = [
    cleanText(payload.handoffNote || payload.handoff_note, ''),
    expenseDateKey ? `DATA_OPERATIVE:${expenseDateKey}` : '',
  ].filter(Boolean).join(' | ');
...
async function countDistinctHandoffClients(sb, payments = []) {
  const rows = Array.isArray(payments) ? payments : [];
  const paymentIds = rows.map((row) => normalizeDbId(row?.id)).filter(Boolean);
  if (!paymentIds.length) return 0;
  const keyByPaymentId = new Map(paymentIds.map((id) => [String(id), `PAYMENT:${id}`]));
  try {
    const { data, error } = await sb
      .from('base_payment_allocations')
      .select('batch_id,arka_payment_id')
      .in('arka_payment_id', paymentIds);
    if (error) throw error;
    for (const allocation of Array.isArray(data) ? data : []) {
      const paymentId = normalizeDbId(allocation?.arka_payment_id);
      const batchId = cleanText(allocation?.batch_id, '');
      if (paymentId && batchId) keyByPaymentId.set(String(paymentId), `BASE_BATCH:${batchId}`);
    }
  } catch (error) {
    if (!isMissingColumnOrFunctionError(error)) throw error;
  }
  return new Set(keyByPaymentId.values()).size;
}
...
async function submitHandoffFallback(sb, payload = {}) {
  ...
  const clientCount = await countDistinctHandoffClients(sb, regularRows);
...
async function voidOrReversePayment(sb, payload = {}) {
  ...
  try {
    const { data: batchAllocation, error: allocationError } = await sb
      .from('base_payment_allocations')
      .select('batch_id')
      .eq('arka_payment_id', paymentId)
      .maybeSingle();
    if (allocationError) throw allocationError;
    if (batchAllocation?.batch_id) throw new Error('BASE_CLIENT_BATCH_VOID_REQUIRED');
  } catch (batchError) {
    if (String(batchError?.message || batchError) === 'BASE_CLIENT_BATCH_VOID_REQUIRED') throw batchError;
    if (!isMissingColumnOrFunctionError(batchError)) throw batchError;
  }
