...
export async function saveOrderToDb(order, optsOrSource = 'PRANIMI') {
  ...
  const insertRow = sanitizeRow(insertRowRaw);

  // OFFLINE FIRST
  if (!isOnline()) {
    await saveOrderLocal({ ...insertRow, id: localOrderId, _local: true, _synced: false });
  ...
  try {
    ...
    await saveOrderLocal({ ...insertRow, id: data.id, _local: false, _synced: true });
  ...
  const row = sanitizeRow(rowRaw);

  if (!isOnline()) {
    // Save in local mirror too (so UI can show it immediately)
    await saveOrderLocal({ ...row, id: oid, _local: true, _synced: false, table: 'transport_orders' });
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) throw error;
    await saveOrderLocal({ ...row, id: data.id, _local: false, _synced: true, table: 'transport_orders' });
    banishGhost(oid);
    return { order_id: data.id };
  } catch (e) {
    await saveOrderLocal({ ...row, id: oid, _local: true, _synced: false, table: 'transport_orders' });
...
export async function updateOrderInDb(id, patch = {}, opts = {}) {
  ...
  const cleanPatch = sanitizeRow({ ...(patch || {}), updated_at: new Date().toISOString() });
    ...
    if (data && data.length > 0) {
      ...
      setTimeout(async () => {
        for (const row of data) {
          try {
            await saveOrderLocal({ ...row, _local: false, _synced: true });
          } catch {}
        }
      }, 10);
    }
...
export async function updateOrderStatus(id, status) {
  ...
  const up = { status: statusClean, updated_at: new Date().toISOString() };
  
  // ✅ PËRDITËSIM LOKAL MENJËHERË: Kjo ndalon vonesën (ghosting) edhe kur je Online
  try {
    const current = await getAllOrdersLocal();
    const target = current.find(x => String(x.id) === String(id));
    if (target) {
      await saveOrderLocal({ ...target, status: statusClean });
    }
  } catch {}

  if (!isOnline()) {
    await pushOp({ op_id: `op_${Date.now()}`, type: "set_status", id, data: { status: statusClean } });
    return { ok: true, offline: true };
  }

  const { error } = await supabase.from('orders').update(up).eq('id', id);
  if (error) throw error;
  return { ok: true };
}
