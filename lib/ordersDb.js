...
function sanitizeRow(obj, table = '', opts = {}) {
  if (!obj || typeof obj !== 'object') return obj;
  const tableName = String(table || obj?.table || obj?._table || '').trim();
  if (tableName === 'transport_orders') {
    return sanitizeTransportOrderPayload(obj, { patch: opts?.patch === true });
  }
  const out = { ...obj };
...
export async function updateOrderInDb(id, patch = {}, opts = {}) {
  ...
  let cleanPatch = sanitizeRow(
    { ...(patch || {}), updated_at: new Date().toISOString(), table },
    table,
    { patch: true },
  );
