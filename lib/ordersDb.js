...
export async function saveOrderToDb(order, source = 'PRANIMI') {
  ...
  if (!isOnline()) {
    ...
    await pushOp({ op_id: `op_${Date.now()}`, type: "insert_order", payload: insertRow });
    return { order_id: localOrderId, _offline: true };
  }

  try {
    const { data, error } = await supabase.from('orders').upsert(insertRow, { onConflict: 'local_oid' }).select('id').single();
