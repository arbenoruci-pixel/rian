...
function queueOp(op){
  try {
    const q = JSON.parse(localStorage.getItem(LS_SYNC_QUEUE) || '[]');
    q.push({ ...op, _queued_at: new Date().toISOString() });
    localStorage.setItem(LS_SYNC_QUEUE, JSON.stringify(q));
  } catch {}
}

function isOffline(){
  try { return typeof navigator !== 'undefined' && navigator && navigator.onLine === false; } catch { return false; }
}
...
export async function saveOrderToDb(order, source = 'PRANIMI') {
  ...
  // OFFLINE: keep local order and enqueue for later sync
  if (isOffline()) {
    queueOp({ type: 'UPSERT_ORDER', table: 'orders', payload: insertRow, source });
    return { order_id: null, client_id: clientId, local: true };
  }
