      ...
      (local || []).forEach((o) => {
        ...
        if (!already || o._synced === false || o._local) {
          map.set(key, {
            ...
            total: Number(o.total ?? o.data?.pay?.euro ?? o.data?.pay?.total ?? o.data?.total ?? 0) || 0,
    ...
    try {
      ...
      if (Array.isArray(arr)) {
        arr.forEach((it) => {
          ...
          const key = String(p?.code || p?.code_n || p?.id || p?.oid || '');
          if (!key) return;
          if (!map.has(key)) {
            map.set(key, {
              id: p?.id || p?.oid || key,
              ...
              total: Number(p?.total ?? p?.data?.pay?.euro ?? p?.data?.pay?.total ?? p?.data?.total ?? 0) || 0,
              paid: Number(p?.paid ?? p?.data?.pay?.paid ?? 0) || 0,
              data: p?.data ?? null,
