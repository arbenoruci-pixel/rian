...
export function buildClientProfileSmartSmsOrder(profile, visit) {
  ...
  return {
    ...
    client_name: clean(visit?.contactName || client?.name) || 'klient',
    client_phone: clean(visit?.contactPhone || client?.phone),
    ...
    client: {
      name: clean(visit?.contactName || client?.name) || 'klient',
      phone: clean(visit?.contactPhone || client?.phone),
...
function buildNeutralStatusText(profile, visit) {
  const clientName = clean(visit?.contactName || profile?.client?.name) || 'klient';
  const status = normalizeStatus(visit?.status);
