  ...
  async function transaction(mode, action) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        forget(db);
        try { db.close(); } catch {}
        // A suspended browser may never deliver onabort. Release the click
        // handler on the deadline even in that case; never acknowledge a write.
        reject(failure);
      }, timeoutMs);
  ...
  return {
    async entries(prefix) {
      const saved = await transaction('readonly', (store, done) => {
        ...
        values.onsuccess = () => done(keys.result.map((key, i) => [key, values.result[i]]));
      });
      const rows = new Map(saved.filter(([key]) => String(key).startsWith(prefix)));
      for (let i = 0; i < (localStorage?.length || 0); i++) {
        const key = localStorage.key(i);
        if (!key?.startsWith(prefix)) continue;
        try { rows.set(key, newest(rows.get(key), localStorage.getItem(key))); }
        catch { if (!rows.has(key)) rows.set(key, null); } // Keep a valid committed copy; preserve the corrupt original.
      }
      return [...rows];
    },
    async getItem(key) {
    ...
    async removeItems(keys) {
      const legacy = new Map(keys.map(key => [key, localStorage?.getItem(key)]));
      await transaction('readwrite', store => { for (const key of keys) store.delete(key); });
      for (const [key, raw] of legacy) {
        try { if (raw && localStorage?.getItem(key) === raw) localStorage.removeItem(key); } catch {}
      }
    },
  };
}
