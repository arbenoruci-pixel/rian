...
export function getHomeSearchQueryMode(query) {
  ...
  // A complete phone number is not an order code, including digits-only paste.
  if (/^\+?\d{8,15}$/.test(normalizePhone(raw))) return 'GENERAL';
...
// A stalled read must release the worker even if the transport has no timeout API.
async function withHomeSearchDeadline(request, timeoutMs = 8000) {
  let timer;
  try {
    return await Promise.race([
      Promise.resolve(request),
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error('HOME_SEARCH_READ_TIMEOUT')), timeoutMs);
      }),
    ]);
  } finally {
    clearTimeout(timer);
...
async function runHomeExactDbQuery(factory, { attempts = 2, timeoutMs = 7000, label = 'HOME_SEARCH_EXACT_DB_TIMEOUT' } = {}) {
  ...
  for (let attempt = 1; attempt <= count; attempt += 1) {
    try {
      ...
      const response = await withHomeSearchDeadline(request, attempt === 1 ? timeoutMs : Math.max(timeoutMs, 12000));
...
async function getRowsFromDbGeneral(query, mode = getHomeSearchQueryMode(query)) {
  ...
  try {
    ...
    const clientResponses = await Promise.all(clientQueries.map((promise) => Promise.resolve(promise).catch(error => ({ data: [], error }))));
    ...
    const responses = await Promise.all(orderQueries.map((promise) => Promise.resolve(promise).catch(error => ({ data: [], error }))));
    rows.incomplete = [...clientResponses, ...responses].some(res => !!res?.error);
...
export async function resolveHomeSearchTarget(result, options = {}) {
  ...
  const selectedCode = safeString(result?.code || result?.clientCode);
  const selectionMatchesQuery = queryBaseCode
    ? /^\d+$/.test(selectedCode) && normalizeNumericCode(selectedCode) === queryBaseCode
    : queryTransportCode
      ? normalizeTransportCodeStrict(selectedCode) === queryTransportCode
      : true;
  if (!selectionMatchesQuery) {
    for (const key of ['id', 'orderId', 'localOid', 'local_oid']) delete authoritativeResult[key];
  }
  ...
  const read = (request) => typeof request?.abortSignal === 'function' ? request.abortSignal(controller.signal) : request;
  const acceptRow = (candidate) => {
    if (!candidate) return null;
    if (queryBaseCode && normalizeNumericCode(candidate.code ?? candidate.client_code) !== queryBaseCode) return null;
    if (queryTransportCode && normalizeTransportCodeStrict(candidate.code_str || `T${candidate.code_n}`) !== queryTransportCode) return null;
    return candidate;
  };
  try {
    return await withHomeSearchDeadline((async () => {
      const { supabase } = await import('@/lib/supabaseClient');
      let row = null;
      ...
      if (kind === 'BASE' && queryBaseCode) {
        const byTypedCode = await read(supabase
          .from('orders')
          .select('*')
          ...
          .limit(1)
          .maybeSingle());
        if (!byTypedCode?.error && byTypedCode?.data) row = acceptRow(byTypedCode.data);
      }

      if (!row && kind === 'BASE' && id && /^\d+$/.test(id)) {
        const byId = await read(supabase.from('orders').select('*').eq('id', Number(id)).limit(1).maybeSingle());
        if (!byId?.error && byId?.data) row = acceptRow(byId.data);
      }

      if (!row && kind === 'TRANSPORT' && looksUuid(id)) {
        const byId = await read(supabase.from('transport_orders').select('*').eq('id', id).limit(1).maybeSingle());
        if (!byId?.error && byId?.data) row = acceptRow(byId.data);
      }

      if (!row && kind === 'BASE' && localOid) {
        const byLocal = await read(supabase.from('orders').select('*').eq('local_oid', localOid).order('updated_at', { ascending: false }).limit(1).maybeSingle());
        if (!byLocal?.error && byLocal?.data) row = acceptRow(byLocal.data);
      }

      if (!row && code) {
        if (kind === 'BASE' && /^\d+$/.test(code)) {
          const byCode = await read(supabase.from('orders').select('*').eq('code', Number(code)).order('updated_at', { ascending: false }).limit(1).maybeSingle());
          if (!byCode?.error && byCode?.data) row = acceptRow(byCode.data);
        } else if (kind === 'TRANSPORT' && strictTransportCode) {
          const codeNumber = Number(strictTransportCode.replace(/\D+/g, ''));
          if (Number.isFinite(codeNumber)) {
            const byCode = await read(supabase.from('transport_orders').select('*').eq('code_n', codeNumber).order('updated_at', { ascending: false }).limit(1).maybeSingle());
            if (!byCode?.error && byCode?.data) row = acceptRow(byCode.data);
          }
        }
      }
...
export async function searchHomeLocalFirst(query) {
  ...
  const sources = await Promise.all([
    ['db-exact', () => getRowsFromDbExact(q, mode), 8000],
    ['db-general', () => getRowsFromDbGeneral(q, mode), 8000],
    ['snapshot', () => getRowsFromSnapshots(), 1500],
    ['indexed-db', () => getRowsFromIndexedDb(), 1500],
    ['base-cache', () => getRowsFromBaseMasterCache(), 1500],
  ].map(async ([name, read, timeout]) => {
    try {
      const rows = await withHomeSearchDeadline(Promise.resolve().then(read), timeout);
      return { name, rows: Array.isArray(rows) ? rows : [], failed: !!rows?.incomplete };
    } catch {
      return { name, rows: [], failed: true };
    }
  }));
  const [dbExactRows, dbGeneralRows, snapshotRows, idbRows, baseMasterRows] = sources.map(source => source.rows);
  ...
  const userResolver = await withHomeSearchDeadline(buildHomeUserResolver(matchedRows), 800).catch(() => readCachedUserResolver());
  ...
  return { results, baseLocalCount, transportLocalCount, incomplete, failedSources };
}
