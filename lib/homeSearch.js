...
function pickTransporter(row, includeTransportIdFallback = false) {
  const data = unwrapData(row);
  return pickFirst(
    ...
    data?.created_by,
    row?.created_by,
    includeTransportIdFallback ? data?.transport_id : ''
  );
}
...
function normalizeResult(row) {
  ...
  const transporter = pickTransporter(row, kind === 'TRANSPORT');
  return {
    kind,
    ...
    transporter,
    broughtBy: kind === 'TRANSPORT' ? (transporter || 'TRANSPORT') : transporter,
