...
//
// V3 permanent-client T-code rule:
...
async function queryCodeExistsInOrders(code) {
  ...
  try {
    ...
    // Never issue a code when Transport history could not be verified.
    return true;
  }
}
...
async function queryCodeExistsInClients(code) {
  ...
  // Query only columns that exist in the production transport_clients schema.
  // The former mixed OR referenced a non-existent `code` column; PostgREST rejected
  // the whole filter and could incorrectly treat a permanent client T-code as free.
  try {
    ...
    // Fail closed: allocator safety is more important than issuing a low code while
    // the client identity table could not be checked.
    return true;
  }
}
...
async function queryCodeExistsInTransportPayments(code) {
  ...
  try {
    ...
    // Compatibility allocator must fail closed when Arka cannot be checked.
    return true;
  }
}
...
async function refreshMirrorFromDb(pin, n = DEFAULT_POOL_SIZE) {
  ...
  // Main path: atomic DB allocator. It returns the smallest globally available code.
  let clean = await reserveViaRpc(owner, safeN);

  // Compatibility fallback for older databases.
  if (!clean.length) {
    clean = await reserveSmallestAvailableTransportCodes(owner, safeN);
  }
...
async function popVerifiedOnlineCode(pin) {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    // A browser mirror is only a cache. The DB must still show that this owner
    // holds the claimed code; released/stolen mirror entries are discarded.
...
export async function getOrReserveTransportCode(reservedBy, opts = {}) {
  ...
  if (oid && isBrowser()) {
    try {
      ...
      if (cached && String(cached).trim()) {
        ...
        // A stale browser cache must never reuse a code that was released or claimed
        // by another draft/device. The DB allocator will provide the next safe code.
  ...
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    ...
    if (!off) throw new Error("S'KA T-KOD TE REZERVUAR OFFLINE. KLIENTI I RI DUHET RUAJTUR ONLINE; KLIENTI EKZISTUES E PERDOR T-KODIN PERMANENT.");
    if (oid && isBrowser()) {
...
export async function releaseTransportCodeReservationForOrder(oid, ownerId = '') {
  const code = getTransportCodeReservationForOrder(oid);
  if (!code || code === 'T0') {
    clearTransportCodeReservationForOrder(oid);
    return false;
  }
  const released = await releaseTransportCodeIfUnused(code, ownerId);
  if (released) clearTransportCodeReservationForOrder(oid);
  return released;
}
