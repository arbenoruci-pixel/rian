...
function isPendingLikeStatus(status = '') {
  return ['pending', 'queued', 'retry', 'retrying', 'processing', 'syncing', 'failed'].includes(String(status || '').trim().toLowerCase());
}
...
export function findMatchingQueuedBaseInsert(snapshot = [], warningIdentity = {}) {
  ...
  for (const item of Array.isArray(snapshot) ? snapshot : []) {
    ...
    if (saveAttemptId && queued.save_attempt_id && queued.save_attempt_id !== saveAttemptId) continue;
    if (code && queued.code && queued.code !== code) continue;
    return queued;
  }
  return null;
}
