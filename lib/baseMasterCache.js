...
let offlineStoreBridgeListenersInstalled = false;
function installOfflineStoreBridgeListeners() {
  if (offlineStoreBridgeListenersInstalled || typeof window === 'undefined') return;
  offlineStoreBridgeListenersInstalled = true;

  try {
    window.addEventListener('tepiha:base-master-cache:patch', (event) => {
      try { patchBaseMasterRow(event?.detail?.row); } catch {}
    }, { passive: true });
  } catch {}

  try {
    window.addEventListener('tepiha:base-master-cache:remove', (event) => {
      try { removeBaseMasterRow(event?.detail?.identity); } catch {}
    }, { passive: true });
  } catch {}
}

try { installOfflineStoreBridgeListeners(); } catch {}
...
export function getBaseRowIdentity(row) {
  ...
  const localOid = String(row?.local_oid || row?.data?.local_oid || row?.data?.oid || '').trim();
  if (id && isPersistedBaseId(id)) return `id:${id}`;
...
export function getBaseRowRank(row) {
  const status = String(getDbTruthStatus(row) || '').trim().toLowerCase();
...
export function normalizeBaseOrderRow(order) {
  ...
  const status = String(getDbTruthStatus({ ...row, data }) || '').trim().toLowerCase();
...
export function getBaseRowsByStatus(status, cache = null) {
  ...
  return (Array.isArray(source?.rows) ? source.rows : []).filter((row) => String(getDbTruthStatus(row) || '').trim().toLowerCase() === wanted);
}
