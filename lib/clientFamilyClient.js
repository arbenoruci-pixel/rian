...
export const FAMILY_INVITATION = 'Në këtë link mund t’i shtoni edhe emrat dhe telefonat e familjarëve që i sjellin tepihat, që t’i lidhim me kartelën tuaj dhe të shmangim ngatërrimet.';
...
export async function prepareFamilySmartMessage(text, options = {}) {
  ...
  const { token } = await familyRequest({ action: 'SIGN_LINK', source, orderId }, options);
  if (!token) return original;
  url.searchParams.set('family', token);
  return `${original.replace(matches[0], url.toString())}\n\n${FAMILY_INVITATION}`;
}
