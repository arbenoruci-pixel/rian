...
export function canTransitionStatus(table, fromStatus, toStatus) {
  ...
  // Terminal visits cannot be reopened by a stale generic status action.
  // Repeat service creates a new visit through the explicit repeat-visit flow.
  const terminal = new Set(['done', 'completed', 'cancelled', 'canceled', 'anuluar']);
  if (terminal.has(from)) return from === to;
