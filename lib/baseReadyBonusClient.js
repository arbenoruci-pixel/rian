...
export const BASE_READY_BONUS_WINDOW_HOURS = 72;
...
const SUMMARY_CACHE_PREFIX = 'tepiha_base_ready_bonus_summary_v2_72h_live:';
const OPPORTUNITY_CACHE_KEY = 'tepiha_base_bonus_opportunities_v2_72h_live';
...
export async function resolveBaseReadyBonusWorker({
  ...
  if (mustPrompt) {
    const entered = window.prompt(
      `${label}\n\nBONUSI: 0.10€ PËR m² NËSE POROSIA U KRYE BRENDA ${BASE_READY_BONUS_WINDOW_HOURS} ORËVE.`,
...
export async function markBaseOrderReadyWithBonus({
  ...
  if (!ref) throw new Error('MUNGON ORDER ID PËR BONUSIN READY.');
  ...
  if (!forceQueue && isOnline()) {
    ...
    try {
      const { data, error } = await Promise.race([
        supabase.rpc('mark_base_order_ready_with_bonus_v1', rpcArgs),
        new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('BASE_READY_NETWORK_TIMEOUT')), 8000); }),
      ]);
      if (error) throw error;
      if (!data?.ok || !data?.order?.id || String(data.order.status).toLowerCase() !== 'gati') throw new Error('BASE_READY_RESPONSE_NOT_VERIFIED');
      return { ...data, offlineQueued: false, idempotencyKey: idem, stageActor };
    } catch (error) {
  ...
  if (!queuedOpId) throw new Error('BASE_READY_LOCAL_QUEUE_FAILED');
  return {
    ok: true,
...
export async function getBaseReadyBonusSummary({
  ...
  if (!actor) throw new Error('MUNGON PIN-I PËR BONUSIN READY.');
...
export async function getBaseBonusOpportunities({ actorPin = '' } = {}) {
  const actor = text(actorPin || getActor()?.pin);
...
// DISPATCH_OWN_HANDOFF_BONUS_BYPASS_V2: non-worker cash collectors receive an empty bonus list.
...
export function describeReadyBonusResult(result = {}) {
  ...
  if (result?.waitingForPayment !== false) {
    return `GATI U RUAJT. BONUSI ${BASE_READY_BONUS_WINDOW_HOURS}H AKTIVIZOHET KUR REGJISTROHET PAGESA QË E MBYLL POROSINË.`;
  }
