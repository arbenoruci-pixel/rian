...
export async function markBaseOrderReadyWithBonus({
  ...
  const resolvedWorker = normalizeWorker(worker || {});
  ...
  if (!resolvedWorker.pin || !isBaseReadyBonusWorkerRole(resolvedWorker.role)) {
    throw new Error('MUNGON PUNËTORI AKTIV QË E PËRFUNDOI POROSINË.');
  }
  if (!slots.length) throw new Error('MUNGON RAFTI / LOKACIONI FINAL.');

  const rpcArgs = {
    p_order_ref: ref,
    p_worker_pin: resolvedWorker.pin,
    p_ready_slots: slots,
  ...
  if (!forceQueue && isOnline()) {
    ...
    if (!error && data?.ok) return { ...data, offlineQueued: false, idempotencyKey: idem };
...
export function describeReadyBonusResult(result = {}) {
  if (result?.offlineQueued) {
    return 'U RUAJT OFFLINE. BONUSI VERIFIKOHET AUTOMATIKISHT KUR VJEN RRJETI.';
  }
  ...
  if (bonus?.eligible) {
    return [
      ...
      `MUAJI ${number(totals?.month_earned, 0).toFixed(2)}€`,
      `PËR ME MBAJT ${number(totals?.available_to_keep, 0).toFixed(2)}€`,
    ].join(' • ');
  }
  return `PA BONUS • ${text(bonus?.reason || 'MBI 48 ORË')}`;
}
