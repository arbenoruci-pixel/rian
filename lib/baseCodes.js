...
export async function takeBaseCode(pin) {
  // --- PRO SAFETY ---
  // iOS Safari vs HomeScreen (PWA) can keep stale localStorage pools.
  // If DB has been factory-reset (server max code is small/0) but local pool is huge,
  // AUTO-HEAL by nuking local pool and refilling from Supabase.
  if (typeof window !== 'undefined' && navigator.onLine) {
    try {
      const cur0 = getBasePool(pin);
      if (cur0.length > 0) {
        const nums = cur0.map((x) => Number(x)).filter((n) => Number.isFinite(n) && n > 0);
        const minLocal = nums.length ? Math.min(...nums) : null;

        // Fetch server max code_n (cached briefly so we don't spam DB)
        const cacheKey = `server_max_code_n_v1`;
        let serverMax = null;
        try {
          const cached = JSON.parse(localStorage.getItem(cacheKey) || 'null');
          if (cached && (Date.now() - Number(cached.ts || 0)) < 30_000) {
            serverMax = Number(cached.v);
          }
        } catch {}
        ...
        if (serverMax == null) {
          const { data, error } = await supabase
            ...
            .limit(1);
          if (!error) {
            serverMax = Number(data?.[0]?.code_n || 0);
            try { localStorage.setItem(cacheKey, JSON.stringify({ v: serverMax, ts: Date.now() })); } catch {}
          }
        }

        // Heuristic: if server is basically fresh but local pool starts at 500+ => stale.
        if (Number.isFinite(serverMax) && serverMax <= 100 && Number.isFinite(minLocal) && minLocal >= 500) {
          // NUKE only code-related keys for this actor
  ...
  // Try local pool first (after auto-heal)
  const cur = getBasePool(pin);
  if (cur.length > 0) {
