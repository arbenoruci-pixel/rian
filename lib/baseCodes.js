...
async function getHighestKnownLocalCode(pin = '') {
  ...
  try {
    ...
    for (const it of Array.isArray(queued) ? queued : []) {
      const e = normalizeUsedEntry(it);
      if (e && e.code > maxCode) maxCode = e.code;
    }
  } catch {}
...
// Normalizo një hyrje të radhës. Mbështet të dy formatet:
//  - i vjetër (bare code): 1234
//  - i ri (metadata):      { code: 1234, localOid, clientPhone, queuedAt }
function normalizeUsedEntry(entry) {
  if (entry && typeof entry === 'object') {
    const code = normalizeCode(entry.code);
    if (code == null) return null;
    const localOid = (entry.localOid != null && String(entry.localOid).trim()) ? String(entry.localOid).trim() : null;
    const clientPhone = (entry.clientPhone != null && String(entry.clientPhone).trim()) ? String(entry.clientPhone).trim() : null;
    const queuedAt = Number.isFinite(Number(entry.queuedAt)) ? Number(entry.queuedAt) : null;
    return { code, localOid, clientPhone, queuedAt };
  }
  const code = normalizeCode(entry);
  if (code == null) return null;
  return { code, localOid: null, clientPhone: null, queuedAt: null };
}

// Ruaj GJITHMONË format me metadata. Dedupe sipas code; bashko metadata nëse ri-radhitet.
function queueUsed(pin, code, meta = {}) {
  const c = normalizeCode(code);
  if (c == null) return;
  const localOid = (meta?.localOid != null && String(meta.localOid).trim()) ? String(meta.localOid).trim() : null;
  const clientPhone = (meta?.clientPhone != null && String(meta.clientPhone).trim()) ? String(meta.clientPhone).trim() : null;

  const raw = lsJsonGet(usedQueueKey(pin), []);
  ...
  for (const it of list) {
    ...
    if (e.code === c) {
      next.push({
        code: c,
        localOid: e.localOid || localOid,
        clientPhone: e.clientPhone || clientPhone,
  ...
  if (!merged) next.push({ code: c, localOid, clientPhone, queuedAt: Date.now() });
  lsJsonSet(usedQueueKey(pin), next);
}

// Kthen hyrje të normalizuara (objekte), duke mbështetur të dy formatet. Dedupe sipas code.
...
// Mark used in DB — VERIFY-GATED (Rregulli i ri: items 2,3,6).
// NUK bën më update direkt `status='used'`. Thërret RPC-në
// mark_base_code_used_after_verify, e cila e refuzon djegien nëse s'ekziston
// një rresht public.orders që përputhet. Nëse RPC mungon ose verify dështon,
// kthen false (kodi mbetet në radhë) — KURRË nuk biem në blind-burn.
async function markUsedInDb(pin, code, opts = {}) {
  const c = normalizeCode(code);
  if (c == null) return false;
  const localOid = String(opts?.localOid || opts?.oid || '').trim() || null;
  const clientPhone = String(opts?.clientPhone || '').trim() || null;

  dbg('markUsedInDb:start', { pin: String(pin || '').trim(), code: c, localOid, hasPhone: !!clientPhone });

  try {
    const { data, error } = await withTimeout(
      supabase.rpc('mark_base_code_used_after_verify', {
        p_code: c,
        p_local_oid: localOid,
        p_client_phone: clientPhone,
      }),
      RPC_TIMEOUT_MS,
      'MARK_USED_RPC_TIMEOUT'
    );

    if (error) {
      // RPC mungon ose gabim DB -> MOS djeg. Mbaje në radhë.
      dbg('markUsedInDb:rpc_error', { codeNum: c, message: error.message, dbcode: error.code || null });
      return false;
    }
    if (!data || data.ok !== true) {
      // p.sh. NO_ORDER / PHONE_MISMATCH -> nuk lejohet djegia tani.
      dbg('markUsedInDb:verify_refused', { codeNum: c, reason: data?.reason || 'UNKNOWN' });
      return false;
    }
    dbg('markUsedInDb:ok', { code: c, burned: data?.burned ?? null, already: !!data?.already, orderId: data?.order_id ?? null });
    return true;
  } catch (e) {
...
export async function ensureUniqueBaseCodeForSave(opts = {}) {
  ...
  const selectedClientId = String(opts?.selectedClientId || opts?.lockedClientId || '').trim();
  const rawSelectedClientCode = String(normalizeCode(opts?.selectedClientCode ?? opts?.lockedClientCode ?? null) || '').trim();
  const selectedClientCode = selectedClientId ? rawSelectedClientCode : '';

  if (rawSelectedClientCode && !selectedClientId) {
    dbg('ensureUniqueBaseCodeForSave:selected_client_code_rejected_id_null', {
      oid: opts?.oid || null,
      selectedClientId: null,
      selectedClientCode: rawSelectedClientCode,
...
export async function markCodeUsed(codeNum, oid, opts = {}) {
  const pin = String(getActorPin() || '').trim();
  const code = normalizeCode(codeNum);
  if (!pin || code == null) return false;

  const localOid = String(oid || opts?.localOid || '').trim() || null;
  const clientPhone = String(opts?.clientPhone || '').trim() || null;

  if (localOid) lsSet(orderCodeKey(localOid), String(code));

  const online = await isOnlineDb();
  if (!online) {
    queueUsed(pin, code, { localOid, clientPhone });
    dbg('markCodeUsed:queued_offline', { code, oid: localOid });
    return true;
  }

  const ok = await markUsedInDb(pin, code, { localOid, clientPhone });
  if (!ok) {
    queueUsed(pin, code, { localOid, clientPhone });
    dbg('markCodeUsed:queued_after_fail', { code, oid: localOid });
  }
...
// READ-only: statusi i një kodi në pool (s'shkruan asgjë). Përdoret vetëm për të vendosur
// drop vs keep për hyrjet legacy pa verify keys. NUK bën update.
async function baseCodePoolStatus(codeNum) {
  const code = normalizeCode(codeNum);
  if (code == null) return { ok: false, status: null, reason: 'NO_CODE' };
  try {
    const { data, error } = await withTimeout(
      supabase.from('base_code_pool').select('status').eq('code', code).limit(1).maybeSingle(),
      2500,
      'BASE_CODE_POOL_STATUS_TIMEOUT'
    );
    if (error) {
      dbg('baseCodePoolStatus:error', { code, message: error.message || String(error), dbcode: error.code || null });
      return { ok: false, status: null, reason: 'DB_ERROR' };
    }
    return { ok: true, status: data ? String(data.status || '') : null };
  } catch (error) {
    dbg('baseCodePoolStatus:throw', { code, error: error?.message || String(error) });
    return { ok: false, status: null, reason: 'THROW' };
  }
}
...
export async function flushBaseUsedQueue(pinArg) {
  ...
  const queued = drainUsedQueue(pin); // hyrje { code, localOid, clientPhone, queuedAt }
  if (!queued.length) {
  ...
  for (const entry of queued) {
    const c = entry.code;
    const hasKeys = !!(entry.localOid || entry.clientPhone);

    const check = await baseOrderExistsForCode(c);
    // s'ka order -> drop, mos e bëj used
    if (check && check.ok === true && check.exists === false) {
      dropped += 1;
      dbg('flushBaseUsedQueue:dropped_no_order', { pin, code: c });
      continue;
    }
    // gabim DB -> mbaje për herë tjetër (ruan metadata)
    ...
    // order ekziston:
    if (hasKeys) {
      // verify i plotë me metadata përmes RPC. Asnjë update direkt.
      const ok = await markUsedInDb(pin, c, { localOid: entry.localOid, clientPhone: entry.clientPhone });
      if (ok) flushed += 1; else still.push(entry);
      continue;
    }

    // legacy pa verify keys: MOS thirr RPC me {} (do kthente MISSING_VERIFY_KEYS).
    const cur = await baseCodePoolStatus(c);
    if (cur && cur.ok === true && cur.status === 'used') {
      // a) tashmë used -> drop nga radha
      dropped += 1;
      dbg('flushBaseUsedQueue:dropped_legacy_already_used', { pin, code: c });
    } else {
      // b) jo used -> mbaje + REVIEW, mos e bë used pa verify
      still.push(entry);
      review += 1;
      dbg('flushBaseUsedQueue:review_missing_verify_keys', { pin, code: c, poolStatus: cur?.status || null });
    }
  }

  if (still.length) lsJsonSet(usedQueueKey(pin), still);
  dbg('flushBaseUsedQueue:done', { pin, flushed, dropped, review, remaining: still.length });

  return { ok: still.length === 0, flushed, dropped, review, remaining: still.length };
}
...
async function baseOrderExistsForCode(codeNum) {
  const code = normalizeCode(codeNum);
  if (code == null) return { ok: false, exists: false, reason: 'NO_CODE' };
  try {
    const { data, error } = await withTimeout(
      supabase
        .from('orders')
        ...
        .limit(1),
      2500,
      'BASE_CODE_ORDER_CHECK_TIMEOUT'
    );
    if (error) {
      dbg('baseOrderExistsForCode:error', { code, message: error.message || String(error), dbcode: error.code || null });
      return { ok: false, exists: false, reason: 'DB_ERROR', error };
    }
    return { ok: true, exists: Array.isArray(data) && data.length > 0 };
  } catch (error) {
    dbg('baseOrderExistsForCode:throw', { code, error: error?.message || String(error) });
    return { ok: false, exists: false, reason: 'THROW', error };
  }
}
...
export async function holdBaseCodeForDraft(codeNum, oid, opts = {}) {
  const pin = String(getActorPin() || '').trim();
  const code = normalizeCode(codeNum);
  const localOid = String(oid || opts?.oid || opts?.local_oid || '').trim();
  if (code == null) return false;
  if (localOid) lsSet(orderCodeKey(localOid), String(code));
  if (!pin) return false;
  ...
  const online = await isOnlineDb();
  if (!online) {
    dbg('holdBaseCodeForDraft:offline_local_only', { pin, code, oid: localOid || null, reason: opts?.reason || null });
    return false;
  }

  const existingOrder = await baseOrderExistsForCode(code);
  if (existingOrder?.exists) {
    dbg('holdBaseCodeForDraft:skip_order_exists', { pin, code, oid: localOid || null });
    return false;
  }
  if (existingOrder && existingOrder.ok === false) {
    dbg('holdBaseCodeForDraft:skip_order_check_failed', { pin, code, oid: localOid || null, reason: existingOrder.reason || null });
    return false;
  }

  try {
    const minutes = Number.isFinite(Number(opts?.minutes)) ? Number(opts.minutes) : (60 * 24 * 365 * 10);
    const lease = new Date(Date.now() + Math.max(1, minutes) * 60 * 1000).toISOString();
    const patch = {
      status: 'reserved',
      reserved_by: pin,
    ...
    const safePin = sanitizePostgrestFilterValue(pin);
    const filter = safePin
      ? `reserved_by.eq.${safePin},reserved_by.is.null`
      : 'reserved_by.is.null';
    ...
    const { error } = await withTimeout(
      supabase
        ...
        .or(filter),
      2500,
      'BASE_CODE_HOLD_TIMEOUT'
    );

    if (error) {
      dbg('holdBaseCodeForDraft:error', { pin, code, oid: localOid || null, message: error.message || String(error), dbcode: error.code || null });
      return false;
    }
    dbg('holdBaseCodeForDraft:ok', { pin, code, oid: localOid || null, reason: opts?.reason || null });
    return true;
  } catch (error) {
    dbg('holdBaseCodeForDraft:throw', { pin, code, oid: localOid || null, error: error?.message || String(error) });
    return false;
  }
}

// Legacy name kept for compatibility with existing Pranimi flows.
// New PRANIMI draft rule: release only a draft reservation owned by this worker and never release a code once an order exists.
export async function releaseLocksForCode(codeNum, opts = {}) {
  const pin = String(getActorPin() || '').trim();
  const localOid = String((opts && typeof opts === 'object' ? (opts.oid || opts.local_oid || opts.id) : '') || '').trim();
  const code = normalizeCode(codeNum ?? (localOid ? lsGet(orderCodeKey(localOid), null) : null));
  if (code == null) return true;
  if (!pin) return false;

  const online = await isOnlineDb();
  if (!online) {
    dbg('releaseLocksForCode:offline_skip_db_release', { pin, code, oid: localOid || null, reason: opts?.reason || null });
    return false;
  }

  const existingOrder = await baseOrderExistsForCode(code);
  if (existingOrder?.exists) {
    if (localOid) clearOrderCodeCache(localOid);
    dbg('releaseLocksForCode:skip_order_exists', { pin, code, oid: localOid || null });
    return true;
  }
  if (existingOrder && existingOrder.ok === false) {
    dbg('releaseLocksForCode:skip_order_check_failed', { pin, code, oid: localOid || null, reason: existingOrder.reason || null });
    return false;
  }

  try {
    const safePin = sanitizePostgrestFilterValue(pin);
    const filter = safePin ? `reserved_by.eq.${safePin},reserved_by.is.null` : 'reserved_by.is.null';
    const { error } = await withTimeout(
      supabase
        .from('base_code_pool')
        .update({
        ...
        .or(filter),
      2500,
      'BASE_CODE_RELEASE_TIMEOUT'
    );

    if (error) {
      dbg('releaseLocksForCode:error', { pin, code, oid: localOid || null, message: error.message || String(error), dbcode: error.code || null });
      return false;
    }
    if (localOid) clearOrderCodeCache(localOid);
    dbg('releaseLocksForCode:ok', { pin, code, oid: localOid || null, reason: opts?.reason || null, poolNow: getPool(pin).length });
    return true;
  } catch (error) {
    dbg('releaseLocksForCode:throw', { pin, code, oid: localOid || null, error: error?.message || String(error) });
    return false;
  }
}
