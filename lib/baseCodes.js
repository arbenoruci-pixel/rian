...
export async function takeBaseCode(pin) {
  ...
  // Online fallback: refill a batch, then take one (keeps pool from ever staying at 0 while online)
  try {
    await refillBasePool(pin, 50);
    const after = readArr(pKey);
    if (after.length) {
      const code = after.shift();
      writeArr(pKey, after);
      return Number(code);
    }
  } catch {
    // ignore and try single
  }

  // Online final fallback: reserve a single code
...
export async function ensureBasePool(pin, minLeft = 5, refillCount = 50) {
  try {
