...
export async function fetchClientProfile(anchorLike, { signal, timeoutMs = 9500 } = {}) {
  ...
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    if (fallback) return { profile: fallback, fromCache: true };
  ...
  try {
    ...
    if (fallback) return { profile: fallback, fromCache: true, networkError: String(error?.code || error?.message || error) };
    throw error;
  } finally {
