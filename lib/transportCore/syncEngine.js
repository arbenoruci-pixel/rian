...
import { extractOfflineCodeLease, markOfflineCodeLeaseFinishedLocal } from '@/lib/offlineCodeBank';
import {
  classifyTransportSyncError,
  isTransportSyncNetworkError,
  makeTransportSyncError,
  selectTransportOrderPatchSource,
} from '@/lib/transportCore/syncPolicy';
...
async function saveLocalTransportOrder(row, syncState = 'synced') {
  ...
  if (!String(next?.client_tcode || '').trim()) {
    const clientCode = normalizeTCodeLoose(next?.code_str || nextData?.code_str || nextData?.order_code || nextData?.order_tcode || nextData?.official_order_code || '');
    if (clientCode) next.client_tcode = clientCode;
  }
  if (!String(next?.code_str || '').trim()) {
    const orderCode = normalizeTCodeLoose(nextData?.code_str || nextData?.order_code || nextData?.order_tcode || nextData?.official_order_code || '');
...
function sanitizeTransportOrderSyncPayload(input, opts = {}) {
  return sanitizeTransportOrderPayload(input, opts);
}
...
function wrapTransportResponseError(error, status = 0) {
  if (!error) return null;
  const wrapped = makeTransportSyncError(error?.message || error, {
    code: error?.code || '',
...
async function processTransportOrderInsert(op = {}) {
  ...
  if (!row?.id) {
    throw makeTransportSyncError('MISSING_TRANSPORT_ID', {
      code: 'MISSING_TRANSPORT_ID',
      status: 400,
    });
  }

  // Offline drafts are finalized through the same atomic RPC as live Self Entry.
  // The final phone lookup happens now, while online. If the phone already exists,
  // the RPC reuses its permanent T-code and the temporary offline reservation is released.
  const result = await insertTransportOrder({
    ...row,
    code_owner: raw?.code_owner || raw?.reserved_by || raw?.data?.created_by_pin || raw?.data?.transport_pin || '',
  });
  if (!result?.ok) {
    const error = new Error(result?.error || 'TRANSPORT_OFFLINE_ORDER_SYNC_FAILED');
    error.code = result?.code || '';
    error.httpStatus = Number(result?.httpStatus || 0);
    if (error.httpStatus > 0) error.status = error.httpStatus;
    throw error;
  }
  const offlineLease = extractOfflineCodeLease(raw);
  if (offlineLease) {
    const finalized = result?.offlineLeaseResult;
    if (finalized?.finalized !== true
      || String(finalized?.lease_token || '') !== String(offlineLease.lease_token || '')
      || String(finalized?.draft_session_id || '') !== String(offlineLease.draft_session_id || '')) {
      throw makeTransportSyncError('TRANSPORT_OFFLINE_LEASE_FINALIZATION_NOT_VERIFIED', {
        code: 'TRANSPORT_OFFLINE_LEASE_FINALIZATION_NOT_VERIFIED',
        status: 409,
      });
    }
    await markOfflineCodeLeaseFinishedLocal(offlineLease, finalized);
  }
  await saveLocalTransportOrder(result.data || row, 'synced');
}
...
async function processTransportOrderPatch(op = {}) {
  ...
  const patchSource = selectTransportOrderPatchSource(payload);
  ...
  const { data, error, status } = await supabase
    ...
    .select('*')
    .maybeSingle();
  if (error) throw wrapTransportResponseError(error, status);
  if (!data) {
  ...
  await saveLocalTransportOrder(data, 'synced');
  return data;
}
...
async function processTransportClientPatch(op = {}) {
  ...
  if (!tcode) {
    throw makeTransportSyncError('MISSING_TRANSPORT_CLIENT_TCODE', {
      code: 'MISSING_TRANSPORT_CLIENT_TCODE',
      status: 400,
    });
  }
  ...
  if (error) throw wrapTransportResponseError(error, status);
  if (!data) {
    throw makeTransportSyncError(`Transport client ${tcode} nuk u gjet për PATCH`, {
...
async function processTransportOp(op = {}) {
  ...
  throw makeTransportSyncError(`Operacion Transport i panjohur: ${type || 'missing'}`, {
    code: 'UNKNOWN_TRANSPORT_SYNC_OP',
    status: 400,
  });
}
    ...
    try {
      ...
      for (const op of ops) {
        ...
        try {
          ...
          const networkLike = isTransportSyncNetworkError(error) || !isOnline();
          const permanent = classifyTransportSyncError(error);
