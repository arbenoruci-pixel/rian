...
function getExactOrderId(order = {}) {
  // Structured order UUID fields outrank a generic wrapper `id`. This prevents a
  // modal/list item ID from shadowing the real transport_orders UUID in fullOrder/data.
  const candidates = [
    (ctx) => ctx?.order_id,
    (ctx) => ctx?.public_order_id,
    (ctx) => ctx?.publicOrderId,
    (ctx) => ctx?.transport_order_id,
    (ctx) => ctx?.db_id,
    (ctx) => ctx?.id,
  ];
...
Faleminderit, KOMPANIA JONI ✨`;
  ...
  if (action === 'transport_pranimi') {
    return [
      ...
      `Porosia juaj u pranua me sukses për larjen e tepihave.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI: ${total.toFixed(2)} €`,
      ``,
      `Kur porosia të jetë gati, do t'ju njoftojmë për konfirmim para dorëzimit.`,
  ...
  if (action === 'transport_followup_no_answer') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat tuaj janë gati për dorëzim, por nuk po arrijmë t'ju konfirmojmë me telefon.`,
      `Ju lutem na ktheni përgjigje sa më shpejt që të caktojmë dorëzimin.`,
      `Pa konfirmim, porosia nuk ngarkohet në furgon.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI PËR PAGESË: ${total.toFixed(2)} €`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_not_accepting') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat tuaj janë gati për dorëzim, por porosia ende nuk është pranuar/konfirmuar për t'u sjellë.`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_confirm_address') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat tuaj janë gati për dorëzim. Na duhet konfirmimi i adresës/orarit para se të niset shoferi.`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_general') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat tuaj janë gati për dorëzim dhe kemi nevojë për konfirmim që ta organizojmë nisjen.`,
