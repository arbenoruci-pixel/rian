...
export function canonicalizePhone(phone, defaultCountryCode = '383') {
  ...
  const countryCode = String(defaultCountryCode || '383').replace(/\D+/g, '') || '383';

  // Kosovo numbers are often entered/stored as 383044..., +383044..., or 044...
  // Messaging apps expect +38344... without the extra local leading zero.
  if (digits.startsWith(countryCode)) {
    const local = digits.slice(countryCode.length).replace(/^0+/, '');
  ...
  // Local Kosovo mobile numbers are commonly saved as 44..., 45..., etc.
  // Keep longer already-international numbers untouched when they do not look local.
  if (countryCode === '383' && digits.length >= 8 && digits.length <= 9) {
...
function isTransportLikeOrder(order = {}) {
  const explicitSource = pickFirstString(order, [
    (ctx) => ctx?._table,
    (ctx) => ctx?.table,
    (ctx) => ctx?.order_table,
    (ctx) => ctx?.source_module,
    (ctx) => ctx?.order_origin,
    (ctx) => ctx?.created_by,
  ], '').toUpperCase();
  if (explicitSource.includes('TRANSPORT')) return true;

  const candidates = [
    (ctx) => ctx?.client_tcode,
    (ctx) => ctx?.transport_client_tcode,
    (ctx) => ctx?.code_str,
    (ctx) => ctx?.t_code,
    (ctx) => ctx?.client?.transport_client_tcode,
    (ctx) => ctx?.client?.tcode,
  ];
  return candidates.some((getter) => /^T\d+$/i.test(pickFirstString(order, [getter], '')));
}

function getTransportLifecycle(order = {}) {
  return pickFirstString(order, [
    (ctx) => ctx?.tcode_lifecycle,
    (ctx) => ctx?.transport_tcode_lifecycle,
  ], '').toUpperCase();
}

function getExactOrderId(order = {}) {
  const top = isPlainObject(order) ? order : {};
  const fullOrder = isPlainObject(top?.fullOrder) ? top.fullOrder : {};
  const data = isPlainObject(top?.data) ? top.data : {};
  const fullData = isPlainObject(fullOrder?.data) ? fullOrder.data : {};

  // Structured DB order IDs always outrank display codes and wrapper/list IDs.
  const structuredIds = [
    top?.order_id,
    fullOrder?.order_id,
    data?.order_id,
    fullData?.order_id,
    top?.public_order_id,
    fullOrder?.public_order_id,
    data?.public_order_id,
    fullData?.public_order_id,
    top?.publicOrderId,
    fullOrder?.publicOrderId,
    data?.publicOrderId,
    fullData?.publicOrderId,
    top?.transport_order_id,
    fullOrder?.transport_order_id,
    data?.transport_order_id,
    fullData?.transport_order_id,
    top?.db_id,
    fullOrder?.db_id,
    data?.db_id,
    fullData?.db_id,
  ...
  for (const value of structuredIds) {
    const raw = String(value ?? '').trim();
    if (/^\d+$/.test(raw) || isUuidLike(raw)) return raw;
  }

  // A full DB row is more authoritative than a modal/list wrapper.
  const genericIds = [fullOrder?.id, top?.id, fullData?.id, data?.id];
  for (const value of genericIds) {
    const raw = String(value ?? '').trim();
    if (/^\d+$/.test(raw) || isUuidLike(raw)) return raw;
  }
...
function getExactBaseOrderId(order = {}) {
  ...
  // Base IDs are numeric. Ignore local UUIDs and prefer structured persisted IDs;
  // this also handles wrappers whose fullOrder.id still contains a local UUID.
  const candidates = [
    top?.order_id,
    fullOrder?.order_id,
    data?.order_id,
...
export function buildOrderTrackUrl(order = {}) {
  if (isTransportLikeOrder(order)) {
    const exactId = getExactOrderId(order);
    if (isUuidLike(exactId)) {
      return `${BASE_CONFIRM_URL}${encodeURIComponent(exactId)}?src=transport`;
    }
    // New permanent-T-code rows must never fall back to a shared client code: that
    // could open another visit. A generic safe URL is preferable to a wrong order.
    if (getTransportLifecycle(order).includes('PERMANENT_CLIENT_TCODE')) return BASE_CONFIRM_URL;

    // Legacy fallback only for messages created before UUID-based Smart SMS.
  ...
  // Base Smart SMS must always open the exact public.orders row. The customer code
  // belongs to the client and can be reused on later visits, so it is legacy-only.
  ...
  // A Base client code is reusable across visits. If the persisted orders.id is
  // missing, fail safe instead of linking a customer to another visit/order.
  // Already-sent legacy /k/<code>?src=base links remain supported by the route.
...
export function buildSmartSmsText(order = {}, actionType = '') {
  ...
  if (action === 'pranimi_baze') {
    ...
    const liveLine = liveUrl && liveUrl !== BASE_CONFIRM_URL
      ? `\n\n📍 Ndiqni statusin LIVE: ${liveUrl}`
      : '';
    return `Përshëndetje ${clientName !== 'klient' ? clientName : 'klient'},
Tepihat e juaj janë në duar të sigurta! U pranuan dhe procesi i pastrimit profesional ka filluar. 🫧
KODI: ${code || '—'}
SASIA: ${pieces || 0} copë (${m2.toFixed(2)} m²)
TOTALI / BORXHI: ${total.toFixed(2)} €${liveLine}
...
Faleminderit, KOMPANIA JONI ✨`;
  ...
  if (action === 'gati_baze') {
    return appendFooter(
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''}, Tepihat e juaj janë GATI (${pieces} copë). Totali: ${total.toFixed(2)}€. Për shkak të hapësirës së kufizuar, nëse nuk i merrni brenda 24 orëve, kompania nuk mban përgjegjësi në rast ngatërrimi apo humbje.${trackText}`
    );
  }

  if (action === 'transport_pranimi') {
    const trackingLines = trackUrl && trackUrl !== BASE_CONFIRM_URL
      ? [
          ``,
          `Për të ndjekur statusin live, preke këtë link:`,
          `${trackUrl}`,
        ]
      : [];
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj u pranuan me sukses për larje.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI: ${total.toFixed(2)} €`,
      ``,
      `Kur tepihat të jenë gati, do t'ju njoftojmë për konfirmim para dorëzimit.`,
      ...trackingLines,
      ``,
      INFO_FOOTER,
    ].join('\n');
  ...
  if (action === 'transport_pikap_konfirmim') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat u morën nga shoferi i kompanisë JONI dhe po dërgohen në bazë për pastrim.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      ``,
  ...
  if (action === 'transport_konfirmim') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj janë gati dhe brenda 1 ore nisen drejt jush.`,
      ``,
      `⚠️ TË LUTEM KONFIRMO:`,
      `Na kthe përgjigje për të konfirmuar që je në shtëpi. Nëse nuk e konfirmon, tepihat NUK ngarkohen në furgon!`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      ...
      `📍 Ndiqni statusin e tepihave live:\n${trackUrl}`,
      ``,
      INFO_FOOTER,
    ].join('\n');
  }

  if (action === 'transport_followup_no_answer') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj janë gati për dorëzim, por nuk po arrijmë t'ju konfirmojmë me telefon.`,
      `Ju lutem na ktheni përgjigje sa më shpejt që të caktojmë dorëzimin.`,
      `Pa konfirmim, tepihat nuk ngarkohen në furgon.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI PËR PAGESË: ${total.toFixed(2)} €`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_not_accepting') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj janë gati për dorëzim, por ende nuk janë pranuar/konfirmuar për t'u sjellë.`,
      `Ju lutem konfirmoni kur mund t'i pranoni. Nëse nuk i pranoni sot, e riplanifikojmë dorëzimin sipas disponueshmërisë.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI PËR PAGESË: ${total.toFixed(2)} €`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_confirm_address') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj janë gati për dorëzim. Na duhet konfirmimi i adresës/orarit para se të niset shoferi.`,
      `Ju lutem dërgoni adresën e saktë, pikën orientuese dhe orarin kur jeni në shtëpi.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI PËR PAGESË: ${total.toFixed(2)} €`,
...
${trackUrl}`,
  ...
  if (action === 'transport_followup_general') {
    return [
      `Përshëndetje ${clientName !== 'klient' ? clientName : ''},`,
      `Tepihat e juaj janë gati për dorëzim dhe kemi nevojë për konfirmim që ta organizojmë nisjen.`,
      `Ju lutem na ktheni përgjigje me orarin kur mund t'i pranoni.`,
      ``,
      `KODI: ${code || '—'}`,
      `COPË: ${pieces || 0}`,
      `TOTALI PËR PAGESË: ${total.toFixed(2)} €`,
...
${trackUrl}`,
  ...
  if (action === 'transport_dorzim') {
    return appendFooter(`Përshëndetje ${clientName !== 'klient' ? clientName : ''}, Tepihat e juaj janë pastruar dhe jemi rrugës për t'i sjellë.${trackText}`);
  }
