...
export function buildSmartSmsText(order = {}, actionType = '') {
  ...
  const compactDetails = `Kodi ${code || '—'} · ${pieces || 0} copë · Totali ${total.toFixed(2)}€`;
  const compactLink = trackUrl && trackUrl !== BASE_CONFIRM_URL ? `Statusi:\n${trackUrl}` : '';
  const compactFooter = 'JONI · +38344735312';
  const compact = (statusLine, instruction) => [
    `${clientName !== 'klient' ? clientName + ', t' : 'T'}epihat e juaj ${statusLine}`,
    compactDetails,
    instruction,
    compactLink,
    compactFooter,
  ...
  if (action === 'pranimi_baze') {
    return compact('u pranuan për pastrim.', 'Ju lajmërojmë kur të jenë gati.');
  }

  if (action === 'gati_baze') {
    return compact('janë GATI.', 'Ju lutemi merrini brenda 24 orëve.');
  }

  if (action === 'transport_pranimi') {
    return compact('u pranuan për pastrim.', 'Ju lajmërojmë para dorëzimit.');
  }
  ...
  if (action === 'transport_pikap_konfirmim') {
    return compact('u morën nga shoferi për pastrim.', 'Ju lajmërojmë para dorëzimit.');
  }

  if (action === 'transport_konfirmim') {
    return compact('janë GATI për dorëzim.', 'Na konfirmoni me përgjigje kur jeni në shtëpi.');
  }
