...
export function getTransportContext() {
  ...
  if (dedicated?.transport_id) {
    const dedicatedRole = normalizeRole(dedicated.role || 'TRANSPORT') || 'TRANSPORT'
    const effectiveRole = dedicatedRole === 'PUNTOR' && dedicated.is_hybrid_transport === true
      ? 'TRANSPORT'
      : dedicatedRole
    return {
      role: effectiveRole,
      ...
      is_hybrid_transport: dedicated.is_hybrid_transport === true,
