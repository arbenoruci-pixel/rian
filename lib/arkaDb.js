...
export async function dbHasPendingHanded(dayKey = null) {
  // IMPORTANT:
  // - If dayKey is provided -> check only that specific day.
  // - If dayKey is null/undefined -> check ALL days.
  const q = supabase
    .from("arka_cycles")
    .select("id")
  ...
  const { data, error } = dayKey ? await q.eq("day_key", String(dayKey)) : await q;
...
export async function dbListPendingHanded(dayKey = null) {
  // Same rule as dbHasPendingHanded:
  // - dayKey provided -> list for that day
  // - otherwise -> list ALL pending HANDED cycles
  const q = supabase
    .from("arka_cycles")
    .select("*")
    .eq("handoff_status", "HANDED")
...
export async function dbOpenCycle({
  ...
  // HARD LOCK: do not allow opening a new cycle if ANY HANDED cash is not yet RECEIVED.
  const hasPending = await dbHasPendingHanded();
  if (hasPending) {
    throw new Error(
