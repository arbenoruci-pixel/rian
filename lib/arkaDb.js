  ...
  try {
    ...
    if (Number.isFinite(amt) && amt > 0) {
      await budgetAddOutMove({
        type: 'IN',
        amount: amt,
        note: `ARKA RECEIVED • ${String(data?.day_key || '')} • CIKLI ${String(data?.cycle_no || '')}`,
...
/* =========================
   MOVES
========================= */
export async function dbAddCycleMove({ cycle_id, type, amount, note, source, created_by, external_id }) {
  if (!cycle_id) throw new Error("cycle_id mungon");
  ...
  // If the schema doesn't have external_id, retry without it.
  if (external_id) {
...
// List cycles for a specific day (by day_id) — used by HISTORI UI
export async function dbListCyclesByDay(day_id) {
  if (!day_id) return [];

  const { data, error } = await supabase
    .from("arka_cycles")
    .select("id, day_id, day_key, cycle_no, status, opened_at, closed_at, expected_cash, cash_counted, discrepancy, handoff_status")
    .eq("day_id", day_id)
