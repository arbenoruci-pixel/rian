...
async function ensureTodayDayRow() {
  ...
  // INSERT minimal (mos u var në kolona ekstra)
...
export async function dbGetCarryoverToday() {
  ...
  if (!day?.id) return { carry_cash: 0, carry_source: null, carry_person_pin: null };
  return {
    carry_cash: Number(day.carry_cash || 0),
    carry_source: day.carry_source || null,
    carry_person_pin: day.carry_person_pin || null,
  };
}

export async function dbSetCarryoverToday({
  carry_cash = 0,
  carry_source = null,
  ...
  const payload = {
    carry_cash: Number(carry_cash || 0),
    carry_source: carry_source ? String(carry_source) : null,
    carry_person_pin: carry_person_pin ? String(carry_person_pin) : null,
    carry_updated_at: new Date().toISOString(),
  };
...
export async function dbGetActiveCycle() {
  // IMPORTANT:
  // The DB enforces ONLY ONE OPEN cycle globally (see unique constraint/index).
  // If an OPEN cycle exists from a previous day, we must still detect it,
  // otherwise the UI will try to open a new one and hit a duplicate constraint.

  // 1) Prefer today's OPEN cycle (fast path)
  const day = await ensureTodayDayRow();
  if (day?.id) {
    const qToday = await supabase
      .from("arka_cycles")
      .select("*")
      .eq("day_id", day.id)
    ...
    if (qToday.error) throw qToday.error;
    if (qToday.data?.id) return qToday.data;
  }

  // 2) Fallback: any OPEN cycle globally
  const { data, error } = await supabase
    .from("arka_cycles")
    .select("*")
    .eq("handoff_status", "OPEN")
...
export async function dbOpenCycle({
  ...
  // Guard: if there is already an OPEN cycle (even from a previous day),
  // return it instead of trying to create a new one (which would violate the
  // "only one open cycle" constraint).
  ...
  try {
    await dbSetCarryoverToday({ carry_cash: 0, carry_source: null, carry_person_pin: null });
  } catch {}
...
export async function dbCloseCycle({
  ...
  try {
    await dbSetCarryoverToday({
      carry_cash: keep,
      carry_source: keep_source ? String(keep_source) : null,
      carry_person_pin: keep_person_pin ? String(keep_person_pin) : null,
    });
  } catch {}
...
/* =========================
   MOVES
========================= */
export async function dbAddCycleMove({ cycle_id, type, amount, note, source, created_by }) {
  if (!cycle_id) throw new Error("cycle_id mungon");
...
export async function dbListHistoryDays(days = 14) {
  ...
  const q1 = await supabase.from("arka_days").select("*").order("day_date", { ascending: false }).limit(n);
  if (!q1.error) return q1.data || [];

  const q2 = await supabase.from("arka_days").select("*").order("day_key", { ascending: false }).limit(n);
  if (q2.error) throw q2.error;
  return q2.data || [];
}
...
export async function dbGetHistoryDay(day_key) {
  ...
  const cycQ = await supabase.from("arka_cycles").select("*").eq("day_key", day_key).order("cycle_no", { ascending: true });
  if (cycQ.error) throw cycQ.error;

  const cycles = cycQ.data || [];

  const cyclesWithTotals = await Promise.all(
    cycles.map(async (c) => {
      const mv = await dbListCycleMoves(c.id);

      const ins = (mv || []).filter((m) => String(m.type || "").toUpperCase() === "IN")
        .reduce((a, m) => a + Number(m.amount || 0), 0);

      const outs = (mv || []).filter((m) => String(m.type || "").toUpperCase() === "OUT")
        .reduce((a, m) => a + Number(m.amount || 0), 0);
