...
export async function dbCloseCycle(params = {}) {
  const cycle_id = params.cycle_id ? String(params.cycle_id) : null;
...
export async function dbAddCycleMove(params = {}) {
  // IMPORTANT: day_id is UUID string (never Number/parseInt)
  let day_id = params.cycle_id || params.day_id;
  if (day_id) day_id = String(day_id);

  // If missing, fetch active OPEN cycle
  if (!day_id) {
...
export async function dbListCycleMoves(cycle_id, limit = 200) {
  const day_id = cycle_id ? String(cycle_id) : null;
  if (!day_id) return [];

  const { data, error } = await supabase
    .from("arka_moves")
    .select("*")
    .eq("day_id", day_id) // UUID string
