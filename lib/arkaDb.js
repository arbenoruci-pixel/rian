...
export async function dbGetCycleMoves(cycleId) {
  const { data, error } = await supabase
    ...
    .select('id, cycle_id, type, amount, note, source, created_by, at, external_id, created_at, created_by_name, created_by_pin, created_by_role')
    .eq('cycle_id', cycleId)
  if (error) return [];
...
export async function dbReceiveCycle(payload = {}) {
  ...
  // LEGACY BUDGET WRITE REMOVED ON PURPOSE.
  // Pranimi i ciklit nuk duhet të shkruajë më në company_budget_moves.
  // Motori i ri corporateFinance (cash_handoffs + company_budget_ledger)
  // është i vetmi burim i së vërtetës për buxhetin e kompanisë.
  return { ok: true };
}
