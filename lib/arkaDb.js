...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name, approved_by_pin }){
  ...
  try {
    // FIX: Sigurohemi qe te dhenat kalojne sakt sipas skemes se DB
    await dbAddCycleMove({
      cycle_id,
      ...
      created_by_pin: approved_by_pin || pending.created_by_pin || null,
      external_id: eid 
    });
...
// SHTUAR EXPORT: Qe te jete i qashem ne frontend
export async function rejectPendingPayment({ pending, rejected_by_name, rejected_by_role, reject_note }) {
  try {
    const eid = pending?.external_id || pending?.externalId;
    if (eid) {
      await supabase.from('arka_pending_payments')
        .update({
          status: 'REJECTED',
