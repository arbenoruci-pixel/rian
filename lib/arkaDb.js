 ...
 *
 * Adds:
 * - EXPENSES: arka_expenses table
 *   dbAddExpense, dbListExpensesToday
 */
...
export async function dbOpenCycle(params = {}) {
  ...
  // NOTE: rpc must exist in Supabase
...
export async function dbCloseCycle(params = {}) {
  // IMPORTANT: arka_days.id is UUID -> keep as string
  const cycle_id = String(params.cycle_id || "");
...
export async function dbAddCycleMove(params = {}) {
  // IMPORTANT: arka_days.id is UUID -> keep as string
  let day_id = params.cycle_id || params.day_id;

  // FIX: if empty, fetch active day (OPEN cycle)
  if (!day_id) {
...
export async function dbAddExpense(params = {}) {
  ...
  const payload = {
    amount,
    category: params.category || "TË TJERA",
    note: params.note || null,
    paid_from: params.paid_from || "ARKA", // ARKA | COMPANY
    personal_pin: params.personal_pin || null,
...
export async function dbListExpensesToday() {
  // Simple list (you can filter by date later if you want)
  const { data, error } = await supabase
    .from("arka_expenses")
    .select("*")
    .order("created_at", { ascending: false })
