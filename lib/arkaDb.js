...
export async function dbOpenCycle(payload = {}) {
  ...
  const insertRow = { day_key, cycle_no, handoff_status: 'OPEN', opening_cash, opening_source: payload.opening_source ?? 'COMPANY', opened_at: new Date().toISOString(), opened_by: payload.opened_by || null, opened_by_pin: payload.opened_by_pin || null };
...
export async function dbAddCycleMove(payload) {
  const { cycle_id, type, amount, note, external_id, created_by_pin } = payload || {};
  ...
  if (error) {
    ...
    // Fallback nese kolona external_id nuk ekziston ne DB tende
...
export async function dbCloseCycle(payload = {}) {
  ...
  const upd = { handoff_status: 'CLOSED', status: 'CLOSED', expected_cash: expected, cash_counted: counted, discrepancy: counted - expected, end_cash: counted, closed_at: new Date().toISOString(), closed_by_pin: payload.closed_by_pin || null };
...
export async function dbReceiveCycle(payload = {}) {
  const upd = { status: 'RECEIVED', handoff_status: 'RECEIVED', received_at: new Date().toISOString(), received_by: payload.received_by || null, received_by_pin: payload.received_by_pin || null };
