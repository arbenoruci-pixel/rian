...
export async function dbOpenCycle(params = {}) {
  ...
  const opening_cash = Number(params.opening_cash ?? params.opening_cash_amount ?? params.initial_cash ?? 0);
...
// Ensure we have a day_key available (used by expenses).
...
export async function dbAddExpense(params = {}) {
  ...
  const amount = Number(params.amount ?? 0);
  const paid_from = (params.paid_from || "ARKA").toUpperCase();
  ...
  // If PERSONAL is selected, we require PIN (same rule as cash source).
...
export async function dbListExpensesForDay(day_key, limit = 200) {
  ...
  // We'll try ordering by 'at' first; if it fails, fallback to 'created_at'.
...
export async function dbAddCompanyMove(params = {}) {
  const amount = Number(params.amount ?? 0);
  const type = (params.type || "OUT").toUpperCase(); // IN | OUT
  const note = params.note || "";
  const created_by = params.created_by || "LOCAL";
...
export async function dbListCompanyMoves(limit = 200) {
  // Support both 'at' and 'created_at'
  const q1 = await supabase
    .from("arka_company_moves")
    .select("*")
    .order("at", { ascending: false })
