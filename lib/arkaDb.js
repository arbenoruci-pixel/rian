...
export async function dbOpenDay({ initial_cash, opened_by, day_key }) {
  // IMPORTANT:
  // - Must NOT always INSERT because day_key is UNIQUE in DB.
  // - Use RPC to implement OPEN / NOOP / REOPEN safely.
  const key = day_key || new Date().toISOString().slice(0, 10); // YYYY-MM-DD

  // Preferred path: RPC (Supabase function)
  try {
  ...
  // Fallback (legacy DB): attempt insert, will fail if unique exists
  const { data, error } = await supabase
    .from("arka_days")
...
export async function dbCloseDay({ day_id, closed_by }) {
  // Preferred path: RPC (keeps audit consistent)
  try {
    const { data, error } = await supabase.rpc("arka_close_day", {
      p_day_id: day_id,
      p_user: closed_by || null,
    });
    if (!error && data) return data;
  } catch {
    // fallback below
