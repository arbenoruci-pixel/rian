...
export async function dbListHistoryDays(limitDays = 30) {
  const { data, error } = await supabase
    .from('arka_cycles')
    // only fields we need (faster + avoids schema drift)
    .select('day_key,day,created_at,cycle_no,end_cash,expected_cash,closed_at')
    .order('created_at', { ascending: false })
    .limit(300);
  ...
  const byDay = new Map();
  for (const c of (data || [])) {
    const day_key = c.day_key || c.day || (c.created_at ? String(c.created_at).slice(0, 10) : null);
    if (!day_key) continue;
    if (!byDay.has(day_key)) {
      byDay.set(day_key, {
        id: day_key,
        day_key,
        // show last known cash of the day (end_cash if present, else expected_cash)
        expected_cash: 0,
      });
    }
    const d = byDay.get(day_key);
    const candidate = Number(c.end_cash ?? c.expected_cash ?? 0);
    if (candidate > Number(d.expected_cash ?? 0)) d.expected_cash = candidate;
  }
