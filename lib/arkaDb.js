...
export async function dbAddCycleMove(payload) {
  const { cycle_id, type, amount, note } = payload || {};
  if (!cycle_id) throw new Error('cycle_id mungon');
  const amt = Number(amount || 0);
  if (!Number.isFinite(amt) || amt <= 0) throw new Error('SHUMA S’ËSHTË VALIDE');

  // Disa versione të DB-së s’kanë kolonat e aktorit (p.sh. created_by_pin).
  // Prandaj: tentojmë me i dërgu, e nëse DB kthen "column ... does not exist",
  // e provojmë përsëri pa to (fail-safe).
  const row = {
    cycle_id,
  ...
  // Opsionale (nëse ekzistojnë në skemë)
  if (payload?.created_by != null) row.created_by = payload.created_by;
  if (payload?.created_by_name != null) row.created_by_name = payload.created_by_name;
  if (payload?.created_by_role != null) row.created_by_role = payload.created_by_role;
  if (payload?.created_by_pin != null) row.created_by_pin = payload.created_by_pin;

  let ins = await supabase.from('arka_cycle_moves').insert(row);
  if (ins.error) {
    const msg = String(ins.error?.message || '');
    // Retry pa kolonat që s’ekzistojnë
    if (msg.includes('does not exist') && msg.includes('created_by')) {
      const retryRow = { ...row };
      delete retryRow.created_by;
      delete retryRow.created_by_name;
      delete retryRow.created_by_role;
      delete retryRow.created_by_pin;
      ins = await supabase.from('arka_cycle_moves').insert(retryRow);
    }
  }
