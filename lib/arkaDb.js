...
export async function dbAddCycleMove(params = {}) {
  ...
  const type = (params.type || "OUT").toUpperCase();
...
// Safe list ordering helper (at vs created_at vs none)
...
export async function dbAddExpense(params = {}) {
  ...
  // 1) Insert expense first (source of truth)
  const payload = {
    day_key,
    amount,
    paid_from,
  ...
  // 2) Auto-create linked moves (idempotent with external_id = expense:<id>)
  ...
  // COMPANY_BUDGET -> arka_company_moves OUT
...
export async function dbListExpensesForDay(day_key, limit = 200) {
  ...
  return listWithSafeOrder("arka_expenses", { day_key: dk }, limit);
}
