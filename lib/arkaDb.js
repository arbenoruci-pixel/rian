...
export async function dbOpenCycle(payload = {}) {
  ...
  const insertRow = { day_key, cycle_no, handoff_status: 'OPEN', opening_cash, opening_source: payload.opening_source ?? 'COMPANY', opened_at: new Date().toISOString() };
...
export async function dbCloseCycle(payload = {}) {
  ...
  const upd = { handoff_status: 'CLOSED', status: 'CLOSED', expected_cash: expected, cash_counted: counted, discrepancy: counted - expected, end_cash: counted, closed_at: new Date().toISOString() };
...
export async function dbReceiveCycle(payload = {}) {
  const upd = { status: 'RECEIVED', handoff_status: 'RECEIVED', received_at: new Date().toISOString(), received_by: payload.received_by || null };
