...
import { budgetAddMove } from './companyBudgetDb';
...
export async function dbReceiveCycle(payload = {}) {
  ...
  // Lexo ciklin (per shumen dhe day_key) para se ta shenojme RECEIVED
  const pre = await supabase
    .from('arka_cycles')
    .select('id, day_key, closing_cash, status')
    .eq('id', cycle_id)
    .maybeSingle();
  if (pre.error) throw pre.error;
  const cycle = pre.data || null;
  ...
  // Regjistro hyrjen ne buxhet te kompanise (idempotent permes external_id)
  // Shuma merret nga closing_cash (nese mungon, s'bejme asgje)
  const amt = Number(cycle?.closing_cash ?? 0);
  if (amt > 0) {
    await budgetAddMove({
      type: 'IN_CASH',
      amount: amt,
      note: `ARKA RECEIVED ${cycle?.day_key || ''}`.trim(),
...
export async function dbListHistoryDays(limitDays = 30) {
  ...
  // Nxjerr ditët prej arka_cycles (day_key ruhet në rreshtet e cikleve)
  const { data, error } = await supabase
    .from('arka_cycles')
    .select('day_key, opening_cash, closing_cash, status, created_at')
  ...
  const byDay = new Map();
  for (const c of (data || [])) {
    const dk = c.day_key || (c.created_at ? String(c.created_at).slice(0, 10) : null);
    if (!dk) continue;
    ...
    d.cycles_count += 1;
    d.opening_sum += Number(c.opening_cash ?? 0);
    d.closing_sum += Number(c.closing_cash ?? 0);
  }
