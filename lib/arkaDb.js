...
// UI page po e importon dbGetTodayDay — e mbajmë për compat.
// Në këtë backend, “day row” ekziston vetëm kur hapet cikli.
// Për pagesa/shpenzime na mjafton day_key.
export async function dbGetTodayDay() {
...
>>>>>>>+main
 type =   const type = (params.type || "OUT").toUpperCase();
>>>>>>>+origin/main
...
// - CASH_TODAY | COMPANY_BUDGET | PERSONAL
   ========================= */
>>>>>>>+main
 list or// Safe list ordering helper (at vs created_at vs none)
async function listWithSafeOrder(table, filters = {}, limit = 200) {
  const base = supabase.from(table).select("*");
  let q = base;
>>>>>>>+origin/main
k, v] of Object.entries(filters || {})) {
    q = q.eq(k, v);
  }

  // try "at" -> "created_at" -> no order
  let r = await q.order("at", { ascending: false }).limit(limit);
  if (r.error) r = await q.order("created_at", { ascending: false }).limit(limit);
  if (r.error) r = await q.limit(limit);

  if (r.error) throw r.error;

  // stable sort client-side if needed
  const data = r.data || [];
  const ts = (row) => {
    const v = row?.at || row?.created_at || row?.inserted_at || row?.createdAt;
...
// paid_from: CASH_TODAY | COMPANY_BUDGET | PERSONAL
...
<<<<<<< main
  i  if (!Number.isFinite(amount) || amount <= 0) throw new Error("INVALID_AMOUNT");
>>>>>>>+main
origin/main
...
>>>>>>>+main
Insert e  // 1) Insert expense first (source of truth)
>>>>>>>+origin/main
= {
    day_key,
    amount,
...
<<<<<<< main
  c  const { data: exp, error } = await supabase
>>>>>>>+main
{ data:   const { data: exp, error: e1 } = await supabase
>>>>>>>+origin/main
expenses")
    .insert(payload)
    .select("*")
...
>>>>>>>+main
-create   // 2) Auto-create linked moves (idempotent with external_id = expense:<id>)
  const ext = exp?.id ? `expense:${exp.id}` : null;

  // CASH_TODAY -> arka_moves OUT (requires active cycle)
>>>>>>>+origin/main
"CASH_TODAY") {
    // If no active cycle, expense is still saved, but we should signal it.
    // However your UI already blocks if cycleOpen==false.
    await dbAddCycleMove({
      type: "OUT",
      amount,
      note: `SHPENZIM • ${category}${note ? ` • ${note}` : ""}`,
      created_by,
<<<<<<< main
      exter      external_id: `EXP_${exp.id}`,
    });
  }
...
d: ext,
      external_id: ext,
>>>>>>>+origin/main
Y_BUDGET -> arka_company_moves OUT
...
>>>>>>>+main
eOrder("  return listWithSafeOrder("arka_expenses", { day_key: dk }, limit);
}

/* =========================
   COMPANY BUDGET MOVES (arka_company_moves)
>>>>>>>+origin/main
...
export async function dbAddCompanyMove(params = {}) {
  ...
  // Idempotency when external_id exists
  if (external_id) {
    const { data: existing, error: e0 } = await supabase
...
export async function dbListCompanyMoves(limit = 200) {
  return listWithSafeOrder("arka_company_moves", {}, limit);
}

<<<<<<< main
// Backward/com// Backward/compat aliases
...
<<<<<<< main
  // Day helper  // Day helpers
  dbGetTodayDay,
  dbGetTodayDayKey,
  // Day helper
  dbGetTodayDay,
>>>>>>>+main
compat f  dbGetTodayDay, // compat for UI
  dbGetTodayDayKey,
>>>>>>>+origin/main
e,
