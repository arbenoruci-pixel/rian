...
export async function dbAddCycleMove(payload) {
  if (!payload) throw new Error("payload mungon");

  // HARD STRIP: never send these (your DB schema doesn't have them)
  try {
    delete payload.created_by_pin;
    delete payload.created_by_pin;
    delete payload.approved_by_pin;
    delete payload.rejected_by_pin;
  } catch {}
  ...
  // Build ONLY the safe columns we know exist.
  const row = {
    cycle_id,
  ...
  // Retry by removing any columns the API says are missing (schema cache).
  let cur = { ...row };
