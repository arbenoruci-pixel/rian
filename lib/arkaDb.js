...
function encodeAuditForNote(note, audit = {}) {
  try {
    const cleanNote = String(note || '');
    const payload = {
      created_by: audit.created_by ?? null,
      created_by_name: audit.created_by_name ?? null,
      created_by_pin: audit.created_by_pin ?? null,
      created_by_role: audit.created_by_role ?? null,
    };
    if (!payload.created_by && !payload.created_by_name && !payload.created_by_pin && !payload.created_by_role) {
      return cleanNote;
    }
...
function decodeAuditFromNote(note) {
  const raw = String(note || '');
  const m = raw.match(/\n?\[\[ARKA_AUDIT:([^\]]+)\]\]\s*$/);
  if (!m) return { cleanNote: raw, audit: null };
  try {
    const audit = JSON.parse(decodeURIComponent(m[1] || '')) || null;
    const cleanNote = raw.slice(0, m.index).trimEnd();
    return { cleanNote, audit };
  } catch {
    return { cleanNote: raw, audit: null };
  }
}

function normalizeCycleMove(row) {
  if (!row) return row;
  const { cleanNote, audit } = decodeAuditFromNote(row.note);
  const created_by_name = row.created_by_name ?? audit?.created_by_name ?? null;
  const created_by_pin = row.created_by_pin ?? audit?.created_by_pin ?? null;
  const created_by_role = row.created_by_role ?? audit?.created_by_role ?? null;
  const created_by = row.created_by ?? audit?.created_by ?? created_by_name ?? null;
  return {
    ...row,
    note: cleanNote,
    created_by,
...
export async function dbGetCycleMoves(cycleId) {
  const { data, error } = await supabase
    ...
    .eq('cycle_id', cycleId);
  if (error) return [];
  return Array.isArray(data) ? data.map(normalizeCycleMove) : [];
}
...
export async function dbAddCycleMove(payload) {
  ...
  const cleanNote = String(note || '');
  const auditPayload = {
    created_by: created_by_name ?? null,
  ...
  const rowMin = {
    ...
    type: String(type || 'OUT').toUpperCase(),
    amount: amt,
    note: encodeAuditForNote(cleanNote, auditPayload),
  };
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (m && m[1]) {
        ...
        if (Object.prototype.hasOwnProperty.call(row, col)) {
          delete row[col];
          if (/^created_by(_name|_pin|_role)?$/i.test(col)) {
            row.note = encodeAuditForNote(cleanNote, auditPayload);
          }
          continue;
        }
      }
