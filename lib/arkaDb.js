...
export async function dbGetCarryoverToday() {
  ...
  if (!day?.id)
    return { carryover_cash: 0, carryover_source: null, carryover_person_pin: null };

  return {
    carryover_cash: Number(day.carryover_cash || 0),
    carryover_source: day.carryover_source || null,
    carryover_person_pin: day.carryover_person_pin || null,
  };
}
...
export async function dbSetCarryoverToday({
  ...
  const payload = {
    carryover_cash: Number(carryover_cash || 0),
    carryover_source: carryover_source ? String(carryover_source) : null,
    carryover_person_pin: carryover_person_pin ? String(carryover_person_pin) : null,
  };
