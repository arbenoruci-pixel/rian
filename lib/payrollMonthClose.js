...
function isWithinMonth(row, month) {
  ...
  if (status === 'SALARY_PAID' || type === 'SALARY_PAYMENT') {
    const marker = `${clean(row?.note)} ${clean(row?.payroll_month)} ${clean(row?.month)}`.toUpperCase();
    if (marker.includes(String(month || '').toUpperCase())) return true;
  }
...
function matchesWorker(row, worker) {
  // In arka_pending_payments the creator fields identify the beneficiary of
  // an advance. Handed/approved fields identify the operator, not the debtor.
  if (up(row?.status) === 'ADVANCE' || up(row?.type) === 'ADVANCE') {
    return matchesPayrollRecipient(row, worker);
  }
  const pin = clean(worker?.pin);
  const name = up(worker?.name);
  const rowPins = [
...
export function matchesPayrollRecipient(row = {}, worker = {}) {
  const recipientPin = clean(row.created_by_pin || row.worker_pin);
  // An explicit recipient PIN wins over names (including same-name staff).
  if (recipientPin) return Boolean(clean(worker.pin) && recipientPin === clean(worker.pin));
  const recipientName = up(row.created_by_name || row.worker_name);
  return Boolean(recipientName && recipientName === up(worker.name));
}
...
export function buildMonthlyPayrollPreview({ workers = [], paymentRows = [], month = getCurrentPayrollMonth() } = {}) {
  ...
  return eligibleWorkers.map((worker, index) => {
    ...
    const salaryPaidRows = workerRows.filter((row) => up(row?.status) === 'SALARY_PAID' || up(row?.type) === 'SALARY_PAYMENT');
    ...
    const salaryPaidAmount = amountOf(salaryPaidRows);
    const salaryPaidAt = salaryPaidRows
    ...
    if (salaryPaidRows.length > 0) {
      statusKind = 'paid';
      statusLabel = 'PAGUAR';
    } else if (openCash > 0 || pendingHandoff > 0) {
      statusKind = 'blocked';
