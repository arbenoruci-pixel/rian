...
// One advisory phone lookup per form. Retry temporary failures twice, then
// wait for the user to retry or for the app/connection to return. Disposal
// invalidates late replies when a phone changes or the form closes.
export function watchDispatchPhoneCheck({ inspect, onResult, onError, onBusy,
  ...
  async function run() {
    ...
    try {
      ...
      if (isRecoverableDispatchPhoneCheck(error) && retries < 2 && available()) {
        schedule([1000, 3000][retries++]);
      }
    } finally {
      running = false;
