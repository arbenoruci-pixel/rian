...
function safeReadLocalStorageJson(key, fallback, { purgeOnError = true } = {}) {
  try {
    ...
    try {
      console.error(`[offlineStore] Corrupted localStorage at ${key}. Auto-healing...`, e);
...
function mirrorPush(row){
  try{
    const list = safeReadLocalStorageJson(LS_QUEUE_MIRROR, []);
...
export async function getAllOrdersLocal(){
  ...
  try {
    ...
    const q = safeReadLocalStorageJson('tepiha_offline_queue_v1', []);
    if (Array.isArray(q)) {
        q.forEach(it => { 
  ...
  let blacklist = [];
  try { blacklist = safeReadLocalStorageJson('tepiha_ghost_blacklist', []); } catch(e) {}
