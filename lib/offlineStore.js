...
function stripSyncMetaFields(obj = {}, table = '', opts = {}) {
  ...
  if (tableName === 'transport_orders') return sanitizeTransportOrderPayload(out, { patch: opts?.patch === true });
  return out;
}
...
async function sendOpToSupabase(op) {
  ...
  if (type === 'patch_order_data') {
    ...
    let patch = stripSyncMetaFields({ ...(payload || {}) }, table, { patch: true });
    delete patch.table;
    delete patch.id;
