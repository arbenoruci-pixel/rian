...
// Columns: id, direction, amount, reason, note, source, created_by, created_by_name: currentUser?.name || 'UNKNOWN',
      created_by_pin,
...
export async function budgetAddMove(move) {
  const payload = {
    ...
    created_by_name: currentUser?.name || 'UNKNOWN',
      created_by_pin: move.created_by_name: currentUser?.name || 'UNKNOWN',
      created_by_pin || null,
...
export async function budgetAddOutMove({ amount, reason, note, source, created_by, created_by_name: currentUser?.name || 'UNKNOWN',
      created_by_pin }) {
