...
export function sanitizeTransportOrderPayload(input = {}, opts = {}) {
  ...
  // client_tcode is a legacy mirror on transport_orders. For new orders mirror the
  // official order code when present; keep legacy fallbacks only so old rows do not vanish.
  const clientTcode = normalizeTCodeLoose(codeStr || base.client_tcode || nextData.client_tcode || '');
