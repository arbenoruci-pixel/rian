...
export function stripImmutableTransportOrderPatchColumns(input = {}) {
  const next = { ...(input || {}) };
  delete next.id;
  delete next.client_id;
  delete next.code_str;
  delete next.client_tcode;
  delete next.code_n;
  delete next.visit_nr;
  if (isPlainObject(next.data)) {
    next.data = stripTransportOrderPatchIdentityAliases(next.data);
  }
  return next;
}

const TRANSPORT_ORDER_DATA_IDENTITY_ALIASES = new Set([
  'code_str',
  'code',
  'client_id',
  'client_tcode',
  'transport_client_tcode',
  'code_n',
  'visit_nr',
  'order_code',
  'order_tcode',
...
const TRANSPORT_CLIENT_DATA_IDENTITY_ALIASES = new Set([
  ...TRANSPORT_ORDER_DATA_IDENTITY_ALIASES,
  'id',
  'code',
  'tcode',
]);

function withoutIdentityAliases(value, aliases) {
...
// PATCH payloads may come from old cached order snapshots. Their JSON often
// repeats the public/client T-code under several aliases. Keeping those stale
// aliases while protecting the real columns creates two conflicting identities
// in one row, so a patch may update business data but must not rewrite them.
export function stripTransportOrderPatchIdentityAliases(value = {}) {
  const out = withoutIdentityAliases(value, TRANSPORT_ORDER_DATA_IDENTITY_ALIASES);
  if (isPlainObject(out.client)) {
    out.client = withoutIdentityAliases(out.client, TRANSPORT_CLIENT_DATA_IDENTITY_ALIASES);
  }
  return out;
}
...
function transportCodeDigits(value) {
  return String(value || '').replace(/\D+/g, '').replace(/^0+/, '');
}

function assertTransportCodePairMatches({ codeStr = '', codeN = null } = {}) {
  const codeDigits = transportCodeDigits(codeStr);
  const rawN = transportCodeDigits(codeN);
  if (!codeDigits || !rawN) return;
  if (codeDigits !== rawN) {
    throw new Error(`TRANSPORT_CODE_MISMATCH: code_str ${normalizeTCodeLoose(codeStr)} nuk përputhet me code_n ${rawN}`);
  }
}
...
export function sanitizeTransportOrderPayload(input = {}, opts = {}) {
  ...
  // Official public order code must come only from order-code fields.
  // Do NOT derive code_str from client_tcode / transport_clients.tcode / data.client.code,
  // otherwise cancelled/deferred client codes can leak into a new active order.
  const codeStr = normalizeTCodeLoose(
    base.code_str ||
    base.order_code ||
    base.order_tcode ||
    base.official_order_code ||
    base.t_code ||
    nextData.code_str ||
    nextData.order_code ||
    nextData.order_tcode ||
    nextData.official_order_code ||
    nextData.t_code ||
    ''
  );
  const omitIdentityColumns = opts.omitIdentityColumns === true || opts.patch === true;
  if (!omitIdentityColumns) {
    assertTransportCodePairMatches({ codeStr, codeN: rawCodeN });
    if (codeStr) out.code_str = codeStr;
  }

  // client_tcode is the permanent Transport client identity. code_str may remain a
  // legacy order alias on historical rows; new permanent-code writes normally use the same T-code.
  const clientTcode = normalizeTCodeLoose(
    base.client_tcode ||
    nextData.transport_client_tcode ||
    nextData.client_tcode ||
    clientObj.transport_client_tcode ||
    clientObj.client_tcode ||
    clientObj.tcode ||
    clientObj.code ||
    codeStr ||
    ''
  );
  if (!omitIdentityColumns && clientTcode) out.client_tcode = clientTcode;

  // For new create payloads that include code_n, normalize it from code_str.
  // Existing update payloads without code_n keep DB values untouched.
  if (!omitIdentityColumns && rawCodeN !== undefined && rawCodeN !== null && codeStr) {
    const derivedCodeN = Number(transportCodeDigits(codeStr));
    if (Number.isFinite(derivedCodeN) && derivedCodeN > 0) out.code_n = derivedCodeN;
  }
  ...
  const clean = stripUndefinedShallow(out);
  if (opts.patch === true) return stripImmutableTransportOrderPatchColumns(clean);
  return clean;
}
