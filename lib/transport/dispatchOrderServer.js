import { resolveFamilyPhone } from '../clientFamilyServer.js';
...
const DISPATCH_ORDER_ROLES = new Set([...ADMIN_ROLES, 'MASTER'].map((role) => String(role || '').trim().toUpperCase()));
const ALLOWED_STATUSES = new Set(['INBOX', 'ASSIGNED']);
const ACTIVE_DISPATCH_ORDER_STATUSES = new Set([
  '',
  'new',
  'inbox',
  'pending',
...
function verifyActiveDispatchOrder(row, phoneKey) {
  if (!row?.id) fail('DISPATCH_ACTIVE_ORDER_NOT_FOUND', 409);
  ...
  if (!rowPhone || rowPhone !== phoneKey) fail('DISPATCH_ACTIVE_ORDER_PHONE_MISMATCH', 409);
  const status = String(row.status || data.status || '').trim().toLowerCase();
  if (!ACTIVE_DISPATCH_ORDER_STATUSES.has(status)) fail('DISPATCH_ACTIVE_ORDER_STATUS_CHANGED', 409);
  ...
  if (!String(row.client_id || '').trim() || !clientTcode || clientTcode !== publicCode) {
    fail('DISPATCH_ACTIVE_ORDER_IDENTITY_INVALID', 409);
  }
  return row;
}
...
export async function listDispatchTransportOrdersServer(bodyLike, { supabase, authUser } = {}) {
  if (!supabase || !authUser?.id || !authUser?.pin) fail('AUTH_REQUIRED', 401);
  if (!DISPATCH_ORDER_ROLES.has(String(authUser.role || '').trim().toUpperCase())) fail('DISPATCH_ORDER_ACTOR_NOT_ALLOWED', 403);
  if (cleanUuid(bodyLike?.expected_actor_id) !== String(authUser.id).toLowerCase()) fail('DISPATCH_ORDER_ACTOR_SESSION_MISMATCH', 403);
  // Same approved-device authority as CREATE. Fixed table/limit; callers cannot
  // select another table or expand this into an unbounded administrative query.
  const { data, error } = await supabase.from('transport_orders').select('*')
    .order('updated_at', { ascending: false }).order('id', { ascending: false }).limit(160);
  if (error || !Array.isArray(data)) fail('DISPATCH_LIST_FAILED', 503);
  return { items: data, actorId: String(authUser.id) };
}

export async function inspectDispatchTransportPhoneServer(bodyLike, { supabase, authUser } = {}) {
  if (!supabase || !authUser?.id || !authUser?.pin) fail('AUTH_REQUIRED', 401);
  ...
  const { data, error } = await supabase.rpc('inspect_dispatch_transport_phone', { p_phone: phone });
  if (error) fail('DISPATCH_PHONE_CHECK_FAILED', 503);
  let payload = normalizeRpcResult(data);
  const familyClient = await resolveFamilyPhone(supabase, 'TRANSPORT', phone);
  if (familyClient) payload = { ...payload, status: 'FOUND', candidate: familyClient };
  const status = String(payload?.status || '').trim().toUpperCase();
  if (!payload || !['FOUND', 'NOT_FOUND', 'CONFLICT'].includes(status)) {
    fail('DISPATCH_PHONE_CHECK_INVALID_RESPONSE', 503);
  }
  if (status === 'CONFLICT') {
    fail('TRANSPORT_PHONE_IDENTITY_CONFLICT', 409, {
      clientIds: Array.isArray(payload?.client_ids) ? payload.client_ids : [],
      tcodes: Array.isArray(payload?.tcodes) ? payload.tcodes : [],
    });
  }

  const client = isPlainObject(payload?.candidate) ? payload.candidate : null;
  if ((status === 'FOUND') !== Boolean(client)) fail('DISPATCH_PHONE_CHECK_INVALID_RESPONSE', 503);
  if (client) {
    const candidatePhone = normalizeTransportPhoneKeyServer(client.phone_digits || client.phone || '');
    const candidateTcode = normalizeTransportTCodeServer(client.tcode || client.client_tcode || client.code_str || '');
    // DISPATCH_HISTORY_PHONE_V1: archived visits may predate the client master.
    // Keep their real order reference separate from a canonical client UUID.
    const historyOrderId = client.source === 'transport_orders' ? cleanUuid(client.row_id) : '';
    const candidateId = cleanUuid(client.id);
    if (candidatePhone !== phoneKey || !candidateTcode
      || (client.id && !candidateId) || (!candidateId && !historyOrderId)) {
      fail('DISPATCH_PHONE_CHECK_CLIENT_MISMATCH', 503);
    }
  }

  const activeOrderSummary = isPlainObject(payload?.active_order) ? payload.active_order : null;
  let activeOrder = null;
  if (activeOrderSummary?.id) {
    const activeOrderId = cleanUuid(activeOrderSummary.id);
    if (!activeOrderId) fail('DISPATCH_PHONE_CHECK_ACTIVE_ID_INVALID', 503);
    activeOrder = verifyActiveDispatchOrder(await fetchExactOrder(supabase, activeOrderId), phoneKey);
  }
...
export async function editDispatchTransportClientServer(bodyLike, { supabase, authUser } = {}) {
  // DISPATCH_BOSS_CONTROLS_V1: approved Dispatch is an elevated operational admin in this app.
  if (!supabase || !authUser?.id || !authUser?.pin) fail('AUTH_REQUIRED', 401);
  const actorRole = String(authUser.role || '').trim().toUpperCase();
  if (!DISPATCH_ORDER_ROLES.has(actorRole)) fail('DISPATCH_ORDER_ACTOR_NOT_ALLOWED', 403);
  ...
  return { ok: true, marker: 'DISPATCH_BOSS_CONTROLS_V1', client: updatedClient, order: updatedOrder };
}
...
export function buildDispatchPhoneRecoveryOrderId(orderId, actorId, phoneKey) {
  // UUID v5: an old installed PWA may resend the same stale ID indefinitely.
  // Deterministic recovery makes every retry target the same replacement visit.
  const namespace = Buffer.from('6ba7b8109dad11d180b400c04fd430c8', 'hex');
  const bytes = createHash('sha1').update(namespace).update(JSON.stringify([
    'tepiha-dispatch-phone-recovery-v1', orderId, actorId, phoneKey,
...
export async function createDispatchTransportOrderServer(bodyLike, { supabase, authUser, recoverStalePhone = true } = {}) {
  if (!supabase || !authUser?.id || !authUser?.pin) fail('AUTH_REQUIRED', 401);
  const actorRole = String(authUser.role || '').trim().toUpperCase();
  if (!DISPATCH_ORDER_ROLES.has(actorRole)) fail('DISPATCH_ORDER_ACTOR_NOT_ALLOWED', 403);

  const body = isPlainObject(bodyLike) ? bodyLike : {};
  // A durable request must never be sent under a different login after resume.
  if (body.expected_actor_id != null && cleanUuid(body.expected_actor_id) !== String(authUser.id).toLowerCase()) {
    fail('DISPATCH_ORDER_ACTOR_SESSION_MISMATCH', 403);
  }
  const orderId = cleanUuid(body.id || body.order_id || body.orderId);
  if (!orderId) fail('DISPATCH_ORDER_UUID_INVALID', 400);
  const requestData = isPlainObject(body.data) ? body.data : {};
  ...
  async function recoverPhoneConflict() {
    if (!recoverStalePhone) fail('TRANSPORT_ORDER_IDEMPOTENCY_PHONE_CONFLICT', 409);
    const recoveredId = buildDispatchPhoneRecoveryOrderId(orderId, String(authUser.id), phoneKey);
    const recovered = await createDispatchTransportOrderServer({ ...body, id: recoveredId }, {
      supabase, authUser, recoverStalePhone: false,
    });
    // Installed clients already accept a different canonical UUID through the
    // verified-active-order response contract. Verify that contract here too.
    // Preserve the actual row/status; a progressed visit is never recreated.
    const isActive = ACTIVE_DISPATCH_ORDER_STATUSES.has(String(recovered.data?.status || '').toLowerCase());
    if (isActive) verifyActiveDispatchOrder(recovered.data, phoneKey);
    return {
      ...recovered,
      deduplicatedActive: isActive,
      recoveredStaleIntent: true,
  ...
  const existing = await fetchExactOrder(supabase, orderId);
  if (existing) {
    const existingPhone = normalizeTransportPhoneKeyServer(
      existing.client_phone || existing.data?.client_phone || existing.data?.client?.phone || '',
    );
    if (isValidTransportPhoneServer(existingPhone) && existingPhone !== phoneKey) {
      return recoverPhoneConflict();
    }
  ...
  if (rpc?.error) {
    const code = rpcErrorCode(rpc.error);
    if (code === 'TRANSPORT_ORDER_IDEMPOTENCY_PHONE_CONFLICT') return recoverPhoneConflict();
    if (code === 'DISPATCH_ACTIVE_ORDER_EXISTS') {
      const inspection = await inspectDispatchTransportPhoneServer({ phone }, { supabase, authUser });
      if (!inspection?.activeOrder) fail('DISPATCH_ACTIVE_ORDER_NOT_FOUND', 409);
      return {
        ok: true,
        data: inspection.activeOrder,
        idempotent: true,
        deduplicatedActive: true,
        requestedOrderId: orderId,
        fingerprint,
      };
    }
