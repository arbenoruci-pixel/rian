...
export async function upsertTransportClient(input) {
  try {
    ...
    if (phone_digits) {
      const variants = phoneDigitVariants(phone_digits);
      ...
      const phoneMatch = !findErr && Array.isArray(found)
        ? found.find((row) => samePhoneDigits(row?.phone_digits || row?.phone || '', phone_digits))
        : null;
    ...
    // No T-code fallback for transport client identity.
    // Existing transport clients are matched only by normalized phone_digits;
    // a same-name or same-T-code row must not silently become the selected client.
