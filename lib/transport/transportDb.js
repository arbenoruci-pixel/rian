...
export async function inspectDispatchTransportPhoneViaApi(phoneValue, options = {}) {
  ...
  const timeoutMs = Math.max(Number(options?.timeoutMs || 0) || 0, 12000);
  ...
  try {
    response = await fetch('/api/transport/order', {
      ...
      body: JSON.stringify({ action: 'PHONE_CHECK', phone: phoneValue }),
    });
  } catch (error) {
    const code = error?.name === 'AbortError'
      ? 'DISPATCH_PHONE_CHECK_TIMEOUT'
      : 'DISPATCH_PHONE_CHECK_NETWORK_FAILED';
  ...
  if (!response.ok || body?.ok !== true) {
    const code = String(body?.error || `DISPATCH_PHONE_CHECK_HTTP_${response.status || 0}`);
    throw Object.assign(new Error(code), { code, httpStatus: Number(response.status) || 0 });
  }
...
async function insertAtomicDispatchOrderViaApi(input, expected) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      const reconciled = await reconcileAtomicDispatchOrder(expected);
      if (reconciled) {
