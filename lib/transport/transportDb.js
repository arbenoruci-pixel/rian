import { lookupFamilyPhone } from '../clientFamilyClient.js';
'use client';
import { approvedApiRequest } from '../approvedApiRequest.js';
import { fetchJsonWithDeadline, withDeadline } from '../boundedRequest.js';
...
function normalizeTransportClientCandidate(row = {}, source = '') {
  ...
  // The lookup RPC already projects id=client_id and row_id=order_id.
  // Raw orders use id=order_id; never turn that ID into a client identity.
  const isLookupCandidate = isOrder && row?.source === 'transport_orders' && Object.hasOwn(row, 'row_id');
  ...
  return {
    id: isLookupCandidate ? (row?.id || null) : isOrder ? (row?.client_id || data?.client_id || client?.id || null) : (row?.id || row?.client_id || null),
    row_id: isLookupCandidate ? row.row_id : row?.id || null,
...
export async function findTransportClientByPhoneOnly(phoneValue, options = {}) {
  ...
  const familyClient = await lookupFamilyPhone('TRANSPORT', phoneValue, { signal });
  if (familyClient) return normalizeTransportClientCandidate(familyClient, 'transport_clients');
...
export async function inspectDispatchTransportPhoneViaApi(phoneValue, options = {}) {
  const phoneKey = normalizeTransportPhoneKey(phoneValue);
  if (!isValidTransportPhoneDigits(phoneKey)) {
    return { phoneKey, client: null, activeOrder: null };
  }

  let body;
  try {
    body = await approvedApiRequest('/api/transport/order', { action: 'PHONE_CHECK', phone: phoneValue }, { timeoutMs: options.timeoutMs || 15000, signal: options.signal });
  } catch (error) {
    if (error?.httpStatus) {
      if ([408, 425, 429, 500, 502, 503, 504].includes(Number(error.httpStatus))
        && !/^(DISPATCH_PHONE_CHECK_FAILED|AUTH_DEVICE_LOOKUP_FAILED|AUTH_USER_LOOKUP_FAILED)$/.test(String(error.code || error.message || ''))
        && !/AUTH_REQUIRED|NOT_APPROVED|NOT_ALLOWED|MISMATCH|CONFLICT|INVALID|DISABLED|RETIRED|OTHER_USER/.test(String(error.code || error.message || ''))) {
        const code = `DISPATCH_PHONE_CHECK_HTTP_${error.httpStatus}`;
        throw Object.assign(new Error(code), { code, httpStatus: error.httpStatus, cause: error });
      }
      throw error;
    }
    if (error?.status === 401 || error?.status === 403) throw error;
    const code = error?.name === 'AbortError' ? 'DISPATCH_PHONE_CHECK_TIMEOUT' : 'DISPATCH_PHONE_CHECK_NETWORK_FAILED';
    throw Object.assign(new Error(code), { code, cause: error });
  }

  const responsePhoneKey = normalizeTransportPhoneKey(body?.phoneKey || phoneValue);
  if (responsePhoneKey !== phoneKey) throw new Error('DISPATCH_PHONE_CHECK_RESPONSE_MISMATCH');

  const client = body?.client && typeof body.client === 'object'
    ? normalizeTransportClientCandidate(body.client, body.client?.source || 'transport_clients')
    : null;
  if (client) {
    const clientPhoneKey = normalizeTransportPhoneKey(client.phone_digits || client.phone || '');
    const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    const historyIdentity = client.source === 'transport_orders' && uuid.test(String(client.row_id || ''));
    if (clientPhoneKey !== phoneKey || (client.id && !uuid.test(String(client.id)))
      || (!client.id && !historyIdentity) || !normTCode(client.tcode || client.client_tcode || client.code_str)) {
      throw new Error('DISPATCH_PHONE_CHECK_CLIENT_MISMATCH');
    }
  }

  const activeOrder = body?.activeOrder && typeof body.activeOrder === 'object'
    ? body.activeOrder
    : null;
  if (activeOrder) {
    const orderPhoneKey = normalizeTransportPhoneKey(
      activeOrder.client_phone || activeOrder?.data?.client_phone || activeOrder?.data?.client?.phone || '',
    );
    if (!activeOrder.id || orderPhoneKey !== phoneKey) {
      throw new Error('DISPATCH_PHONE_CHECK_ACTIVE_ORDER_MISMATCH');
    }
  }

  return { phoneKey, client, activeOrder };
}

export async function editDispatchTransportClientViaApi(input = {}, options = {}) {
  // DISPATCH_BOSS_CONTROLS_V1 compatibility: bounded legacy request; new edits use EDIT_ORDER.
  return approvedApiRequest('/api/transport/order', { action: 'CLIENT_ADMIN_EDIT',
    client_id: input.clientId || input.client_id, order_id: input.orderId || input.order_id,
    name: input.name, phone: input.phone, address: input.address,
  }, options);
}

export async function editDispatchOrderViaApi(input, options = {}) {
  return approvedApiRequest('/api/transport/order', { ...input, action: 'EDIT_ORDER' }, options);
}
...
async function reconcileAtomicDispatchOrder(expected) {
  try {
    let query = supabase
    ...
    if (typeof query?.timeout === 'function') {
      query = query.timeout(10000, 'DISPATCH_ORDER_RECONCILE_TIMEOUT');
    }
    const { data, error } = await query;
    if (error || !data) return null;
    assertAtomicTransportOrder(data, expected);
    return data;
  } catch {
    return null;
  }
...
async function insertAtomicDispatchOrderViaApi(input, expected) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    let body;
    try {
      ({ response, body } = await fetchJsonWithDeadline('/api/transport/order', {
        method: 'POST',
        credentials: 'same-origin',
        cache: 'no-store',
        headers: { 'content-type': 'application/json' },
        // Durable Dispatch requests already survive navigation in IndexedDB.
        // Foreground saves need not share the browser's limited keepalive budget
        // with incident uploads. Navigation/timeout retries retain the same UUID.
        keepalive: !input.expected_actor_id && requestJson.length < 60000,
        body: requestJson,
      }, input.expected_actor_id ? 12000 : 35000));
    } catch (error) {
      lastFailure = {
        ok: false,
      ...
      const reconciled = input.expected_actor_id ? null
        : await withDeadline(() => reconcileAtomicDispatchOrder(expected), 10000).catch(() => null);
      if (reconciled) {
        return { ok: true, data: reconciled, idempotent: true, reconciledAfterTimeout: true };
      }
      if (attempt === 0 && !input.expected_actor_id) continue;
      return lastFailure;
    }
    if (attempt === 0 && response.status === 401 && body?.error === 'AUTH_REQUIRED') {
      try {
        const session = await ensureApprovedDeviceSession({ force: true, timeoutMs: 5500 });
        if (session?.offline) return { ok: false, error: 'DISPATCH_ORDER_API_NETWORK_FAILED' };
        continue;
      } catch (error) {
        return { ok: false, error: error?.code || 'AUTH_REQUIRED' };
      }
    }
    if (!response.ok || body?.ok !== true || !body?.data) {
      ...
      if (attempt === 0 && !input.expected_actor_id && [502, 503, 504].includes(Number(response.status))) continue;
      return lastFailure;
    }

    try {
      if (body?.recoveredStaleIntent === true) {
        if (String(body?.requestedOrderId || '') !== expected.id || !isTransportOrderUuid(body.data.id)) {
          throw new Error('DISPATCH_RECOVERY_RESPONSE_MISMATCH');
        }
        assertAtomicTransportOrder(body.data, { ...expected, id: body.data.id });
      } else if (body?.deduplicatedActive === true) {
        assertDeduplicatedActiveDispatchOrder(body.data, expected);
      } else {
    ...
    return {
      ...
      deduplicatedActive: body?.deduplicatedActive === true,
      recoveredStaleIntent: body?.recoveredStaleIntent === true,
      requestedOrderId: String(body?.requestedOrderId || ''),
      fingerprint: String(body?.fingerprint || ''),
      offlineLeaseResult: body?.offlineLeaseResult || null,
    };
  }
  return lastFailure || { ok: false, error: 'DISPATCH_ORDER_API_FAILED' };
}

async function insertAtomicTransportSelfEntryViaApi(input, expected) {
  let requestJson = '';
  try { requestJson = JSON.stringify(input); } catch {
    return { ok: false, error: 'TRANSPORT_SELF_ENTRY_API_PAYLOAD_INVALID' };
  }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    let response;
    try {
      response = await fetch('/api/transport/self-order', {
        method: 'POST',
        credentials: 'same-origin',
        cache: 'no-store',
        headers: { 'content-type': 'application/json' },
      ...
      lastFailure = {
        ok: false,
        error: error?.name === 'AbortError'
          ? 'TRANSPORT_SELF_ENTRY_API_TIMEOUT'
          : 'TRANSPORT_SELF_ENTRY_API_NETWORK_FAILED',
        cause: error,
      };
      if (attempt === 0) continue;
      return lastFailure;
    ...
    if (!response.ok || body?.ok !== true || !body?.data) {
      lastFailure = {
        ok: false,
        error: String(body?.error || `TRANSPORT_SELF_ENTRY_API_HTTP_${response.status || 0}`),
  ...
  return lastFailure || { ok: false, error: 'TRANSPORT_SELF_ENTRY_API_FAILED' };
}
...
const DISPATCH_ACTIVE_DEDUP_STATUSES = new Set([
  '',
  'new',
  'inbox',
  'pending',
...
function assertDeduplicatedActiveDispatchOrder(row = {}, expected = {}) {
  if (!row?.id) throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_ID_MISSING');
  if (!row?.client_id) throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_CLIENT_ID_MISSING');
  const wantedPhone = normalizeTransportPhoneKey(expected?.phone || '');
  const rowPhone = normalizeTransportPhoneKey(row?.client_phone || row?.data?.client_phone || row?.data?.client?.phone || '');
  if (!wantedPhone || wantedPhone !== rowPhone) throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_PHONE_MISMATCH');
  const status = String(row?.status || row?.data?.status || '').trim().toLowerCase();
  if (!DISPATCH_ACTIVE_DEDUP_STATUSES.has(status)) throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_STATUS_MISMATCH');
  const permanentTcode = normTCode(
    row?.client_tcode || row?.data?.transport_client_tcode || row?.data?.client_tcode || row?.data?.client?.tcode || '',
  );
  const publicCode = normTCode(row?.code_str || row?.data?.code_str || row?.data?.order_code || '');
  if (!permanentTcode || publicCode !== permanentTcode) {
    throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_TCODE_MISMATCH');
  }
  if (!(Number(row?.visit_nr) > 0)) throw new Error('DISPATCH_ACTIVE_ORDER_VERIFY_VISIT_NR_MISSING');
  return { row, permanentTcode };
}
...
export async function insertTransportOrder(input) {
  ...
  try {
    ...
    const createFlow = String(dataObj?.transport_create_flow || dataObj?.order_create_flow || '').trim().toUpperCase();
    const useAdminPranimiServerCreate = ADMIN_ROLES.includes(createdByRole)
      && createFlow === 'PRANIMI'
      && tcodeAllocationMode === 'ATOMIC_DB';
    ...
    // Transport Pranimi writes are authorized by the approved device on the
    // server. This also upgrades legacy queued Transport rows that predate the
    // explicit atomic marker, without rewriting other roles in the sync engine.
    if (createdByRole === 'TRANSPORT') {
      const clientAddress = String(
        input?.address || input?.client_address || clientData?.address || dataObj?.address || '',
      ).trim();
      return insertAtomicTransportSelfEntryViaApi({
        ...input,
        action: 'CREATE',
        id: orderId,
    ...
    // ADMIN/DISPATCH Pranimi is online-only until its offline lease lifecycle
    // has a dedicated server finalizer. Never consume or ignore a browser code
    // here: a supplied code/lease must remain visible as an unsupported request.
    if (useAdminPranimiServerCreate) {
      const hasOfflineLease = !!(
        dataObj?.offline_code_lease
        || dataObj?.pranimi_code_lifecycle?.offline_code_lease
        || input?.offline_code_lease
      );
      const suppliedPranimiCode = normTCode(
        input?.code_str
        || input?.client_tcode
        || input?.order_code
        || dataObj?.code_str
        || dataObj?.order_code
        || dataObj?.order_tcode
        || dataObj?.client_tcode
        || '',
      );
      if (hasOfflineLease || suppliedPranimiCode) {
        return { ok: false, error: 'DISPATCH_PRANIMI_OFFLINE_CODE_UNSUPPORTED', httpStatus: 409 };
      }
      const clientAddress = String(
        input?.address || input?.client_address || clientData?.address || dataObj?.address || '',
      ).trim();
    ...
    if (useDispatchServerCreate) {
      ...
      return insertAtomicDispatchOrderViaApi({
        id: orderId,
        expected_actor_id: input.expected_actor_id || undefined,
        client_name: clientName,
        client_phone: clientPhone,
        address: clientAddress,
        gps_lat: input?.gps_lat ?? dataObj?.gps_lat ?? clientData?.gps_lat ?? clientData?.gps?.lat ?? null,
