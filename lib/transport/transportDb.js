...
export async function findTransportClientByPhoneOnly(phoneValue, options = {}) {
  ...
  try {
    ...
    (Array.isArray(data) ? data : []).forEach((row) => pushCandidate(row, 'transport_clients'));
  } catch (error) {
    throw new Error(`TRANSPORT_CLIENT_PHONE_LOOKUP_FAILED: ${error?.message || error || 'UNKNOWN'}`);
  }

  if (!candidates.length) {
    // Strict normalized fallback for legacy client formatting. This must run before
    // order-history recovery so an existing master client always wins.
    let query = supabase
      .from('transport_clients')
      .select('id,tcode,name,phone,phone_digits,address,gps_lat,gps_lng,updated_at')
      .order('updated_at', { ascending: false })
      .limit(5000);
    if (typeof query?.timeout === 'function') query = query.timeout(Math.max(timeoutMs, 7500), 'TRANSPORT_CLIENT_PHONE_HISTORY_TIMEOUT');
    const { data, error } = await query;
    if (error) throw new Error(`TRANSPORT_CLIENT_PHONE_HISTORY_LOOKUP_FAILED: ${error?.message || error || 'UNKNOWN'}`);
    (Array.isArray(data) ? data : []).forEach((row) => pushCandidate(row, 'transport_clients'));
  }
  ...
  if (!candidates.length) {
    ...
    const { data, error } = await query;
    if (error) throw new Error(`TRANSPORT_ORDER_PHONE_LOOKUP_FAILED: ${error?.message || error || 'UNKNOWN'}`);
    (Array.isArray(data) ? data : []).forEach((row) => pushCandidate(row, 'transport_orders'));
  }

  if (!candidates.some((row) => row?.source === 'transport_clients')) {
    // Always inspect the complete small historical order set after the exact query.
    // One exact-format row is not enough to prove identity: an older formatting variant
    // for the same normalized phone may carry a conflicting legacy T-code.
    let query = supabase
      .from('transport_orders')
      .select('id,client_id,client_tcode,code_str,client_name,client_phone,data,created_at,updated_at')
      .order('updated_at', { ascending: false })
      .limit(5000);
    if (typeof query?.timeout === 'function') query = query.timeout(Math.max(timeoutMs, 7500), 'TRANSPORT_ORDER_PHONE_HISTORY_TIMEOUT');
    const { data, error } = await query;
    if (error) throw new Error(`TRANSPORT_ORDER_PHONE_HISTORY_LOOKUP_FAILED: ${error?.message || error || 'UNKNOWN'}`);
    (Array.isArray(data) ? data : []).forEach((row) => pushCandidate(row, 'transport_orders'));
  }

  const clientIds = new Set(candidates.map((row) => String(row?.id || '').trim()).filter(Boolean));
  const tcodes = new Set(candidates.map((row) => normTCode(row?.tcode || '')).filter(Boolean));
  if (clientIds.size > 1 || tcodes.size > 1) {
    throw new Error(`TRANSPORT_PHONE_IDENTITY_CONFLICT:${phoneKey}:clients=${Array.from(clientIds).join(',') || '-'}:tcodes=${Array.from(tcodes).join(',') || '-'}`);
  }
