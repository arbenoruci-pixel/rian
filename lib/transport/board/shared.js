...
export function getCode(row) {
  ...
  return String(row?.code_str || row?.data?.code_str || row?.data?.order_code || row?.data?.order_tcode || row?.data?.official_order_code || row?.order_code || row?.t_code || row?.data?.t_code || row?.client_tcode || row?.data?.client_tcode || row?.code || "").trim();
}
