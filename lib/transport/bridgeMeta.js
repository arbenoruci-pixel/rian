...
const TRANSPORT_BASE_ALLOWED_STATUSES = new Set(['pastrim', 'at_base', 'in_base', 'base']);
const TRANSPORT_BASE_BLOCKED_STATUSES = new Set(['gati', 'ready', 'done', 'delivered', 'dorzuar', 'dorezuar', 'dorëzuar', 'canceled', 'cancelled', 'failed']);

function collectBridgeStatusSources(row, data, transport) {
  const rowData = asObj(row?.data);
  const fullOrder = asObj(row?.fullOrder);
  const fullOrderData = asObj(fullOrder?.data);
  const nodes = [
    transport,
    row?.transport, row?.transport_meta, row?.transportOrder,
    rowData?.transport, rowData?.transport_meta, rowData?.transportOrder,
  ...
  return Array.from(new Set([
    ...
    fullOrderData?.state,
    ...nodes.flatMap((node) => [node?.status, node?.state]),
  ].map((value) => normalizeStatus(value)).filter(Boolean)));
}
...
export function getTransportBaseSummary(row){
  ...
  const summary = {
    code: String(row?.code_str || data?.code_str || data?.order_code || data?.order_tcode || data?.official_order_code || row?.client_tcode || data?.client_tcode || data?.client?.tcode || '').trim().toUpperCase(),
...
export function isTransportBridgeReadyForBase(input) {
  ...
  const statuses = collectBridgeStatusSources(row, data, transport);

  if (statuses.some((status) => TRANSPORT_BASE_BLOCKED_STATUSES.has(status))) {
    return false;
  }

  const status = statuses.find((value) => TRANSPORT_BASE_ALLOWED_STATUSES.has(value)) || '';
  if (status) {
    return true;
  }
