...
const roles = new Set([...ADMIN_ROLES, 'MASTER', 'PUNTOR', 'PUNETOR', 'WORKER', 'BAZIST', 'BASE']);
...
export function normalizeReadyNotification(body, actor) {
  ...
  if (!roles.has(String(actor.role).toUpperCase())) fail('READY_NOTIFICATION_ROLE_DENIED', 403);
  ...
  if (!/^[1-9][0-9]*$/.test(orderId) || !Number.isSafeInteger(Number(orderId))) fail('READY_NOTIFICATION_ORDER_INVALID');
...
export async function readyNotificationServer(body, { supabase, authUser }) {
  ...
  if (!roles.has(String(authUser.role).toUpperCase())) fail('READY_NOTIFICATION_ROLE_DENIED', 403);
  ...
  if (body.action === 'GET_READY_NOTIFICATIONS') {
    ...
    if (!ids.length || ids.length > 100 || ids.some(id => !/^[1-9][0-9]*$/.test(id))) fail('READY_NOTIFICATION_ORDER_INVALID');
