...
// NOTE:
// Në këtë projekt, tabela e përdoruesve quhet `tepiha_users`.
// Kudo ku bëjmë INSERT/UPDATE duhet të përdorim këtë tabelë.
const WRITE_TABLE = 'tepiha_users';
...
export async function removeOrDeactivateUser(id) {
  if (!id) return { ok: false, error: new Error('ID is required') };
  ...
  try {
    ...
    return { ok: true, mode: 'deactivated', item: data || null };
  } catch (err) {
    return { ok: false, error: err, missingTable: isMissingTableError(err) };
  }
}
