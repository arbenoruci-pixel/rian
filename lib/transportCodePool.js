...
// - When online and POOL is low, reserve a batch via RPC reserve_transport_codes_batch(reserved_by,count)
...
export async function refillPoolIfNeeded(reservedBy, opts = {}) {
  ...
  if (pool.length >= threshold) {
    // also try flush used queue in background-ish
...
export async function takeCodeFromPoolOrOnline(reservedBy) {
  ...
  if (pool.length > 0) {
    ...
    // keep pool filled when possible
