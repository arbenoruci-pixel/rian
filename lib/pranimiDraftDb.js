...
async function findOrderRowById(sb, id) {
  const raw = String(id || '').trim();
  if (!/^\d+$/.test(raw)) return null;
  const { data, error } = await sb
    .from('orders')
    .select(PRANIMI_DRAFT_SELECT)
    .eq('id', Number(raw))
    .maybeSingle();
  if (error) throw error;
  return data || null;
...
async function writeDraftRow(sb, row, existing = null) {
  if (existing?.id) {
    if (isBlockingOrder(existing)) {
      return { ok: false, blocked: true, reason: 'EXISTING_FINAL_ORDER_FOR_LOCAL_OID', row: existing };
    }

    const expectedUpdatedAt = String(existing?.updated_at || '').trim();
    if (!expectedUpdatedAt) {
      return { ok: false, blocked: true, reason: 'DRAFT_COMPARE_AND_SWAP_TIMESTAMP_MISSING', row: existing };
    }

    // V476 TOCTOU guard: update the draft only if the exact row version read
    // before the write is still current. If final save wins the race and bumps
    // updated_at, this update matches zero rows and cannot downgrade the order.
    const { data, error } = await sb
      .from('orders')
      .update(row)
      .eq('id', existing.id)
      ...
      .maybeSingle();
    if (error) throw error;
    if (data) return { ok: true, blocked: false, row: data, via: 'update_compare_and_swap' };

    const current = await findOrderRowById(sb, existing.id);
    if (current && isBlockingOrder(current)) {
      return { ok: false, blocked: true, reason: 'FINAL_ORDER_WON_DRAFT_SAVE_RACE', row: current };
    }
    return { ok: false, blocked: true, reason: 'DRAFT_COMPARE_AND_SWAP_LOST', row: current || existing };
  }

  try {
    const { data, error } = await sb.from('orders').insert(row).select(PRANIMI_DRAFT_SELECT).maybeSingle();
    if (error) throw error;
    return { ok: true, blocked: false, row: data || null, via: 'insert' };
  } catch (error) {
    // A concurrent final insert/upsert can win after the initial lookup. Re-read
    // by local_oid and surface it as a protected conflict instead of retrying an
    // unconditional draft upsert over the final row.
    const current = await findDraftRowByLocalOid(sb, row?.local_oid);
    if (current && isBlockingOrder(current)) {
      return { ok: false, blocked: true, reason: 'FINAL_ORDER_WON_DRAFT_INSERT_RACE', row: current };
    }
    throw error;
  }
}
...
async function upsertDraft(sb, body = {}) {
  ...
  try {
    writeResult = await writeDraftRow(sb, prepareDraftRow(input, { topStatus: PRANIMI_DB_DRAFT_STATUS, reason }), existing);
  } catch (error) {
    firstError = error;
  }

  if (writeResult?.blocked) {
    return { ok: false, error: writeResult.reason || 'DRAFT_WRITE_BLOCKED', status: 409, row: writeResult.row || existing || null };
  }

  if (!writeResult?.ok) {
    try {
      writeResult = await writeDraftRow(
        sb,
        prepareDraftRow(input, { topStatus: PRANIMI_DB_DRAFT_FALLBACK_TOP_STATUS, reason: `${reason}_fallback_pranim` }),
        existing
      );
    } catch (error) {
  ...
  return { ok: true, verified: true, row: verified, via: writeResult?.via || (firstError ? 'api_service_fallback_pranim' : 'api_service_incomplete') };
}
