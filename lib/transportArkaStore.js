...
// Compat shim restored so transport payment flow can build and keep working.
...
function readRuntimeActor() {
  if (typeof window === 'undefined') return { pin: '', name: '', role: '' };
  try {
    const raw = localStorage.getItem('CURRENT_USER_DATA');
    const u = raw ? JSON.parse(raw) : null;
    if (u?.pin) return { pin: String(u.pin || ''), name: String(u.name || ''), role: String(u.role || '') };
  } catch {}
  try {
    const raw = localStorage.getItem('tepiha_transport_session_v1');
    const s = raw ? JSON.parse(raw) : null;
    if (s?.transport_pin || s?.pin || s?.transport_id) {
      return {
        pin: String(s.transport_pin || s.pin || s.transport_id || ''),
        name: String(s.transport_name || s.name || ''),
        role: String(s.role || 'TRANSPORT'),
      };
    }
  } catch {}
...
export async function addTransportTransferToBase({ transportId, transporterName = '', actorPin = '', actorName = '', actorRole = '', amount = 0, note = '' }) {
  const st = readTransportArka(transportId);
  const n = Number(amount || 0);
  if (!Number.isFinite(n) || n <= 0) return { ok: false, error: 'Invalid amount' };

  const runtimeActor = readRuntimeActor();
  const realPin = String(actorPin || runtimeActor.pin || '').trim();
  const realName = String(actorName || runtimeActor.name || transporterName || '').trim();
  const realRole = String(actorRole || runtimeActor.role || 'TRANSPORT').trim();
  ...
  const pseudoOrder = {
    ...
    created_by_pin: realPin || null,
    created_by_name: realName || null,
    created_by_role: realRole || null,
  };

  await addArkaRecordFromOrder(pseudoOrder, {
    paid: n,
    worker: realName || transporterName || `TRANSPORTER ${transportId}`,
    workerPin: realPin || null,
    source: 'TRANSPORT',
    extra: { transportId, amount: n, actorPin: realPin || null, actorName: realName || null, actorRole: realRole || null },
  });
...
export function openTransportDay() { return null; }
export function closeTransportDay() { return null; }
