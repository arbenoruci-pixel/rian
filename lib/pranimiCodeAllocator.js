...
// Online uses the normal DB allocator. Offline may consume only a server-leased code
// from the device/user's exclusive offline bank.

export const PRANIMI_ALLOCATOR_VERSION = 'oneway-v40-offline-bank';
export const PRANIMI_OFFLINE_PROOF_MAX_AGE_MS = 3 * 60 * 60 * 1000;

const ORDER_CODE_LS_PREFIX = 'base_order_code:';
const ORDER_PROOF_LS_PREFIX = 'pranimi_code_assignment_proof_v40:';
const LEGACY_PROOF_LS_PREFIX = 'pranimi_code_assignment_proof_v39_1:';
...
function isOfflineBankSource(value) {
  return String(value || '').trim().toUpperCase().includes('OFFLINE_BANK');
}

function normalizeProof(value = {}) {
  if (!value || typeof value !== 'object') return null;
  const pin = normalizePinForAllocator(value.pin ?? value.reserved_by ?? value.owner_id);
  ...
  const leaseExpiresAt = normalizeIso(value.lease_expires_at ?? value.leaseExpiresAt ?? value.expires_at);
  const source = String(value.source || 'DB_VERIFIED');
  const leaseToken = normalizeUuid(value.lease_token ?? value.token);
  const deviceId = String(value.device_id || '').trim();
  if (!pin || !oid || code == null || !verifiedAt || !leaseExpiresAt) return null;
  if (isOfflineBankSource(source) && (!leaseToken || !deviceId)) return null;
  return {
    ...
    offline_bank: isOfflineBankSource(source),
  };
}
...
function proofMatches(value, { pin, oid, code, now = Date.now() } = {}) {
  ...
  if (proof.offline_bank) {
    if (!proof.lease_token || !proof.device_id) return { ok: false, reason: 'OFFLINE_BANK_PROOF_INCOMPLETE', proof };
    return { ok: true, reason: 'OFFLINE_BANK_LEASE_PROOF', proof };
  }

  if (!Number.isFinite(verifiedAt) || verifiedAt + PRANIMI_OFFLINE_PROOF_MAX_AGE_MS <= now) {
    return { ok: false, reason: 'PROOF_TOO_OLD', proof };
  }
...
function normalizeAssignment(value, fallback = {}) {
  ...
  return {
    code,
    status: String(row.status || row.lease_status || fallback.status || 'reserved').trim().toLowerCase(),
    reserved_by: normalizePinForAllocator(row.reserved_by ?? row.pin ?? row.owner_id ?? fallback.pin),
    draft_session_id: String(row.draft_session_id ?? row.oid ?? fallback.oid ?? '').trim(),
    lease_expires_at: normalizeIso(row.lease_expires_at ?? row.leaseExpiresAt ?? row.expires_at ?? fallback.lease_expires_at),
    verified: row.verified !== false,
    reason: row.reason || null,
    source: row.source || fallback.source || 'DB_ASSIGNMENT',
    lease_token: normalizeUuid(row.lease_token ?? row.token),
...
export function createPranimiCodeAllocatorCore(deps = {}) {
  ...
  function getAssignmentProof(oidInput) {
    const oid = String(oidInput || '').trim();
    return oid ? normalizeProof(storage.getProof?.(oid)) : null;
  }
  ...
  function writeProof({ pin, oid, code, verdict = {}, source }) {
    ...
    if (!proof) {
      throw makeError('DB/BANKA NUK KTHEU PROVË TË PLOTË TË REZERVIMIT', 'ASSIGNMENT_PROOF_INCOMPLETE', { verdict });
    }
  ...
  async function onlineNow() {
    return db.isOnline ? !!(await db.isOnline()) : true;
  }
  ...
  async function verifyAssignedCode({ pin, oid, code, meaningful = false } = {}) {
    ...
    const existingProof = getAssignmentProof(id);
    if (existingProof?.offline_bank) {
      const offline = proofMatches(existingProof, { pin: realPin, oid: id, code: candidate });
      if (offline.ok) {
        return { displayable: true, verified: true, offlineBank: true, reason: offline.reason, code: candidate, proof: offline.proof };
      }
    }
  ...
  async function getOrAllocateInternal({ pin, oid, meaningful = false } = {}) {
    ...
    if (existing != null) {
      ...
      if (verdict.displayable) {
        return {
          code: existing,
          reused: true,
          source: verdict.proof?.offline_bank || verdict.offlineBank
            ? 'OFFLINE_BANK_ASSIGNMENT'
            : (verdict.offline ? 'OFFLINE_VERIFIED_ASSIGNMENT' : 'DB_VERIFIED_ASSIGNMENT'),
          verdict,
        };
      }
    ...
    if (!(await onlineNow())) {
      if (!db.reserveOffline) {
        throw makeError("S'KA KOD TË VERIFIKUAR PËR KËTË DRAFT. LIDHU ONLINE.", 'BASE_CODE_OFFLINE_EMPTY');
      }
      const assignment = normalizeAssignment(
        await db.reserveOffline({ pin: realPin, oid: id, meaningful }),
        { pin: realPin, oid: id, status: 'reserved', source: 'OFFLINE_BANK' },
      );
      if (!assignment) throw makeError("S'KA KOD TË REZERVUAR OFFLINE", 'BASE_CODE_OFFLINE_EMPTY');
      if (
        assignment.status !== 'reserved' ||
        assignment.reserved_by !== realPin ||
        assignment.draft_session_id !== id ||
        !assignment.lease_expires_at ||
        !assignment.lease_token ||
        !assignment.device_id ||
        assignment.verified === false
      ) {
        throw makeError('BANKA OFFLINE KTHEU REZERVIM TË PAVLEFSHËM', 'OFFLINE_BANK_ASSIGNMENT_MISMATCH', { assignment });
      }
      storage.setAssigned?.(id, assignment.code);
      const proof = writeProof({ pin: realPin, oid: id, code: assignment.code, verdict: assignment, source: 'OFFLINE_BANK' });
      log('allocator:reserved_offline_bank', { pin: realPin, oid: id, code: assignment.code });
      return { code: assignment.code, reused: false, source: 'OFFLINE_BANK', assignment, proof };
    }

    if (!db.reserveOne) throw makeError('RPC ZYRTAR I ALLOKIMIT MUNGON', 'PRANIMI_ALLOCATOR_RPC_REQUIRED');
    const assignment = normalizeAssignment(
      await db.reserveOne({ pin: realPin, oid: id, meaningful }),
      { pin: realPin, oid: id, status: 'reserved' },
    );
  ...
  async function getOrAllocateForDraft(args = {}) {
    ...
    const promise = getOrAllocateInternal({ ...args, pin, oid }).finally(() => {
  ...
  async function adoptAndVerifyForDraft({ pin, oid, code, meaningful = false } = {}) {
    ...
    if (verdict.displayable) {
      return {
        code: candidate,
        reused: true,
        source: verdict.proof?.offline_bank ? 'OFFLINE_BANK_ASSIGNMENT' : (verdict.offline ? 'RESTORED_OFFLINE_PROOF' : 'RESTORED_DB_VERIFIED'),
        verdict,
      };
    }
  ...
  async function renewForDraft({ pin, oid, code, meaningful = false } = {}) {
    ...
    const existingProof = getAssignmentProof(id);
    if (existingProof?.offline_bank) {
      const proof = proofMatches(existingProof, { pin: realPin, oid: id, code: candidate });
      return { ok: proof.ok, offline: !(await onlineNow()), offlineBank: true, reason: proof.reason, code: candidate };
    }
    ...
    const assignment = normalizeAssignment(verdict, { pin: realPin, oid: id, status: 'reserved' });
    if (!assignment || assignment.code !== candidate || assignment.reserved_by !== realPin || assignment.draft_session_id !== id || !assignment.lease_expires_at) {
      return { ok: false, reason: 'RENEW_IDENTITY_MISMATCH' };
    }
  ...
  async function consumeForDraft({ pin, oid, code, orderId, clientPhone = '' } = {}) {
    ...
    if (candidate == null || candidate !== assigned) {
      throw makeError('KODI FINAL NUK ËSHTË KODI I DRAFTIT', 'CONSUME_CODE_MISMATCH', { assignedCode: assigned, requestedCode: candidate });
    }
    ...
    const proof = getAssignmentProof(id);
    ...
    if (!(result === true || result?.ok === true)) {
      return { ok: false, code: candidate, retainBinding: true, reason: result?.reason || 'CONSUME_NOT_CONFIRMED', result };
    }
    return { ok: true, code: candidate, orderId: exactOrderId, result, offlineBank: proof?.offline_bank === true, awaitingAcknowledgement: true };
  }
  ...
  async function releaseForDraft({ pin, oid, code, reason = 'release_draft' } = {}) {
    ...
    const proof = getAssignmentProof(id);
    if (proof?.offline_bank && db.releaseOffline) {
      const result = await db.releaseOffline({ code: candidate, pin: realPin, oid: id, reason, proof });
      if (!(result === true || result?.ok === true)) {
        return { ok: false, offline: !(await onlineNow()), reason: result?.reason || 'OFFLINE_BANK_LOCAL_RELEASE_FAILED', retainBinding: true };
      }
      clearLocal(id);
      return { ok: true, code: candidate, offlineBank: true, result };
    }
  ...
  async function verifyExistingClientCode({ clientId, code, phone = '', name = '' } = {}) {
    ...
    return result?.ok === true
      ? { ...result, ok: true, code: candidate, clientId: id }
      : { ...result, ok: false, reason: result?.reason || 'EXISTING_CLIENT_VERIFY_REFUSED' };
  }
  ...
  async function finalizeExistingClientDraft({ pin, oid, finalCode, orderId } = {}) {
    ...
    const proof = getAssignmentProof(id);
    if (proof?.offline_bank) {
      return { ok: false, reason: 'OFFLINE_BANK_RELEASE_DEFERRED_TO_OUTBOX_SYNC', retainBinding: true, tempCode: assigned };
    }
    ...
    if (!(result === true || result?.ok === true)) {
      return { ok: false, reason: result?.reason || 'EXISTING_CLIENT_TEMP_RELEASE_NOT_CONFIRMED', retainBinding: true, tempCode: assigned };
    }
  ...
  function acknowledgeFinalizedDraft({ pin, oid, code, orderId = '' } = {}) {
    ...
    if (proof && (proof.pin !== realPin || proof.oid !== id || proof.code !== assigned)) {
      return { ok: false, reason: 'ACK_PROOF_IDENTITY_MISMATCH' };
    }
    clearLocal(id);
  ...
  return {
    version: PRANIMI_ALLOCATOR_VERSION,
    assignedCodeForDraft,
    getAssignmentProof,
    verifyAssignedCode,
    getOrAllocateForDraft,
    adoptAndVerifyForDraft,
    renewForDraft,
    consumeForDraft,
    releaseForDraft,
    verifyExistingClientCode,
    finalizeExistingClientDraft,
    acknowledgeFinalizedDraft,
  };
}
...
function browserStorage() {
  ...
  return {
    getAssigned: (oid) => {
      try { return ls ? normalizeCodeForAllocator(ls.getItem(`${ORDER_CODE_LS_PREFIX}${oid}`)) : null; }
      catch { return null; }
    },
    setAssigned: (oid, code) => {
      try { ls?.setItem(`${ORDER_CODE_LS_PREFIX}${oid}`, String(normalizeCodeForAllocator(code) || '')); } catch {}
    },
    clearAssigned: (oid) => { try { ls?.removeItem(`${ORDER_CODE_LS_PREFIX}${oid}`); } catch {} },
    getProof: (oid) => {
      try {
        const raw = ls?.getItem(`${ORDER_PROOF_LS_PREFIX}${oid}`) || ls?.getItem(`${LEGACY_PROOF_LS_PREFIX}${oid}`);
        return raw ? JSON.parse(raw) : null;
      } catch { return null; }
    },
    setProof: (oid, proof) => {
      try {
        ls?.setItem(`${ORDER_PROOF_LS_PREFIX}${oid}`, JSON.stringify(proof));
        ls?.removeItem(`${LEGACY_PROOF_LS_PREFIX}${oid}`);
      } catch {}
    },
    clearProof: (oid) => {
      try {
        ls?.removeItem(`${ORDER_PROOF_LS_PREFIX}${oid}`);
        ls?.removeItem(`${LEGACY_PROOF_LS_PREFIX}${oid}`);
      } catch {}
    },
  };
...
export function getPranimiCodeAllocator() {
  ...
  browserAllocator = createPranimiCodeAllocatorCore({
    ...
    db: {
      ...
      async reserveOne({ pin, oid }) {
        const m = await import('./baseCodes.js');
        return m.getOrAssignPranimiCodeInDb(oid, pin);
      },
      async reserveOffline({ pin, oid }) {
        const m = await import('./offlineCodeBank.js');
        return m.takeOfflineBaseCode({ pin, draftId: oid });
      },
      async releaseOffline({ pin, oid }) {
        const m = await import('./offlineCodeBank.js');
        return m.releaseOfflineCodeForDraft('base', pin, oid);
      },
      async verifyDisplayable({ code, pin, oid, meaningful }) {
        const m = await import('./baseCodes.js');
        return m.verifyPranimiCodeAssignmentInDb(code, { oid, pinOverride: pin, meaningful });
      },
      async renew({ code, pin, oid, meaningful }) {
        const m = await import('./baseCodes.js');
        return m.renewPranimiCodeAssignmentInDb(code, oid, { pinOverride: pin, meaningful });
      },
      async markUsed({ code, pin, oid, orderId, clientPhone }) {
        const m = await import('./baseCodes.js');
        return m.consumePranimiCodeAssignmentInDb(code, oid, { pinOverride: pin, orderId, clientPhone });
      },
      async release({ code, pin, oid, reason }) {
        const m = await import('./baseCodes.js');
        return m.releasePranimiCodeAssignmentInDb(code, oid, { pinOverride: pin, reason });
      },
      async releaseAfterExistingClient(args) {
        const m = await import('./baseCodes.js');
        return m.releasePranimiTempCodeAfterExistingClientSaveInDb(args);
      },
      async verifyExistingClient(args) {
        const m = await import('./baseCodes.js');
        return m.verifyExistingClientCodeForSave(args);
      },
    },
