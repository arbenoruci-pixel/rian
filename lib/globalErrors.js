...
function ensureChunkHealFallbackUi(message = 'Lidhja e dobët, provo sërish.') {
  ...
  try {
    ...
    wrap.style.inset = 'auto 14px 14px 14px';
    ...
    closeBtn.textContent = 'MBYLLE';
    closeBtn.style.padding = '12px 14px';
    closeBtn.style.borderRadius = '12px';
    closeBtn.style.border = '1px solid rgba(255,255,255,0.18)';
    closeBtn.style.background = 'transparent';
    closeBtn.style.color = '#fff';
    closeBtn.style.fontWeight = '800';
    closeBtn.style.letterSpacing = '0.6px';
    closeBtn.style.cursor = 'pointer';
    closeBtn.onclick = () => { try { wrap.remove(); } catch {} };

    const autoHide = () => { try { wrap.remove(); } catch {} };
    try { window.addEventListener('tepiha:first-ui-ready', autoHide, { once: true, passive: true }); } catch {}
    try { window.addEventListener('tepiha:route-ui-alive', autoHide, { once: true, passive: true }); } catch {}
