...
export const OFFLINE_RUNTIME_VERSION = 'offline-first-runtime-v2-no-banner';
...
async function batchMergeRowsOnce(storeName, rows = []) {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const fail = (error) => {
        if (failure) return;
        failure = error || new DOMException(`OFFLINE_SNAPSHOT_${storeName}_TX_ABORTED`, 'AbortError');
        // A failed read cannot establish whether a row contains pending local work.
        // Abort the whole merge; never attempt a fallback put in this transaction.
        try { tx.abort(); } catch { reject(failure); }
      };
      tx.oncomplete = () => failure ? reject(failure) : resolve(true);
      tx.onerror = (event) => fail(event?.target?.error || tx.error);
      tx.onabort = () => {
        failure ||= tx.error || new DOMException(`OFFLINE_SNAPSHOT_${storeName}_TX_ABORTED`, 'AbortError');
        reject(failure);
      };

      for (const remote of safeRows) {
        let getReq;
        try { getReq = store.get(remote.id); }
        ...
        getReq.onsuccess = () => {
          ...
          try {
            ...
            putReq.onerror = (event) => fail(event?.target?.error || putReq.error);
            written += 1;
          } catch (error) { fail(error); }
        };
        getReq.onerror = (event) => fail(event?.target?.error || getReq.error);
      }
    });
  } finally { db.close(); }
...
async function batchMergeRows(storeName, rows = []) {
  try { return await batchMergeRowsOnce(storeName, rows); }
  catch (error) {
    if (!['AbortError', 'TransactionInactiveError', 'InvalidStateError'].includes(error?.name)) throw error;
    // One fresh transaction after a suspension/abort. Re-read every row so a
    // local edit made between attempts still wins. Quota/data errors never loop.
    return batchMergeRowsOnce(storeName, rows);
  }
}
