...
export function createDispatchOutbox({ storage, getActorId, submit, online = () => true,
  ...
  function writeItem(item, notify = true) {
  ...
  function migrateLegacy() {
    ...
    for (const legacy of data.items) {
      ...
      if (!storage.getItem(itemKey(item.id))) writeItem(item, false);
    }
    // Do not erase a legacy write that arrived during migration from an old tab.
    if (storage.getItem(DISPATCH_OUTBOX_KEY) === raw) storage.removeItem(DISPATCH_OUTBOX_KEY);
  }
  ...
  function read() {
    ...
    for (const key of keys) {
      ...
      if (item.state === 'sent' && now() - item.updatedAt >= DAY) storage.removeItem(key);
  ...
  function list() {
    ...
    return clone(read().filter((item) => item.actorId === actorId));
  }
  ...
  function update(id, change) {
    ...
    return writeItem({ ...item, ...change, updatedAt: now() });
  }
