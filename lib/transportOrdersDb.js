...
import { preserveTransportMeasurements } from '@/lib/transport/measurementGuard';
import {
  ...
  stripImmutableTransportOrderPatchColumns,
} from '@/lib/transport/sanitize';
import { makeTransportSyncError } from '@/lib/transportCore/syncPolicy';
...
function sanitizeTransportOrderPayload(input, opts = {}) {
  const patchMode = opts?.patch === true;
  const sanitized = sanitizeSharedTransportOrderPayload(input, { patch: patchMode });
  return patchMode ? stripImmutableTransportOrderPatchColumns(sanitized) : sanitized;
}
...
export async function updateTransportOrderById(id, patch) {
  ...
  const guardedPatch = guardTransportAssignStatusOverwrite(localCurrent?.status, patch || {});
  const localPatch = { ...(guardedPatch || {}), updated_at: guardedPatch?.updated_at || nowIso() };
  let remotePatch = sanitizeTransportOrderPayload(localPatch, { patch: true });

  // TRANSPORT_MEASUREMENT_LOSS_GUARD_V1:CLIENT — status/cancel/payment actions
  // from an old phone snapshot must not replace already-saved measurements.
  // The database trigger is the atomic final boundary; this preflight also keeps
  // the local mirror/UI consistent with the protected server result immediately.
  if (isOnline() && remotePatch?.data && typeof remotePatch.data === 'object') {
    try {
      const { data: remoteCurrent, error: remoteCurrentError } = await supabase
        .from('transport_orders')
        .select('id,data')
        .eq('id', oid)
        .maybeSingle();
      if (remoteCurrentError) throw remoteCurrentError;
      if (remoteCurrent?.id) {
        const guardedData = preserveTransportMeasurements(remoteCurrent.data, remotePatch.data);
        if (guardedData.protected) {
          remotePatch = { ...remotePatch, data: guardedData.data };
        }
      }
    } catch {
      // Do not block the intended action when the preflight read is unavailable.
      // The DB trigger still protects the authoritative row atomically.
    }
  }
  ...
  try {
    ...
    if (error) throw error;
    if (!data) {
      throw makeTransportSyncError(`Transport order ${oid} nuk u gjet për PATCH`, {
    ...
    await saveLocalTransportOrder({ ...(localCurrent || { id: oid }), ...remotePatch }, { sync_state: 'pending' });
    await enqueueTransportPatch(oid, remotePatch);
    throw error;
  }
}
...
export async function updateClientCoords(tcode, coords) {
  ...
  if (error) throw error;
  if (!data) {
    throw makeTransportSyncError(`Transport client ${tc} nuk u gjet për PATCH`, {
...
export async function updateTransportOrdersByIds(ids, patch = {}) {
  ...
  await Promise.all(uniq.map(async (id) => {
    const existing = await getByKey('transport_orders', id);
    const guardedPatch = guardTransportAssignStatusOverwrite(existing?.status, patch || {});
    const localPayload = { ...(guardedPatch || {}), updated_at: guardedPatch?.updated_at || patch?.updated_at || nowIso() };
    const remotePayload = sanitizeTransportOrderPayload(localPayload, { patch: true });
    payloadById.set(id, remotePayload);
    await saveLocalTransportOrder({ ...(existing || { id }), ...remotePayload }, { sync_state: 'pending' });
  }));

  if (!isOnline()) {
    await Promise.all(uniq.map((id) => enqueueTransportPatch(id, payloadById.get(id) || {})));
    return { ok: true, count: uniq.length, offline: true };
  }

  try {
    await Promise.all(uniq.map(async (id) => {
      const remotePayload = payloadById.get(id) || {};
      const { data, error } = await supabase
        .from('transport_orders')
      ...
      if (error) throw error;
      if (!data) {
        throw makeTransportSyncError(`Transport order ${id} nuk u gjet për PATCH`, {
          code: 'TRANSPORT_ORDER_PATCH_TARGET_NOT_FOUND',
          status: 404,
        });
      }
...
export async function searchTransportClientCandidatesByOrders({ transportId = '', query = '', limit = 20, signal = null, timeoutMs = 7000, timeoutLabel = 'TRANSPORT_CLIENT_ORDER_SEARCH_TIMEOUT' } = {}) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    out.push({
      ...
      tcode: String(row?.data?.transport_client_tcode || row?.data?.client?.transport_client_tcode || meta.code || '').trim(),
      transport_client_tcode: String(row?.data?.transport_client_tcode || row?.data?.client?.transport_client_tcode || '').trim(),
      order_code: meta.code || '',
