...
export async function updateTransportOrderById(id, patch) {
  ...
  let remotePatch = sanitizeTransportOrderPayload(localPatch);
    ...
    try {
      ...
      if (remoteCurrent?.id) {
        ...
        if (guardedData.protected) {
          ...
          localPatch.data = guardedData.data;
        }
      }
    } catch {
...
export async function updateTransportOrdersByIds(ids, patch = {}) {
  ...
  await Promise.all(uniq.map(async (id) => {
    ...
    payloadById.set(id, localPayload);
    await saveLocalTransportOrder({ ...(existing || { id }), ...localPayload }, { sync_state: isOnline() ? 'synced' : 'pending' });
  }));

  if (!isOnline()) {
    await Promise.all(uniq.map((id) => enqueueTransportPatch(id, sanitizeTransportOrderPayload(payloadById.get(id) || patch || {}))));
    return { ok: true, count: uniq.length, offline: true };
  }

  try {
    await Promise.all(uniq.map(async (id) => {
      const remotePayload = sanitizeTransportOrderPayload(payloadById.get(id) || patch || {});
      const { error } = await supabase.from('transport_orders').update(remotePayload).eq('id', id);
      if (error) throw error;
    }));
