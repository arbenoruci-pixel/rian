...
let baseMasterCacheModulePromise = null;
let syncEngineModulePromise = null;
let transportSyncEngineModulePromise = null;
...
function loadBaseMasterCacheModule() {
  if (!baseMasterCacheModulePromise) {
    baseMasterCacheModulePromise = import('@/lib/baseMasterCache').catch((error) => {
      baseMasterCacheModulePromise = null;
      throw error;
    });
  }
  return baseMasterCacheModulePromise;
}
function loadSyncEngineModule() {
  if (!syncEngineModulePromise) {
    syncEngineModulePromise = import('@/lib/syncEngine').catch((error) => {
      syncEngineModulePromise = null;
      throw error;
    });
  }
  return syncEngineModulePromise;
}
function loadTransportSyncEngineModule() {
  if (!transportSyncEngineModulePromise) {
    transportSyncEngineModulePromise = import('@/lib/transportCore/syncEngine').catch((error) => {
      transportSyncEngineModulePromise = null;
      throw error;
    });
  }
  return transportSyncEngineModulePromise;
}
...
async function runSync(...args) {
  const mod = await loadSyncEngineModule();
...
async function runTransportSync(...args) {
  const mod = await loadTransportSyncEngineModule();
  return mod.runTransportSync(...args);
}
async function scheduleRunTransportSync(...args) {
  const mod = await loadTransportSyncEngineModule();
  return mod.scheduleRunTransportSync(...args);
}
function patchBaseMasterRowLazy(row) {
  void loadBaseMasterCacheModule()
    .then((mod) => {
      if (typeof mod?.patchBaseMasterRow === 'function') mod.patchBaseMasterRow(row);
    })
    .catch(() => {});
}
...
async function repairPendingBaseCreateOps(...args) {
  ...
  if (typeof mod?.repairPendingBaseCreateOps === 'function') return mod.repairPendingBaseCreateOps(...args);
  return { ok: false, skipped: 'syncRecovery-unavailable' };
}
...
async function saveTransportOrderLocalOnly(order = {}) {
  ...
  if (!String(next?.client_tcode || '').trim()) {
    const clientCode = String(
      next?.code_str ||
      next?.data?.code_str ||
      next?.data?.order_code ||
      next?.data?.order_tcode ||
      next?.data?.official_order_code ||
      ''
    ).trim();
  ...
  if (!String(next?.code_str || '').trim()) {
    ...
    if (orderCode) next.code_str = orderCode;
  }
...
async function refreshSnapshot() {
  try {
    ...
    const activeOpIds = new Set(snapshot.map((item) => String(item?.op_id || '')).filter(Boolean));
    const deadSnapshots = (Array.isArray(dead) ? dead : [])
      .filter((item) => !activeOpIds.has(String(item?.original_op_id || item?.op?.op_id || '')))
      .map((item) => ({
        ...mapOpToSnapshot(item?.op || {}),
        id: String(item?.dead_id || item?.original_op_id || item?.op?.op_id || rid()),
        op_id: String(item?.original_op_id || item?.op?.op_id || ''),
        dead_id: String(item?.dead_id || ''),
        status: String(item?.status || item?.op?.status || 'dead_letter'),
        lastError: item?.error || item?.op?.lastError || null,
        dead_letter: true,
      }));
    outboxSnapshotCache = [...snapshot, ...deadSnapshots];
    if (typeof window !== 'undefined') {
      try { window.localStorage.setItem(SNAPSHOT_KEY, JSON.stringify(outboxSnapshotCache)); } catch {}
    }
    broadcast();
...
export async function enqueueBaseOrder(payload = {}) {
  ...
  const opId = String(payload?.data?.outbox_op_id || payload?.outbox_op_id || rid());
  const baseData = (payload?.data && typeof payload.data === 'object') ? { ...payload.data } : {};
  const lifecycle = (baseData?.pranimi_code_lifecycle && typeof baseData.pranimi_code_lifecycle === 'object')
    ? { ...baseData.pranimi_code_lifecycle }
    : null;
  const nextData = {
    ...baseData,
    local_oid: oid,
    save_attempt_id: baseData?.save_attempt_id || lifecycle?.save_attempt_id || null,
    outbox_op_id: opId,
    _idem: idem,
    sync_safety: {
      ...((baseData?.sync_safety && typeof baseData.sync_safety === 'object') ? baseData.sync_safety : {}),
      local_oid: oid,
      save_attempt_id: baseData?.save_attempt_id || lifecycle?.save_attempt_id || null,
      outbox_op_id: opId,
      op_id: opId,
  ...
  if (lifecycle) {
    nextData.pranimi_code_lifecycle = {
      ...lifecycle,
      local_oid: lifecycle?.local_oid || oid,
      outbox_op_id: opId,
      op_id: opId,
    };
  }
  ...
  try { patchBaseMasterRowLazy(order); } catch {}
  ...
  const queuedOp = await enqueueOutboxItem({
    op_id: opId,
    op: 'insert_order',
    kind: 'base_order',
    uniqueValue: oid,
  ...
  return { ok: true, offline: true, id: oid, op_id: queuedOp?.op_id || opId, outbox_op_id: queuedOp?.op_id || opId, payload: outboxPayload };
}
