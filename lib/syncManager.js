...
function safeReadManagerJson(key, fallback, { purgeOnError = true } = {}) {
  ...
  try {
    ...
    try {
      console.error(`[syncManager] Corrupted localStorage at ${key}. Auto-healing...`, e);
      if (purgeOnError) localStorage.removeItem(key);
    } catch {}
    return fallback;
  }
}
...
function loadOutbox() {
  if (!isBrowser()) return [];
  const parsed = safeReadManagerJson(OUTBOX_KEY, []);
...
function acquireLock() {
  ...
  const now = Date.now();
  const lock = safeReadManagerJson(LOCK_KEY, null);
