...
export async function ensureMealDecisionBeforeHandoff({
  ...
  const rawPresetChoice = cleanText(presetChoice).toLowerCase();
...
export async function createExpenseEntry({
  ...
  const res = await arkaTransaction({
    ...
    expenseDateKey: cleanText(expenseDateKey),
    idempotencyKey: buildArkaIdempotencyKey(
      ARKA_ACTION.EXPENSE_REQUEST,
      [actorPin, 'EXPENSE', cleanText(expenseDateKey, arkaTodayBoundsIso(new Date()).dateKey), amt],
      { randomSuffix: true },
    ),
  });
  return res?.payment || res?.row || null;
