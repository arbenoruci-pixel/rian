...
export function createIncidentDelivery({ storage, fetcher, online = () => true, onConfirmed = () => {}, timeoutMs = 8000, storageEvents = globalThis.window }) {
  ...
  let scanned = false;
  // Scan once per boot, not on every incident and every idle retry. Other tabs
  // update only diagnostic entries via storage events; business keys are ignored.
  function scanStored() {
    if (scanned) return;
    scanned = true;
    storedEntries.clear();
    try {
      for (let i = 0; i < (storage?.length || 0); i++) {
        ...
        if (name?.startsWith(INCIDENT_PENDING_PREFIX)) storedEntries.set(name, storage.getItem(name) || '');
      }
    } catch {}
  }
  const storageChanged = event => {
    if (event.storageArea && event.storageArea !== storage) return;
    if (event.key === null) { scanned = false; return; }
    if (!event.key?.startsWith(INCIDENT_PENDING_PREFIX)) return;
    if (event.newValue === null) storedEntries.delete(event.key);
    else storedEntries.set(event.key, event.newValue);
  };
  storageEvents?.addEventListener?.('storage', storageChanged);
  try {
  ...
  function hasCapacity(key, raw) {
    ...
    scanStored();
    const entries = new Map(storedEntries);
    for (const [k, b] of pending) entries.set(INCIDENT_PENDING_PREFIX + k, JSON.stringify(b));
  ...
  function restore() {
    scanStored();
    for (const [name, raw] of storedEntries) {
      try {
  ...
  function acknowledge(key, body) {
    ...
    pending.delete(key);
    try { storage?.removeItem(INCIDENT_PENDING_PREFIX + key); storedEntries.delete(INCIDENT_PENDING_PREFIX + key); } catch {}
  ...
  function send(body) {
    ...
    if (!confirmed.has(key)) {
      ...
      try { storage?.setItem(INCIDENT_PENDING_PREFIX + key, raw); storedEntries.set(INCIDENT_PENDING_PREFIX + key, raw); } catch {}
    }
    return deliver(body);
  }
  ...
  return { send, flush, dispose() { storageEvents?.removeEventListener?.('storage', storageChanged); } };
}
