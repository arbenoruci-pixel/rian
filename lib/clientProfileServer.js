...
import { readClientFamily } from './clientFamilyServer.js';
...
function normalizeVisit(row, source, currentOrderId = '') {
  ...
  return {
    ...
    contactName: rowClientName(row),
    contactPhone: rowClientPhone(row),
    code: code || '—',
...
export async function buildClientProfile(bodyLike, { supabase, authUser } = {}) {
  ...
  const family = sourceClient?.id ? await readClientFamily(supabase, source, sourceClient.id) : null;
  const linkedMembers = family?.members?.length > 1 ? family.members : null;
  ...
  if (authoritativePhoneKey && !linkedMembers) {
    if (!baseClient) {
  ...
  let baseRows, transportRows;
  let familyDebt = null;
  if (linkedMembers) {
    const members = await Promise.all(linkedMembers.map(async m => cleanClient(await readClientById(supabase, m.source, m.clientId), m.source)));
    if (members.some(m => !m)) fail('CLIENT_PROFILE_CLIENT_NOT_FOUND', 404);
    baseClient ||= members.find(m => m.source === 'BASE') || null;
    transportClient ||= members.find(m => m.source === 'TRANSPORT') || null;
    const histories = await Promise.all(members.map(m => readOrdersForClient(supabase, m.source, m, m.phone, m.id === sourceClient.id && m.source === source ? anchor : null)));
    const baseMap = new Map(), transportMap = new Map();
    histories.forEach((rows, i) => { const map = members[i].source === 'BASE' ? baseMap : transportMap; rows.forEach(row => map.set(String(row.id), row)); });
    baseRows = [...baseMap.values()]; transportRows = [...transportMap.values()];
    // Legacy Base debt is keyed by phone, Transport debt by permanent T-code.
    // Read each balance bucket once even when several codes share a phone.
    const phones = [...new Map(members.filter(m => m.phoneKey).map(m => [m.phoneKey, m.phone])).values()];
    const debtParts = await Promise.all([
      ...phones.map(phone => readCarryDebt(supabase, null, null, phone)),
      ...members.filter(m => m.source === 'TRANSPORT' && m.code).map(m => readCarryDebt(supabase, null, { code: m.code }, '')),
    ]);
    familyDebt = debtParts.reduce((sum, part) => ({ base: money(sum.base + part.base), transport: money(sum.transport + part.transport), total: money(sum.total + part.total) }), { base: 0, transport: 0, total: 0 });
  } else {
  [baseRows, transportRows] = await Promise.all([
    readOrdersForClient(
      supabase,
  ...
  const [carryDebt, payments] = await Promise.all([
    familyDebt || readCarryDebt(supabase, baseClient, transportClient, authoritativePhone),
