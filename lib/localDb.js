...
const OFFLINE_BANK_META_PREFIX = 'offline_code_bank_v1:';
const OFFLINE_BANK_CACHE_PREFIX = 'tepiha_offline_code_bank_cache_v1:';
const OFFLINE_BANK_SUMMARY_KEY = 'tepiha_offline_code_bank_summary_v1';
const OFFLINE_BANK_TARGET = 10;
...
// iOS can suspend the page between a successful reservation RPC and the end of an
// IndexedDB transaction (for example when Airplane Mode is enabled immediately).
// Commit the tiny offline-code-bank mirror synchronously before the first await so
// the exact leased codes survive even if the app is backgrounded at that moment.
function precommitOfflineCodeBankMeta(storeName, value) {
  if (storeName !== 'meta') return false;
  if (typeof window === 'undefined' || !window.localStorage) return false;

  const key = String(value?.key || '').trim();
  const scope = String(value?.scope || '').trim().toLowerCase();
  const ownerId = String(value?.owner_id || '').trim();
  const deviceId = String(value?.device_id || '').trim();
  if (!key.startsWith(OFFLINE_BANK_META_PREFIX)) return false;
  if (!['base', 'transport'].includes(scope) || !ownerId || !deviceId) return false;

  const items = Array.isArray(value?.items) ? value.items : [];
  const available = items.filter((item) => String(item?.state || '').trim().toLowerCase() === 'available').length;
  const assigned = items.filter((item) => String(item?.state || '').trim().toLowerCase() === 'assigned').length;
  const cacheKey = `${OFFLINE_BANK_CACHE_PREFIX}${scope}:${ownerId}:${deviceId}`;

  try {
    window.localStorage.setItem(cacheKey, JSON.stringify(value));

    const previous = safeJsonParse(window.localStorage.getItem(OFFLINE_BANK_SUMMARY_KEY), {}) || {};
    const next = {
      ...previous,
      version: String(value?.version || previous?.version || 'offline-code-bank-v1'),
      device_id: deviceId,
      ...
      [scope]: {
        ...
        assigned,
        total: items.length,
        target: OFFLINE_BANK_TARGET,
        refreshed_at: String(value?.refreshed_at || ''),
        durable_precommit: true,
      },
    };
    window.localStorage.setItem(OFFLINE_BANK_SUMMARY_KEY, JSON.stringify(next));
...
export async function putValue(storeName, value) {
  precommitOfflineCodeBankMeta(storeName, value);
  const db = await openAppDb();
