...
export function markRouteUiAlive(label = '', pathOverride = '', extra = {}) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('tepiha:route-ui-alive', { detail: payload }));
  } catch {}
  try {
    window.dispatchEvent(new CustomEvent('tepiha:first-ui-ready', { detail: payload }));
  } catch {}
  try {
    document.dispatchEvent(new CustomEvent('tepiha:route-ui-alive', { detail: payload }));
  } catch {}
  try { logDebugEvent('route_ui_alive', { path, label: payload.label, resumeToken: Number(payload?.resumeToken || 0) || 0 }); } catch {}
  try { bootLog('route_ui_alive', { path, label: payload.label, resumeToken: Number(payload?.resumeToken || 0) || 0 }); } catch {}
}
...
export function useRouteAlive(label = '') {
  ...
  useEffect(() => {
    ...
    const commit = (reason = 'commit') => {
      ...
      try { markRouteUiAlive(label || reason, pathname, { resumeToken: now }); } catch {}
    };
