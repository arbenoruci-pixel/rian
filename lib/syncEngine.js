...
async function execOpRemote(op) {
  ...
  // Unifikim: përdorim vetëm 'insert_order' si op_type.
  // Pranojmë edhe legacy 'UPSERT_ORDER' por e mapojmë në 'insert_order'.
  const normalizedType = (op?.type === 'UPSERT_ORDER' || op?.type === 'upsert_order' || op?.type === 'insert_order')
    ? 'insert_order'
    : op?.type;
