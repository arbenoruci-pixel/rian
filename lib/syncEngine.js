...
async function execOpRemote(op) {
  ...
  try {
    pushGlobalError('sync/execOpRemote', e, { status: r.status, type: op?.type, body: j });
  } catch {}
  throw e;
}
...
export const runSync = async (opts = {}) => {
  ...
  try {
    ...
    try { await syncOfflineNow(); } catch (e) { try { pushGlobalError('sync/syncOfflineNow', e); } catch {} }
    try { await syncTransportDraftsNow(); } catch (e) { try { pushGlobalError('sync/syncTransportDraftsNow', e); } catch {} }
