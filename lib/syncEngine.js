...
async function finalizePranimiCodeLifecycleAfterBackgroundVerify(args = {}) {
  const mod = await loadSyncRecoveryModule();
  if (typeof mod?.finalizePranimiCodeLifecycleAfterRecovery !== 'function') {
    const error = new Error('PRANIMI_CODE_LIFECYCLE_RECOVERY_HELPER_MISSING');
    error.code = 'PRANIMI_CODE_LIFECYCLE_PENDING';
    error.retryable = true;
    throw error;
  }
  return mod.finalizePranimiCodeLifecycleAfterRecovery(args);
}
...
function getErrorText(error) {
  try {
    ...
    return JSON.stringify({
      ...
      status: error?.status || '',
      name: error?.name || '',
      responseError: error?.response?.error || error?.response?.message || '',
    });
  } catch {
    return String(error || '');
  }
...
function isDeviceAuthorizationError(error) {
  ...
  return status === 401
    || text.includes('DEVICE_NOT_APPROVED')
    || text.includes('DEVICE_LINKED_TO_OTHER_USER')
    || text.includes('AUTH_REQUIRED')
    || text.includes('ACTOR_SESSION_MISMATCH')
    || text.includes('ROLE_MISMATCH')
    || text.includes('PIN_RETIRED_USE_CURRENT_PIN')
...
function validateOpShape(op = {}) {
  ...
  if (!['insert_order', 'set_status', 'patch_order_data', 'update', 'gati_payment_delivery', 'base_ready_bonus_transition', 'arka_transaction', 'upload_storage'].includes(type)) return 'UNSUPPORTED_SYNC_OPERATION';
  if (type === 'upload_storage' && (!payload?.bucket || !payload?.path || payload?.data == null)) return 'INVALID_STORAGE_OPERATION';
  ...
  if (type === 'base_ready_bonus_transition') {
...
function stripNonSchemaCols(row, table = 'orders', opts = {}) {
...
async function updateByIdOrLocalOid(table, id, patch) {
  let clean = stripNonSchemaCols({ ...(patch || {}) }, table, { patch: true });
  ...
  const field = table === 'orders' && !isNumericDbId(id) ? 'local_oid' : 'id';
  let res = await supabase.from(table).update(clean).eq(field, id).select('id').maybeSingle();
  if (res.error) throw res.error;
  if (!res.error && res.data) return res.data;

  if (table === 'orders' && field === 'id') {
    res = await supabase.from(table).update(clean).eq('local_oid', id).select('id').maybeSingle();
    if (!res.error && res.data) return res.data;
  }

  if (res.error) throw res.error;
  // Zero affected rows are not an acknowledgement. Retain the operation so
  // a pending create can arrive before its update, including after an outage.
  throw Object.assign(new Error('SYNC_ORDER_NOT_FOUND'), { code: 'SYNC_ORDER_NOT_FOUND', retryable: true });
}
...
async function verifyInsertedBaseOrderInDb(table = 'orders', row = {}, clean = {}, localId = '', serverRow = null) {
  ...
  async function queryOne(via, apply) {
    try {
      ...
      const { data, error } = await query.maybeSingle();
      if (error) {
        queryErrors.push({ via, error });
        return null;
      }
      if (data) return { ok: true, row: data, via };
    } catch (error) {
      queryErrors.push({ via, error });
    }
    return null;
  ...
  return {
    ...
    error: queryErrors[0]?.error || null,
    errors: queryErrors,
    row: null,
    via: '',
    local_oid: localOid,
...
function buildDbVerifiedBaseData({ row = {}, clean = {}, remoteRow = {}, verified = {}, op = {} } = {}) {
  ...
  const lifecycleAck = verified?.codeLifecycle && typeof verified.codeLifecycle === 'object'
    ? verified.codeLifecycle
    : null;
  const mergedData = {
    ...remoteData,
    ...rowData,
  ...
  nextData.pranimi_code_lifecycle = {
    ...
    db_verified_at: at,
    server_id: serverId,
    ...(lifecycleAck?.tracked ? {
      code_lifecycle_finalize_state: 'ACKNOWLEDGED',
      code_lifecycle_finalize_source: String(lifecycleAck?.source || 'syncEngine'),
      code_lifecycle_finalize_reason: String(lifecycleAck?.reason || 'PRANIMI_CODE_LIFECYCLE_ACKNOWLEDGED'),
      code_lifecycle_acknowledged_at: String(lifecycleAck?.acknowledgedAt || at),
      code_lifecycle_finalize_order_id: String(lifecycleAck?.orderId || serverId),
      code_lifecycle_finalize_local_oid: String(lifecycleAck?.draftId || localOid),
      code_lifecycle_finalize_code: Number(lifecycleAck?.code || clean?.code || row?.code || remoteRow?.code || 0),
      code_lifecycle_finalize_mode: String(lifecycleAck?.mode || cleanData?.pranimi_code_lifecycle?.code_lifecycle_mode || rowData?.pranimi_code_lifecycle?.code_lifecycle_mode || ''),
      ...(lifecycleAck?.tempCode ? { code_lifecycle_temp_code: Number(lifecycleAck.tempCode) } : {}),
    } : {}),
  };
  return nextData;
}

function buildDbVerifyStateUpdateError(error, reason = 'DB_VERIFY_STATE_UPDATE_FAILED', details = {}) {
  const e = new Error(String(reason || 'DB_VERIFY_STATE_UPDATE_FAILED'));
  e.code = 'DB_VERIFY_STATE_UPDATE_FAILED';
  e.reason = String(reason || 'DB_VERIFY_STATE_UPDATE_FAILED');
  e.details = Object.keys(details || {}).length > 0
    ? details
...
async function markRemoteBaseOrderDbVerified(table = 'orders', remoteRow = {}, row = {}, clean = {}, verified = {}, op = {}) {
  ...
  if (!serverId || !/^\d+$/.test(serverId)) {
    throw buildDbVerifyStateUpdateError(null, 'DB_VERIFY_STATE_SERVER_ID_MISSING', { serverId });
  }
  const expectedLocalOid = String(
    remoteRow?.local_oid ||
    clean?.local_oid ||
    row?.local_oid ||
    clean?.data?.local_oid ||
    row?.data?.local_oid ||
    ''
  ).trim();
  if (!expectedLocalOid) {
    throw buildDbVerifyStateUpdateError(null, 'DB_VERIFY_STATE_LOCAL_OID_MISSING', { serverId });
  }
  ...
  if (!data) {
    throw buildDbVerifyStateUpdateError(null, 'DB_VERIFY_STATE_UPDATE_ZERO_ROWS', {
      serverId,
      expectedLocalOid,
    });
  }
  ...
  if (returnedId !== serverId || returnedLocalOid !== expectedLocalOid) {
    throw buildDbVerifyStateUpdateError(null, 'DB_VERIFY_STATE_UPDATE_IDENTITY_MISMATCH', {
      serverId,
      returnedId,
      expectedLocalOid,
      returnedLocalOid,
...
async function processOp(op) {
  ...
  if (type === 'insert_order') {
    ...
    if (existingRemote) {
      ...
      const existingCodeLifecycle = table === 'orders'
        ? await finalizePranimiCodeLifecycleAfterBackgroundVerify({
            localRow: {
              ...row,
              ...clean,
              data: {
            ...
            source: 'syncEngine_existing_remote',
          })
        : null;
      const existingVerify = {
        ...
        row: existingRemote,
        via: 'existing_remote',
        codeLifecycle: existingCodeLifecycle,
      };
      ...
      await saveOrderLocal({
        ...
        table,
        data: verifiedExistingRemote?.data || buildDbVerifiedBaseData({ row, clean, remoteRow: verifiedExistingRemote, verified: existingVerify, op }),
        _local: false,
        _synced: true,
        _syncPending: false,
        _syncing: false,
    ...
    const verifiedCodeLifecycle = table === 'orders'
      ? await finalizePranimiCodeLifecycleAfterBackgroundVerify({
          localRow: {
            ...row,
            ...clean,
          ...
          remoteRow: verified?.row || serverRow,
          source: 'syncEngine_post_upsert_verify',
        })
      : null;
    const verifiedWithLifecycle = {
      ...(verified || {}),
      codeLifecycle: verifiedCodeLifecycle,
    };
    let verifiedRow = (table === 'orders' && verifiedWithLifecycle?.row) ? verifiedWithLifecycle.row : serverRow;
    if (table === 'orders') {
      verifiedRow = await markRemoteBaseOrderDbVerified(table, verifiedRow, row, clean, verifiedWithLifecycle, op);
    }
    ...
    await saveOrderLocal({
      ...
      ...clean,
      ...verifiedRow,
      data: verifiedRow?.data || buildDbVerifiedBaseData({ row, clean, remoteRow: verifiedRow, verified: verifiedWithLifecycle, op }),
  ...
  if (type === 'patch_order_data' || type === 'update') {
    ...
    const patch = stripNonSchemaCols({ ...(rawPatch || {}) }, table, { patch: true });
  ...
  if (type === 'gati_payment_delivery') {
    // GATI_OFFLINE_RELIABILITY_V4:SYNC — retrying this operation is safe because the ARKA
    ...
    // The service-role RPC above owns both the money write and the delivery
    // transition atomically (including partial DELIVERY_TO_DORZIM payments).
    // Never replay the queued client snapshot over its canonical result.
    if (!isDeliveredBaseStatus(paidOrder?.status || paidOrder?.data?.status)) {
      const error = new Error('GATI_OFFLINE_ORDER_NOT_DELIVERED');
      error.code = 'GATI_OFFLINE_ORDER_NOT_DELIVERED';
      error.retryable = true;
      throw error;
    }
    const paidData = readNestedObject(paidOrder?.data);
    await saveOrderLocal({
      ...paidOrder,
      id: String(paidOrder.id),
      status: paidOrder.status || paidData.status || 'dorzim',
      data: paidData,
      _local: false,
      _synced: true,
      _syncPending: false,
      ...
      pending_ops: 0,
      dirty: false,
      updated_at: paidOrder?.updated_at || paidData?.updated_at || nowIso(),
    });
    return true;
  }

  if (type === 'base_ready_bonus_transition') {
    // BASE_READY_48H_BONUS_V1:SYNC — DB owns the 48h decision and creates the ARKA bonus atomically.
    const rpcArgs = {
  ...
  if (type === 'upload_storage') {
    ...
    if (!bucket || !path || dataValue == null) throw new Error('INVALID_STORAGE_OPERATION');
  ...
  running = (async () => {
    ...
    for (const op of ops) {
      ...
      try {
        ...
        const deviceAuthorizationBlocked = isDeviceAuthorizationError(e);
        ...
        if (
          lifecycleSafeRetryCode === 'SYNC_ORDER_NOT_FOUND' ||
          lifecycleSafeRetryCode === 'DB_VERIFY_FAILED' ||
          lifecycleSafeRetryCode === 'DB_VERIFY_STATE_UPDATE_FAILED' ||
          lifecycleSafeRetryCode === 'PRANIMI_CODE_LIFECYCLE_PENDING'
        ) {
          await markInsertMirrorState(op, {
            _syncing: false,
            _syncPending: true,
            _syncFailed: true,
            _syncError: lifecycleSafeRetryCode,
          });
          const nextOp = buildRetriedOp(op, e, { networkLike: false });
          await pushOp(nextOp);
          syncDebugLog(lifecycleSafeRetryCode === 'PRANIMI_CODE_LIFECYCLE_PENDING' ? 'code_lifecycle_retry_queued' : 'db_verify_retry_queued', {
            op_id: String(nextOp?.op_id || op?.op_id || ''),
            local_oid: String(getPayload(nextOp)?.local_oid || getPayload(nextOp)?.data?.local_oid || nextOp?.id || ''),
            save_attempt_id: String(
              getPayload(nextOp)?.data?.pranimi_code_lifecycle?.save_attempt_id ||
              getPayload(nextOp)?.data?.save_attempt_id ||
              ''
            ),
            retryAt: Number(nextOp?.nextRetryAt || 0),
            status: String(nextOp?.status || 'pending'),
            reason: String(e?.reason || e?.message || lifecycleSafeRetryCode),
          });
          if (String(op?.type || op?.op || '').trim() === 'insert_order') {
            try { rememberBaseCreateRecovery(getPayload(op), { status: 'verify_wait', source: 'syncEngine', note: lifecycleSafeRetryCode }); } catch {}
          }
          continue;
        }

        if (deviceAuthorizationBlocked) {
          // Never dead-letter an idempotent money command merely because the
          // browser cookie/device approval needs repair. Keep it paused for a
          // later approved session; the server guard remains fail-closed.
          await markInsertMirrorState(op, {
            _syncing: false,
            _syncPending: true,
            _syncFailed: false,
            _syncError: String(e?.message || 'DEVICE_NOT_APPROVED'),
          });
          const nextOp = buildRetriedOp(op, e, { networkLike: false });
          await pushOp(nextOp);
          syncDebugLog('device_session_retry_queued', {
            op_id: String(nextOp?.op_id || op?.op_id || ''),
            type: String(op?.type || op?.op || ''),
            retryAt: Number(nextOp?.nextRetryAt || 0),
            status: String(nextOp?.status || 'pending'),
          ...
          try {
            window.dispatchEvent(new CustomEvent('tepiha:device-session-required', {
              detail: { error: String(e?.message || 'DEVICE_NOT_APPROVED') },
            }));
          } catch {}
          continue;
        }
