...
function buildDbVerifiedBaseData({ row = {}, clean = {}, remoteRow = {}, verified = {}, op = {} } = {}) {
  ...
  const truthStatus = getDbTruthStatus(remoteRow) || String(clean?.status || row?.status || nextData?.status || '').trim();
  if (truthStatus) nextData.status = truthStatus;
  nextData.pranimi_code_lifecycle = {
    ...(((remoteData?.pranimi_code_lifecycle && typeof remoteData.pranimi_code_lifecycle === 'object') ? remoteData.pranimi_code_lifecycle : {})),
    ...(((rowData?.pranimi_code_lifecycle && typeof rowData.pranimi_code_lifecycle === 'object') ? rowData.pranimi_code_lifecycle : {})),
    ...(((cleanData?.pranimi_code_lifecycle && typeof cleanData.pranimi_code_lifecycle === 'object') ? cleanData.pranimi_code_lifecycle : {})),
    local_oid: localOid || '',
