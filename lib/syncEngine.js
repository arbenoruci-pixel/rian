...
function shouldRetryWithoutDeliveredAt(message = "") {
  const msg = String(message || "").toLowerCase();
  return msg.includes("delivered_at") || msg.includes("column delivered_at") || msg.includes("snake_case");
}
...
async function execOpRemote(op) {
  ...
  try {
    ...
    const msg = e?.message || "";
    if (!shouldRetryWithoutDeliveredAt(msg)) {
      try {
    ...
    const retryBody = stripDeliveredAtDeep(cloneJson(body));

    try {
      return await postOfflineSync(retryBody);
    } catch (retryErr) {
      try {
        pushGlobalError("sync/execOpRemote_retry_without_delivered_at", retryErr, {
          status: retryErr?.responseStatus,
          type: op?.type,
...
export const runSync = async (opts = {}) => {
  ...
  try {
    ...
    try {
      pushGlobalError("sync/runSync_outer", err, { sent, failed });
    } catch {}
