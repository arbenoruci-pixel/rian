...
import {
  ...
  isPranimiFinalOrderStatus,
  isPranimiFinalOrderRow,
  isPranimiDraftOrderStatus,
  isPranimiDraftFlaggedData,
} from './pranimiOrderLifecycle.js';
...
function sanitizeTransportOrdersPayload(table, payload, opts = {}) {
  if (table !== 'transport_orders') return cleanPayload(payload);
  return sanitizeTransportOrderPayload(payload, { patch: opts?.patch === true });
}
...
async function normalizeBaseOrderWritePayload(table, id, patch = {}, { insert = false } = {}) {
  ...
  const requestedStatus = String((hasStatus ? out?.status : out?.data?.status) || '').trim();
  const incomingDraftWrite = isPranimiDraftOrderStatus(requestedStatus)
    || (hasData && isPranimiDraftFlaggedData(out.data));

  // V476: a late/in-flight PRANIMI draft autosave must never downgrade a
  // finalized order back to pranim/incomplete. This is the generic client-side
  // write barrier; the service draft API also uses a DB compare-and-swap guard.
  if (isPranimiFinalOrderRow(current || {}) && incomingDraftWrite) {
    const error = new Error('PRANIMI_FINAL_ORDER_DRAFT_DOWNGRADE_BLOCKED');
    error.code = 'PRANIMI_FINAL_ORDER_DRAFT_DOWNGRADE_BLOCKED';
    error.orderId = String(current?.id || id || '');
    throw error;
  }
...
export async function listOrderRecords(tableInput, options = {}) {
  ...
  if (options?.fetchAll) {
    ...
    throw new Error(`ORDER_FETCH_ALL_LIMIT_REACHED_${table}_${maxRows}`);
  }
...
export async function updateOrderRecord(tableInput, id, patch = {}, options = {}) {
  const table = resolveTable(tableInput);
  ...
  const row = sanitizeTransportOrdersPayload(
    table,
    { ...(normalizedPatch || {}), updated_at: normalizedPatch?.updated_at || new Date().toISOString() },
    { patch: true },
  );
  let request = supabase.from(table).update(row).eq('id', id);
  // Compare-and-set prevents a slow status action overwriting another worker's change.
  if (options.expectedUpdatedAt) request = request.eq('updated_at', options.expectedUpdatedAt);
  const { data, error } = await request.select('id');
  if (error) throw error;
  if (!Array.isArray(data) || !data.some(item => String(item?.id) === String(id))) {
    const code = options.expectedUpdatedAt ? 'ORDER_STATUS_CONFLICT' : 'ORDER_UPDATE_NOT_APPLIED';
...
export async function createOrderRecord(tableInput, row = {}) {
  ...
  if (table === 'transport_orders') {
    ...
    return { ok: true, table, id: result?.data?.id ?? payload?.id ?? null, data: result?.data || null };
  }
...
export async function upsertOrderRecord(tableInput, row = {}, options = {}) {
  ...
  if (table === 'transport_orders') {
    const result = await insertTransportOrder(payload);
    if (!result?.ok) throw new Error(result?.error || 'TRANSPORT_ORDER_UPSERT_FAILED');
    return { ok: true, table, id: result?.data?.id ?? payload?.id ?? null, data: result?.data || null };
  }
...
export async function transitionOrderStatus(tableInput, id, status, extraPatch = {}) {
  ...
  if (!current?.id) throw new Error('ORDER_NOT_FOUND');
  ...
  return updateOrderRecord(table, id, patch, { expectedUpdatedAt: current.updated_at });
}
  ...
  if (table === 'transport_orders') {
    ...
    try {
      const { data, error } = await supabase
        .from(table)
        .select(select)
        .eq('data->>legacy_order_code', transportCode)
