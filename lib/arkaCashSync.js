...
export async function recordCashMove(payload={}){
  ...
  // Prano edhe camelCase nga faqe te ndryshme
  const created_by_pin = payload.created_by_pin ?? payload.createdByPin ?? null;
  const created_by_name = payload.created_by_name ?? payload.createdBy ?? payload.user ?? null;
  // Ne UI duam me e pa emrin; PIN-i ruhet si kolone e ndare (created_by_pin)
  const created_by = (created_by_name && String(created_by_name).trim())
    ? String(created_by_name).trim()
    : (created_by_pin ? `PIN_${String(created_by_pin).trim()}` : 'SYSTEM');
  ...
  try {
    ...
    if (!cycle?.id || cycle.closed_at || hasHanded) {
      return await createPendingCashPayment({ ...payload, external_id, amount: amt, created_by_pin, created_by_name });
    }
...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_pin, approved_by_name, approved_by_role }){
  ...
  try {
    ...
    // DB update me sa me shume audit info qe te jete e mundur. Nese schema s'i ka kolonat, bie ne update minimal.
    try {
      await supabase
        .from('arka_pending_payments')
        .update({
      ...
      await supabase
        .from('arka_pending_payments')
        .update({ status: 'APPLIED', applied_at: now, applied_cycle_id: cycle_id })
        .eq('external_id', eid);
    }
