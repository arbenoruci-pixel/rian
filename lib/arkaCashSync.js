...
// NEW: Nëse pagesa ka ndodhur para hapjes së ciklit aktual,
// trajtohet si pagesë kur ARKA ka qenë e MBYLLUR → shkon në PENDING.
const occurred_at =
  payload.occurred_at ||
  payload.occurredAt ||
  payload.created_at ||
...
try {
  if (occurred_at && cycle?.opened_at) {
    const occ = new Date(occurred_at).getTime();
    const opn = new Date(cycle.opened_at).getTime();
    if (Number.isFinite(occ) && Number.isFinite(opn) && occ < opn) {
