...
export async function createPendingCashPayment(payload={}){
  ...
  const external_id = payload.externalId || payload.external_id || `pend_${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const isTransport = /^T\d+/i.test(String(payload.order_code || payload.orderCode || payload.code || '').trim());
...
export async function recordCashMove(payload={}){
  ...
  try {
    ...
    const pendingRes = await createPendingCashPayment({ ...payload, external_id, amount: amt, status: isTransport ? 'COLLECTED' : (payload.status || 'PENDING'), type: isTransport ? 'TRANSPORT' : (payload.type || 'IN') });
