...
// ---- schema tolerant helpers (PIN fields optional) ----
function _isSchemaColumnError(err) {
  const m = String(err?.message || '').toLowerCase();
  return m.includes('schema cache') || (m.includes('could not find') && m.includes('column')) || m.includes('does not exist');
}

async function _withSchemaFallback(fnRich, fnLite) {
  const r1 = await fnRich();
  if (!r1?.error) return r1;
  if (_isSchemaColumnError(r1.error)) {
    return await fnLite();
  }
  return r1;
}
...
// ---- PIN-attached pending handlers ----
export async function applyPendingPaymentToCycle({ pending, cycleId, actor }) {
  try {
    if (!pending?.external_id) return { ok: false, error: 'MISSING_EXTERNAL_ID' };
    if (!cycleId) return { ok: false, error: 'MISSING_CYCLE_ID' };

    const pin = actor?.pin ? String(actor.pin) : null;
    const name = actor?.name ? String(actor.name) : null;
    const role = actor?.role ? String(actor.role) : null;

    const moveRich = () => supabase
    ...
    const ins = await _withSchemaFallback(moveRich, moveLite);
    if (ins.error) return { ok: false, error: ins.error.message || String(ins.error) };

    const updRich = () => supabase
      .from('arka_pending_payments')
      .update({
        status: 'APPLIED',
    ...
    const upd = await _withSchemaFallback(updRich, updLite);
    if (upd.error) return { ok: false, error: upd.error.message || String(upd.error) };

    return { ok: true, move: ins.data };
  } catch (e) {
    return { ok: false, error: e?.message || String(e) };
  }
}

export async function rejectPendingPayment({ pending, actor, reject_note }) {
  try {
    if (!pending?.external_id) return { ok: false, error: 'MISSING_EXTERNAL_ID' };
