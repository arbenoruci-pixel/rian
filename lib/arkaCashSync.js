...
export async function applyPendingPaymentToCycle({ pending, pendingId, cycle_id, cycleId, approved_by_name, actor }){
  let p = pending;
  const cid = cycle_id || cycleId;
  // Support wrapper API: applyPendingPaymentToCycle({pendingId, cycleId, actor})
  if (!p && pendingId) {
  ...
  if (!p || !cid) return { ok: false };
  const eid = p.external_id || p.externalId;
  const a = actor || {};
  const actorPin = a.pin ?? p?.approved_by_pin ?? p?.created_by_pin ?? null;
  const actorName = a.name ?? approved_by_name ?? p?.approved_by_name ?? p?.created_by_name ?? null;
  const actorRole = a.role ?? p?.approved_by_role ?? p?.created_by_role ?? null;
  ...
  try {
    ...
    await dbAddCycleMove({
      ...
      note: p.note || 'Pending Payment',
      source: 'ORDER_PAY',
      // actor (shfaqet si emër/pin në DB nëse ekzistojnë kolonat)
      created_by: actorName || 'SYSTEM',
      created_by_name: actorName || null,
      created_by_pin: actorPin || null,
      created_by_role: actorRole || null,
      external_id: eid 
    });

    // DB: skema mund t'i ketë ose jo kolonat approved_* / applied_*.
    // Provo me audit të plotë; nëse bie, bie në update minimal.
    const now = new Date().toISOString();
    const full = {
      status: 'APPLIED',
      applied_cycle_id: cid,
      // prefero approved_* (si në DB tuaj)
      approved_at: now,
      approved_by_pin: actorPin,
      approved_by_name: actorName,
      approved_by_role: actorRole,
      // mbaj edhe applied_at (nëse ekziston)
      applied_at: now,
    };
    let u1 = await supabase.from('arka_pending_payments').update(full).eq('external_id', eid);
    if (u1.error) {
      // provo pa kolonat që mund të mos ekzistojnë
      const minimal = { status: 'APPLIED', applied_cycle_id: cid };
      const u2 = await supabase.from('arka_pending_payments').update(minimal).eq('external_id', eid);
