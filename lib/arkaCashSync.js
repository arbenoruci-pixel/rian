...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name }){
  ...
  try {
    ...
    await supabase.from('arka_pending_payments').update({ status: 'APPLIED', applied_at: new Date().toISOString(), applied_cycle_id: cycle_id }).eq('external_id', eid);
...
export async function rejectPendingPayment({ pending, rejected_by_pin, rejected_by_name, rejected_by_role, reject_note } = {}){
  ...
  const eid = pending.external_id || pending.externalId;
  try{
    await supabase.from('arka_pending_payments').update({
      status:'REJECTED',
    ...
    try{
      ...
      if(Array.isArray(data)){
        for(const row of data){
          ...
          const pend=cur.pay?.pendingCash;
          if(!Array.isArray(pend)) continue;
          const i=pend.findIndex(x=>String(x.external_id||x.externalId||'')===String(eid));
          if(i>-1){
            pend[i].status='REJECTED';
            pend[i].rejected_at=new Date().toISOString();
            await supabase.from('orders').update({data:cur}).eq('id',row.id);
            break;
          }
        }
    ...
    try{
      const next = lsRead().map(x => (x.external_id===eid||x.externalId===eid)?{...x,status:'REJECTED'}:x);
      lsWrite(next);
    }catch{}
