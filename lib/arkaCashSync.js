// ARKA CLEAN LOCK: decide pending vs direct based on when payment happened
function shouldGoPending(payment, cycle) {
  try {
    const pAt = payment?.created_at ? new Date(payment.created_at) : null;
    const cAt = cycle?.opened_at ? new Date(cycle.opened_at) : null;
    if (!cycle?.id) return true;
    if (!pAt || !cAt) return false; // can't compare -> keep existing behavior
    return pAt.getTime() < cAt.getTime();
  } catch {
    return false;
  }
}
...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name, approved_by_pin, approved_by_role }){
  ...
  try {
    ...
    if (!up.data || up.data.length === 0) throw new Error('RLS_BLOCKED_UPDATE');
...
export async function rejectPendingPayment({ pending, rejected_by_pin, rejected_by_name, rejected_by_role, reject_note } = {}) {
  ...
  try {
    ...
    try {
      await supabase.from('arka_pending_payments').update({ status: 'REJECTED', type: 'DEBT', rejected_at: now }).eq('external_id', eid);
    } catch {}
  }
  ...
  try {
    const next = lsRead().map(x => (x.external_id === eid || x.externalId === eid)
      ? { ...x, status: 'REJECTED', type: 'DEBT', rejected_at: now, rejected_by_pin: rejected_by_pin ?? null, rejected_by_name: rejected_by_name ?? null, rejected_by_role: rejected_by_role ?? null, reject_note: reject_note ?? null }
      : x
    );
    lsWrite(next);
  } catch {}
...
export async function upsertPendingFromPayment(supabase, payment, actor) {
  const amount =
    Number(payment?.amount ?? payment?.paid_amount ?? payment?.total_paid ?? payment?.cash ?? 0) || 0;

  const external_id =
    payment?.external_id ||
    payment?.payment_id ||
    (payment?.order_id ? `order_${payment.order_id}_${payment?.created_at || Date.now()}` : `pay_${Date.now()}`);
  ...
  const payload = {
    ...
    order_code: payment?.order_code ?? payment?.code ?? null,
    client_name: payment?.client_name ?? payment?.customer_name ?? payment?.name ?? null,
    stage: payment?.stage ?? payment?.status ?? null,
    created_by_pin: actor?.pin ? String(actor.pin) : null,
    created_by_name: actor?.name ?? null,
    created_by_role: actor?.role ?? null,
    external_id,
    type: 'IN',
    note: payment?.note ?? payment?.memo ?? null,
  };

  // If already exists, do nothing (keep first record)
  const { data, error } = await supabase
    .from('arka_pending_payments')
    .insert(payload, { onConflict: 'external_id', ignoreDuplicates: true })
    .select()
    .maybeSingle();
