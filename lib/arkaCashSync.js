...
async function updateOrderPendingStatus(external_id, nextStatus, extra = {}, order_id = null) {
  if (!external_id) return;
  ...
  try {
    ...
    const { data } = await supabase.from('orders').select('id,data').order('created_at', { ascending: false }).limit(120);
...
async function listRowsByStatuses(statuses = [], limit = 500) {
  const wanted = new Set((statuses || []).map((x) => String(x || '').toUpperCase()));
  const hardLimit = Math.max(1, Math.min(Number(limit || 500), 1000));
  const rows = [];
  const seen = new Set();
  ...
  try {
    const { data } = await supabase
      .from('arka_pending_payments')
      .select('*')
      .in('status', Array.from(wanted))
      .order('created_at', { ascending: false })
      .limit(hardLimit);
    if (Array.isArray(data)) {
      for (const row of data) {
        const eid = String(row?.external_id || '').trim();
    ...
    try {
      ...
      if (Array.isArray(data)) {
        for (const order of data) {
          const pends = order?.data?.pay?.pendingCash || [];
          for (const row of pends) {
            const st = String(row?.status || '').toUpperCase();
            const eid = String(row?.external_id || row?.externalId || '').trim();
            if (!wanted.has(st) || !eid || seen.has(eid)) continue;
            seen.add(eid);
            rows.push({ ...row, external_id: eid, order_id: row?.order_id || order?.id || null });
  ...
  return { ok: true, items: rows.slice(0, hardLimit) };
}

export async function listDispatchHandoffs(limit = 500) {
  const res = await listRowsByStatuses(['HANDED'], limit);
  const map = new Map();
  ...
  return {
    ok: true,
    items: Array.from(map.values()).sort((a, b) => b.total - a.total || a.name.localeCompare(b.name)),
  };
}

export async function listAcceptedCashPayments(limit = 500) {
  return listRowsByStatuses(['ACCEPTED'], limit);
}

export async function confirmHandoffByDispatch(actorPin, receiver = {}) {
  const pin = String(actorPin || '').trim();
  if (!pin) return { ok: false, error: 'MISSING_PIN' };

  const rowsRes = await listRowsByStatuses(['HANDED'], 1000);
  const rows = (rowsRes.items || []).filter((row) => String(row?.created_by_pin || row?.pin || '').trim() === pin);
  const total = rows.reduce((sum, row) => sum + (Number(row?.amount || 0) || 0), 0);
  if (!rows.length || total <= 0) return { ok: true, count: 0, total: 0, items: [] };

  const now = new Date().toISOString();
  const receiverPin = receiver?.pin || null;
  const receiverName = receiver?.name || null;
  const receiverRole = receiver?.role || null;

  for (const row of rows) {
    const eid = row.external_id || row.externalId;
    if (!eid) continue;
    ...
    try {
      await supabase
        ...
        .update({
          status: 'ACCEPTED',
          accepted_at: now,
          accepted_by_pin: receiverPin,
          accepted_by_name: receiverName,
          accepted_by_role: receiverRole,
        })
        .eq('external_id', eid)
        .eq('status', 'HANDED');
    } catch {}

    await updateOrderPendingStatus(eid, 'ACCEPTED', {
      accepted_at: now,
      accepted_by_pin: receiverPin,
      accepted_by_name: receiverName,
      accepted_by_role: receiverRole,
    }, row.order_id || row.orderId || null);

    try {
      const next = lsRead().map((x) => {
        const xid = x.external_id || x.externalId;
        if (String(xid) !== String(eid)) return x;
        return {
          ...x,
          status: 'ACCEPTED',
          accepted_at: now,
    ...
    try {
      await supabase.from('company_budget_moves').upsert({
        direction: 'IN',
        amount: Number(row?.amount || 0) || 0,
        reason: 'DISPATCH_ACCEPTED',
        note: `DISPATCH ACCEPTED${row?.created_by_name ? ` · ${row.created_by_name}` : ''}${row?.order_code ? ` · #${row.order_code}` : ''}`,
        source: 'DISPATCH',
        external_id: `dispatch_accept_${eid}`,
        ref_day_id: row?.order_id || row?.orderId || null,
        ref_type: 'ARKA_PENDING_PAYMENT',
        created_by: receiverName || 'DISPATCH',
        created_by_name: receiverName || null,
        created_by_pin: receiverPin || null,
      }, { onConflict: 'external_id', ignoreDuplicates: true });
    } catch {}
  }

  return { ok: true, count: rows.length, total: Number(total.toFixed(2)), items: rows };
}
