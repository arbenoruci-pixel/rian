...
export async function recordCashMove(payload={}){
  ...
  const orderCode = String(payload.order_code || payload.code || '').toUpperCase();
  const pType = String(payload.type || '').toUpperCase();
  const isTransport = pType === 'TRANSPORT' || orderCode.startsWith('T');

  try {
    // 1) KRIJO ALWAYS PENDING (ose COLLECTED per TRANSPORT)
    const pendingRes = await createPendingCashPayment({
      ...payload,
      external_id,
      amount: amt,
      type: isTransport ? 'TRANSPORT' : (payload.type || 'IN'),
      status: isTransport ? 'COLLECTED' : 'PENDING',
    });

    // TRANSPORT CASH: mos e aplikoni automatikisht ne ciklin ditor.
    // Transportusi i mban paret deri sa t'i dorezoje (handoff) te dispatch/baze.
    if (isTransport) {
      return pendingRes;
    }
