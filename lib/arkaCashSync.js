...
// Fast-fail network calls so UI doesn't hang (iOS offline/online flaps)
...
async function backupPendingIntoOrder(payload = {}, external_id, rowLike = {}) {
  try {
    ...
    const { data, error } = await withTimeout(
      supabase.from('orders').select('id,data').eq('id', order_id).maybeSingle(),
      4500
    );
...
export async function createPendingCashPayment(payload={}){
  ...
  try {
    ...
    if (ok) {
      ...
      const { data, error } = await withTimeout(
        supabase.from('arka_pending_payments').insert(row).select('*').single(),
        4500
      );
      if (!error) {
        let rowData = data;
        try {
          if (String(row.type||'').toUpperCase()==='TRANSPORT' && String(rowData?.status||'').toUpperCase()!=='COLLECTED') {
            await withTimeout(
              supabase.from('arka_pending_payments').update({ status: 'COLLECTED' }).eq('external_id', row.external_id),
              4500
            );
      ...
      if (error.code === '23505') {
        ...
        supabase.from('arka_pending_payments').select('*').eq('external_id', external_id).maybeSingle(),
        4500
      );
...
export async function recordCashMove(payload={}){
  ...
  const isTransport = (typeUpper === 'TRANSPORT') || /^T\d+/.test(oc);
