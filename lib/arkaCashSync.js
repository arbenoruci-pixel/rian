...
export async function createPendingCashPayment(payload={}){
  ...
  const row = { 
    external_id, 
    // Allow callers to override status (e.g. TRANSPORT uses COLLECTED)
...
export async function recordCashMove(payload={}){
  ...
  // TRANSPORT: cash is collected by the transporter and must NOT auto-apply to the daily base cycle.
  // We keep it as status=COLLECTED in arka_pending_payments until an authorized user approves/applies it.
  const orderCode = String(payload.order_code || payload.code || '').trim();
  const isTransport = /^T\d+/i.test(orderCode) || String(payload.type || '').toUpperCase() === 'TRANSPORT';
  
  try {
    // 1) Create pending row
    const pendingRes = await createPendingCashPayment({
      ...payload,
      external_id,
      amount: amt,
      ...(isTransport ? { status: 'COLLECTED', type: 'TRANSPORT' } : {}),
    });

    // If TRANSPORT, stop here (no auto-apply)
    if (isTransport) return pendingRes;
