...
export async function createPendingCashPayment(payload={}){
  ...
  const row = { 
    ...
    note: payload.note || '',
    created_by_pin_REMOVED: payload.created_by_pin_REMOVED || payload.user_pin || null,
...
// Kthehet e grupuar sipas PIN-it (created_by_pin_REMOVED) per UI.
export async function listOwedCashPaymentsByPin(limit=500){
  ...
  // 3) Group by created_by_pin_REMOVED (ose pin)
  const byPin = new Map();
  for (const r of rows) {
    const pin = r.created_by_pin_REMOVED || r.pin || 'PA_PIN';
