...
export async function createPendingCashPayment(payload={}){
  ...
  const row = { 
    ...
    // NOTE: do NOT send created_at from the client. In some setups this column
    // is DEFAULT/GENERATED and rejects explicit values.
  };
...
export async function listOwedCashPaymentsByPin(limit=500){
  // 1) Merr pagesat PENDING/OWED nga DB (arka_pending_payments)
  let rows = [];
  try {
    const { data } = await supabase
      .from('arka_pending_payments')
      .select('*')
      // Legacy: disa versione e quanin statusin 'OWED'. Tash DB po e perdor 'PENDING'.
...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name, approved_by_pin, approved_by_role }){
  ...
  try {
    // 1) Sheno pagesen si APPLIED ne arka_pending_payments (PA insertuar ne buxhet)
    // Qellimi: nese RLS e bllokon update, mos me kriju levizje ne buxhet gabimisht.
...
// Try full update first; if DB doesn't have these columns, fall back to minimal update.
    let up = await supabase
      ...
      .update(fullUpdate)
      .in('status', ['PENDING','OWED'])
      .match(pid ? { id: pid } : { external_id: eid })
      .select('id,status,applied_cycle_id');
...
if (up.error) throw up.error;
    if (!up.data || up.data.length === 0) {
      // Nese veq eshte APPLIED, trajto si OK (p.sh. UI e vjeter / double click)
      const chk = await supabase
        .from('arka_pending_payments')
        .select('id,status,applied_cycle_id')
        .match(pid ? { id: pid } : { external_id: eid })
        .maybeSingle();
      if (chk?.data?.status !== 'APPLIED') throw new Error('RLS_BLOCKED_UPDATE');
    }

    // 2) Tash krijo levizjen ne company_budget_moves per ciklin aktual (nese s'ekziston)
    // FIX: external_id mbetet i njejte (eid), mos e ndrysho.
    // (Nese kjo insert deshton, pagesa prap mbetet APPLIED, por s'ka levizje -> e rregullojm me resync.)
    try {
      // Avoid duplicates by checking existing external_id
