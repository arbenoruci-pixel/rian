...
function todayKey() {
  // IMPORTANT: use LOCAL date (not UTC) so "dita" matches what users see on the device.
  // Using toISOString() would shift the day around midnight for many timezones,
  // which then breaks: open day detection + payments not showing in ARKA.
  const d = new Date();
...
export async function recordCashMove({
  ...
  try {
    ...
    const open = await dbGetOpenDay(todayKey());
