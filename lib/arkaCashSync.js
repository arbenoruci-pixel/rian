...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name }){
  ...
  try {
    ...
    if (!mv?.ok) {
      const msg = mv?.error?.message || mv?.error || 'dbAddCycleMove failed';
      throw new Error(msg);
    }
    ...
    const upd = await supabase
      ...
      .eq('external_id', eid);
    if (upd?.error) throw new Error(upd.error.message || 'update arka_pending_payments failed');
...
export async function rejectPendingPayment({ pending, rejected_by_pin, rejected_by_name, rejected_by_role, reject_note } = {}) {
  ...
  const r1 = await supabase.from('arka_pending_payments').update(payload).eq('external_id', eid);
  if (r1?.error) {
    // nese kolonat nuk ekzistojne, bej update minimal
    const msg = String(r1.error.message || '');
    if (msg.includes('does not exist') || msg.includes('schema cache')) {
      const r2 = await supabase
        .from('arka_pending_payments')
        .update({ status: 'DEBT', rejected_at: now })
        .eq('external_id', eid);
