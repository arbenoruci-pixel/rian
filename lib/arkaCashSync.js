...
export async function applyPendingPaymentToCycle({ pending, cycle_id, approved_by_name }){
  if (!pending || !cycle_id) return { ok: false };

  // UI may pass either:
  // 1) a single pending row (has external_id)
  // 2) a grouped object from listPendingCashPayments() (has items[])
  const items = Array.isArray(pending.items) ? pending.items : [pending];

  let applied = 0;
  const errors = [];

  for (const it of items) {
    const eid = it.external_id || it.externalId;
    if (!eid) {
      errors.push('MISSING_EXTERNAL_ID');
      continue;
    }
    const amt = Number(it.amount ?? it.sum ?? it.total ?? 0) || 0;
    if (!(amt > 0)) {
      errors.push(`BAD_AMOUNT:${eid}`);
      continue;
    }

    try {
      // 1) Add cash movement into the active cycle
      await dbAddCycleMove({
        cycle_id,
        type: it.type || 'IN',
        amount: amt,
        note: it.note || 'Pending Payment',
        source: 'ORDER_PAY',
        created_by: approved_by_name || 'SYSTEM',
        external_id: eid,
      });

      // 2) Mark pending as applied (DB)
      const { error: eUp } = await supabase
        .from('arka_pending_payments')
        .update({ status: 'APPLIED', applied_at: new Date().toISOString(), applied_cycle_id: cycle_id })
        .eq('external_id', eid);
      if (eUp) throw eUp;

      // 3) Mirror into order JSON fallback, and localStorage fallback
      ...
      errors.push(`${eid}:${err?.message || 'ERR'}`);
    }
  }

  return errors.length ? { ok: applied > 0, applied, error: errors.join(' | ') } : { ok: true, applied };
}
