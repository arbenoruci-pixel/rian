...
export async function rejectPendingPayment({ pending, rejected_by_pin, rejected_by_name, rejected_by_role, reject_note } = {}) {
  ...
  try {
    ...
    try {
      await supabase.from('arka_pending_payments').update({ status: 'OWED', type: 'WORKER_DEBT', rejected_at: now }).eq('external_id', eid);
    } catch {}
  }
  ...
  try {
    const next = lsRead().map(x => (x.external_id === eid || x.externalId === eid)
      ? { ...x, status: 'OWED', type: 'WORKER_DEBT', rejected_at: now, rejected_by_pin: rejected_by_pin ?? null, rejected_by_name: rejected_by_name ?? null, rejected_by_role: rejected_by_role ?? null, reject_note: reject_note ?? null }
      : x
    );
    lsWrite(next);
  } catch {}
  ...
  try {
    ...
    if (Array.isArray(data)) {
      data.forEach(row => {
        const pends = row.data?.pay?.pendingCash || [];
        pends.forEach(p => {
          if (p.status === 'OWED' && String(p.created_by_name || p.name || '').trim() === name) {
