...
export async function recordCashMove({
  ...
  const t = String(type || "IN").toUpperCase() === "OUT" ? "OUT" : "IN";
  const src = normSrc(source);

  // If there is an OPEN cycle, write directly as a move
  try {
    const c = await dbGetActiveCycle();
    if (c?.id) {
      return await dbAddCycleMove({
        cycle_id: c.id,
        type: t,
        amount: amt,
        note: String(note || ""),
        source: src,
        created_by,
        person_pin: src === "PERSONAL" ? (String(person_pin || "").trim() || null) : null,
      });
    }
  } catch {
    // ignore and fall back to pending table
  }

  // Otherwise queue it for later sync
  const row = {
    status: "PENDING",
    type: t,
    amount: amt,
  ...
  try {
    ...
    // If table doesn't exist, just no-op so build/runtime doesn't crash
    return { ok: false, queued: false, error: e?.message || String(e) };
  }
}
