  ...
  return {
      ...
      return serialize(() => transaction('readwrite', (store, done, fail) => {
        ...
        read.onsuccess = () => {
          try {
            ...
            if (Number(previous?.deletedAt || previous?.ts || 0) > row.ts
              || (previous?.deletedAt && previous.deletedAt >= row.ts)) return done({ ok: true, skipped: true });
    ...
    list(scope = '') {
      return serialize(async () => {
        ...
        }).catch(error => { if (legacy.size) return []; throw error; });
        for (const row of saved) {
          const old = legacy.get(String(row.id));
          if (!old || Number(row.deletedAt || row.ts || 0) >= Number(old.ts || 0)) legacy.set(String(row.id), row);
        }
