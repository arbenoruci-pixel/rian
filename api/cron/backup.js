import { apiFail, apiOk, createAdminClientOrThrow, pickEnv, asObject, jparse } from '../_helpers.js';
...
function buildTable(title, rows, columns) {
  ...
  return `<div style="margin:0 0 24px 0;"><h2 style="margin:0 0 10px 0;font-size:18px;">${escapeHtml(title)} (${rows.length})</h2><table style="width:100%;border-collapse:collapse;"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const html = `<div style="font-family:Arial,sans-serif;color:#111;"><h1 style="margin:0 0 12px 0;font-size:22px;">Backup ditor i porosive aktive</h1><p style="margin:0 0 18px 0;font-size:14px;">Gjeneruar më ${escapeHtml(fmtDateTime(new Date().toISOString()))}</p>${buildTable('Baza / Pastrimi', baseRows, [{ label: 'Kodi', value: (r) => r.code || '-' },{ label: 'Klienti', value: (r) => r.client_name || '-' },{ label: 'Tel', value: (r) => r.client_phone || '-' },{ label: 'Statusi', value: (r) => r.status || '-' },{ label: 'Totali €', value: (r) => money(r.total) },{ label: 'Përditësuar', value: (r) => fmtDateTime(r.updated_at) }])}${transportSections || buildTable('Transporti', [], [{ label: 'Kodi', value: () => '-' },{ label: 'Klienti', value: () => '-' },{ label: 'Tel', value: () => '-' },{ label: 'Statusi', value: () => '-' },{ label: 'Vizita', value: () => '-' },{ label: 'Përditësuar', value: () => '-' }])}</div>`;
    ...
    return apiOk(res, { sent_to: to, base_count: baseRows.length, transport_count: transportRows.length, resend });
  } catch (error) {
