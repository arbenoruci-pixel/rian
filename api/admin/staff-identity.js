...
function safeError(error) {
  if (error instanceof StaffIdentityMutationError) return error;
  return new StaffIdentityMutationError('STAFF_IDENTITY_REQUEST_FAILED', 500);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const output = await runStaffIdentityMutation(body, { supabase, authUser });
    return apiOk(res, output);
  } catch (error) {
    const safe = safeError(error);
    console.error('[staff-identity]', {
      code: safe.code,
