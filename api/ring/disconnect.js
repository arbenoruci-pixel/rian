import { disconnectRing } from '../../lib/ringIntegrationServer.js';
import {
  apiFail,
  apiOk,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await disconnectRing({ supabase });
    return apiOk(res, result);
  } catch (error) {
    const safe = safeRingError(error);
    console.error('[ring-disconnect]', { code: safe.code, status: safe.httpStatus });
