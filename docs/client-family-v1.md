...
Status: reactivated for production on 2026-09-14 under the user's subsequent explicit instruction to deploy so they can perform the remaining app test themselves. The prior browser-verification hold is superseded. The verified backup is retained.
...
- The user subsequently requested deployment for their own manual app test. The same verified family SQL was reapplied successfully as `20260914011841_reactivate_client_family_links_v1`, with bounded lock/statement timeouts. Base and Transport family resolution is active; no synthetic test data was seeded for this release.
...
- Active migration version: `20260914011841_reactivate_client_family_links_v1` (initial application and rollback are recorded above).
