...
function TransportItemPageInner() {
  ...
  async function setStatus(nextStatus) {
    ...
    try {
      ...
      const nextData = { ...currentData, status: nextStatus };
      const cleanNext = String(nextStatus || '').toLowerCase();
      if (cleanNext === 'pickup') {
        const actor = readActor() || getTransportSession() || {};
        nextData.pickup_at = nextData.pickup_at || nowIso;
        nextData.accepted_at = nextData.accepted_at || nowIso;
        nextData.started_at = nextData.started_at || nowIso;
        nextData.accepted_by_pin = nextData.accepted_by_pin || actor?.pin || actor?.transport_pin || null;
        nextData.accepted_by_name = nextData.accepted_by_name || actor?.name || actor?.full_name || actor?.username || null;
      }
      if (cleanNext === 'pastrim' || cleanNext === 'pastrimi') {
        nextData.pastrim_started_at = nextData.pastrim_started_at || nowIso;
        nextData.brought_to_base_at = nextData.brought_to_base_at || nowIso;
        nextData.base_arrived_at = nextData.base_arrived_at || nowIso;
      }
      await updateTransportOrderById(row.id, { status: nextStatus, data: nextData });
      setRow((prev) => prev ? { ...prev, status: nextStatus, data: nextData } : prev);
    } catch (e) {
