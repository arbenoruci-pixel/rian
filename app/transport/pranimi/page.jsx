...
export default function TransportPranim() {
  ...
  const [payAdd, setPayAdd] = useState(0); // "KLIENTI DHA"
  ...
  useEffect(() => {
    ...
    draftTimerRef.current = setTimeout(() => {
      const hasData = (name && name.length > 1) || (phone && phone.length > 3) || tepihaRows.length > 0 || stazaRows.length > 0;
  ...
  const dueNow = useMemo(() => Math.max(0, Number((Number(totalEuro || 0) - Number(clientPaid || 0)).toFixed(2))), [totalEuro, clientPaid]);
  const payGiven = useMemo(() => Math.max(0, Number((Number(payAdd || 0)).toFixed(2))), [payAdd]);
  const payApplied = useMemo(() => Number(Math.min(payGiven, dueNow).toFixed(2)), [payGiven, dueNow]);
  const payChange = useMemo(() => Number(Math.max(0, payGiven - dueNow).toFixed(2)), [payGiven, dueNow]);
  ...
  function applyPayAndClose() {
    ...
    const applied = Number(Math.min(cashGiven, dueNow).toFixed(2));
  ...
  async function saveOrder() {
    ...
    try {
      ...
      // Record collected cash (ONLY what is applied to the bill, not what the client "gave")
      if (Number(paidEuro || 0) > 0) {
        await recordCashMove({
          amount: Number(paidEuro || 0),
          method: 'CASH',
          type: 'TRANSPORT',
          status: 'COLLECTED',
    ...
    <div className="wrap" style={{ paddingBottom: 200 }}>
      ...
      <section className="card">
        ...
        <div className="chip-row modern">
          {TEPIHA_CHIPS.map((v) => (
            <button key={v} type="button" className="chip chip-modern" onClick={(e) => applyChip('tepiha', v, e)} style={chipStyleForVal(v, false)}>
              {v.toFixed(1)}
            </button>
          ))}
        </div>
      ...
      <section className="card">
        ...
        <div className="chip-row modern">
          {STAZA_CHIPS.map((v) => (
            <button key={v} type="button" className="chip chip-modern" onClick={(e) => applyChip('staza', v, e)} style={chipStyleForVal(v, false)}>
              {v.toFixed(1)}
            </button>
          ))}
        </div>
      ...
      <section className="card">
        <div className="row util-row" style={{ gap: 10 }}>
          ...
          <button
            ...
            onTouchEnd={(e) => { e.preventDefault(); endPayHold(); }}
          >
            € PAGESA
          </button>
        </div>
        ...
        <div className="payfs">
          <div className="payfs-top">
            <div>
              ...
              <div className="payfs-sub">KODI: {normalizeTCode(codeRaw)} • {name || '—'}</div>
            </div>
          ...
          <div className="payfs-body">
            <div className="card" style={{ marginTop: 0 }}>
              ...
              <div className="tot-line" style={{ borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 10, paddingTop: 10 }}>
                DUHET: <strong>{dueNow.toFixed(2)} €</strong>
              </div>
              <div className="tot-line">KLIENTI DHA: <strong>{payGiven.toFixed(2)} €</strong></div>
              <div className="tot-line">SOT MERREN: <strong>{payApplied.toFixed(2)} €</strong></div>
              {payChange > 0 && <div className="tot-line">KTHIM: <strong style={{ color: '#2563eb' }}>{payChange.toFixed(2)} €</strong></div>}
            </div>
              ...
              <div className="field-group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setPayAdd(v === '' ? 0 : Number(String(v).replace(',', '.')));
                  }}
                />

                <div className="chip-row" style={{ marginTop: 10 }}>
                  {PAY_CHIPS.map((v) => (
                    <button key={v} className="chip" type="button" onClick={() => setPayAdd(v)}>{v}€</button>
                  ))}
                  <button className="chip" type="button" onClick={() => setPayAdd(dueNow)} style={{ opacity: 0.9 }}>TOTAL</button>
                  <button className="chip" type="button" onClick={() => setPayAdd(0)} style={{ opacity: 0.9 }}>FSHI</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {showPriceSheet && (
        <div className="payfs">
          ...
          <div className="payfs-footer">
            ...
            <button className="btn primary" onClick={() => { setPricePerM2(priceTmp); setShowPriceSheet(false); }}>RUJ</button>
          </div>
        </div>
      )}

      {/* STAIRS SHEET (unchanged UI) */}
      {showStairsSheet && (
            ...
            <div className="field-group" style={{ marginTop: 12 }}>
              ...
              <div className="chip-row">
                {SHKALLORE_QTY_CHIPS.map((n) => (
                  <button key={n} className="chip" type="button" onClick={() => { setStairsQty(n); vibrateTap(15); }} style={Number(stairsQty) === n ? { outline: '2px solid rgba(255,255,255,0.35)' } : null}>
                    {n}
                  </button>
                ))}
              </div>
            ...
            <div className="field-group">
              ...
              <div className="chip-row">
                {SHKALLORE_PER_CHIPS.map((v) => (
                  <button key={v} className="chip" type="button" onClick={() => { setStairsPer(v); vibrateTap(15); }} style={Number(stairsPer) === v ? { outline: '2px solid rgba(255,255,255,0.35)' } : null}>
                    {v}
                  </button>
                ))}
              </div>
        ...
        <div className="payfs">
          <div className="payfs-top">
            <div>
              ...
              <div className="payfs-sub">HAP ose FSHI</div>
            </div>
          ...
          <div className="payfs-body">
            <div className="card" style={{ marginTop: 0 }}>
              {drafts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '20px', color: 'rgba(255,255,255,0.7)' }}>S'ka drafte.</div>
              ) : drafts.map(d => (
                <div key={d.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '10px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 900, fontSize: 15 }}>{normalizeTCode(d.codeRaw)}</div>
                    <div style={{ fontSize: 12, opacity: 0.8 }}>{d.name || 'Pa Emër'} • {d.phone || '-'}</div>
                    <div style={{ fontSize: 10, opacity: 0.5 }}>{new Date(d.ts).toLocaleString()}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn secondary" style={{ padding: '6px 10px', fontSize: 11 }} onClick={() => loadDraft(d)}>HAP</button>
                    <button className="btn secondary" style={{ padding: '6px 10px', fontSize: 11, color: '#ef4444' }} onClick={() => deleteDraft(d.id)}>FSHI</button>
                  </div>
                </div>
              ))}
            </div>
