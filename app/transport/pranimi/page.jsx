...
function assertVerifiedTransportOrderRow(row = {}, expected = {}) {
  ...
  const expectedPhoneKey = normalizeTransportPhoneKey(expected?.phone || '');
  const rowPhoneKey = normalizeTransportPhoneKey(row?.client_phone || rowData?.client_phone || rowClient?.phone || '');
  if (expectedPhoneKey && rowPhoneKey && rowPhoneKey !== expectedPhoneKey) {
    throw new Error('TRANSPORT_ORDER_VERIFY_PHONE_MISMATCH');
  }
  ...
  const rowTransportPin = String(rowData?.transport_pin || '').trim();
  if (expectedTransportPin && rowTransportPin !== expectedTransportPin) {
    throw new Error('TRANSPORT_ORDER_VERIFY_TRANSPORT_PIN_MISMATCH');
  }
  const expectedDriverPin = String(expected?.driverPin || '').trim();
  const rowDriverPin = String(rowData?.driver_pin || '').trim();
  if (expectedDriverPin && rowDriverPin !== expectedDriverPin) {
    throw new Error('TRANSPORT_ORDER_VERIFY_DRIVER_PIN_MISMATCH');
  }
  const expectedCreatedByPin = String(expected?.createdByPin || '').trim();
  const rowCreatedByPin = String(rowData?.created_by_pin || '').trim();
  if (expectedCreatedByPin && rowCreatedByPin !== expectedCreatedByPin) {
    throw new Error('TRANSPORT_ORDER_VERIFY_CREATED_BY_PIN_MISMATCH');
  }
  const expectedCreatedByRole = String(expected?.createdByRole || '').trim().toUpperCase();
  const rowCreatedByRole = String(rowData?.created_by_role || '').trim().toUpperCase();
  if (expectedCreatedByRole && rowCreatedByRole !== expectedCreatedByRole) {
    throw new Error('TRANSPORT_ORDER_VERIFY_CREATED_BY_ROLE_MISMATCH');
  }
  const expectedOrigin = String(expected?.orderOrigin || '').trim().toUpperCase();
  const rowOrigin = String(rowData?.order_origin || '').trim().toUpperCase();
  if (expectedOrigin && rowOrigin !== expectedOrigin) {
    throw new Error('TRANSPORT_ORDER_VERIFY_ORIGIN_MISMATCH');
  }
  const expectedPickupDate = String(expected?.pickupDate || '').trim();
  const rowPickupDate = String(rowData?.pickup_date || '').trim();
  if (expectedPickupDate && rowPickupDate !== expectedPickupDate) {
    throw new Error('TRANSPORT_ORDER_VERIFY_PICKUP_DATE_MISMATCH');
  }
  const expectedPickupWindow = String(expected?.pickupWindow || '').trim();
  const rowPickupWindow = String(rowData?.pickup_window || '').trim();
  if (expectedPickupWindow && rowPickupWindow !== expectedPickupWindow) {
    throw new Error('TRANSPORT_ORDER_VERIFY_PICKUP_WINDOW_MISMATCH');
  }
  return row;
}

async function verifyTransportOrderRemoteRow(expected = {}) {
  const id = String(expected?.id || '').trim();
  if (!id) throw new Error('TRANSPORT_ORDER_VERIFY_MISSING_ID');
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    throw new Error('TRANSPORT_ORDER_VERIFY_OFFLINE');
  }
  const query = supabase
    ...
    .eq('id', id)
    .maybeSingle();
  const { data, error } = await withSupabaseTimeout(query, 9000, 'TRANSPORT_ORDER_REMOTE_VERIFY_TIMEOUT', { id });
  if (error) throw error;
  if (!data) throw new Error('TRANSPORT_ORDER_NOT_FOUND_AFTER_SAVE');
  return assertVerifiedTransportOrderRow(data, expected);
}

async function createTransportOrderOnlineVerifiedOrQueue(payload = {}, expected = {}, runtime = {}) {
  // The old sticky transport_offline_mode_v1 flag must not decide the create path.
  ...
  const verify = async (stage = 'VERIFY') => {
    try {
      return await verifyTransportOrderRemoteRow(expected);
    } catch (verifyError) {
      const msg = String(verifyError?.message || verifyError || 'TRANSPORT_ORDER_NOT_FOUND_AFTER_SAVE');
      throw decorateTransportSaveError(new Error(`${stage}: transport_order nuk u gjet/verifikua në DB për id=${String(expected?.id || payload?.id || '-')} code=${String(expected?.codeStr || payload?.code_str || '-')}. ${msg}`), { noOfflineQueue: true });
    }
  };
  ...
  try {
    ...
    if (!insertRes?.ok) {
      ...
      // A DB insert can fail after a partial write. Accept only a real DB verification.
      try { return await verify('TRANSPORT_ORDER_INSERT_RESPONSE_FAILED_BUT_VERIFY'); } catch (verifyError) {
        const joined = `${insertError.message || ''}${verifyError?.message ? ` | ${verifyError.message}` : ''}`;
        throw decorateTransportSaveError(new Error(`TRANSPORT_ORDER_DB_SAVE_FAILED: ${joined}`), { noOfflineQueue: true });
      }
    }
    return await verify('TRANSPORT_ORDER_AFTER_INSERT_VERIFY');
  } catch (error) {
    if (error?.offlineQueued || error?.noOfflineQueue) throw error;
    ...
    // Timeout/network errors while navigator is still online must remain visible.
    // Also try a final DB verify in case the insert committed but response was lost.
    if (isTransportNetworkLikeError(error)) {
      try { return await verify('TRANSPORT_ORDER_NETWORK_ERROR_BUT_VERIFY'); } catch (verifyError) {
        const joined = `${error?.message || error || ''}${verifyError?.message ? ` | ${verifyError.message}` : ''}`;
        throw decorateTransportSaveError(new Error(`TRANSPORT_ORDER_ONLINE_SAVE_NOT_VERIFIED: ${joined}`), { noOfflineQueue: true });
      }
    }

    // DB/data errors must stay visible; do not hide them behind SMS success or a silent queue.
    throw decorateTransportSaveError(error, { noOfflineQueue: true });
  }
}
...
function normalizeTransportClientCandidate(c = {}) {
  ...
  const tcode = normalizeTcode(isOrderHistory
    ? (c?.transport_client_tcode || c?.client?.transport_client_tcode || c?.tcode || c?.client_tcode || c?.client?.tcode || c?.client?.code || '')
    : (c?.tcode || c?.client_tcode || c?.code || c?.code_str || '')
  );
  return {
...
function PranimiPageInner() {
  ...
  // New transport orders start in the same PIKAP lane as dispatch-created pickups.
  // They move to PASTRIM only after the transporter marks them brought to base.
  const createStatus = 'pickup';
        ...
        if (isEdit) {
            ...
            if (row) {
                ...
                if (isTransportOrderPaymentBlocked(row)) {
                  alert('POROSIA ËSHTË E ANULUAR. KRIJO NJË VIZITË TË RE; PAGESA NË KËTË POROSI ËSHTË E BLLOKUAR.');
  ...
  useEffect(() => {
    try {
      ...
      // Clear stale local offline markers as soon as the browser reports online.
      // This prevents old queue/failure state from hijacking the next live self-entry.
      if (!browserOffline) localStorage.removeItem(OFFLINE_MODE_KEY);
      setOfflineMode(Boolean(browserOffline));
      setNetState({
        ok: !browserOffline,
        reason: browserOffline ? 'OFFLINE' : '',
      });
    } catch {}
    const onOnline = () => {
  ...
  function isSelectedTransportPhoneClient(candidate = {}) {
      ...
      if (cCode && cCode !== 'T0' && normalizeTcode(clientTcode || '') === cCode && sameTransportPhone(c.phone_digits || c.phone, phonePrefix + phone)) return true;
      return false;
  }
  ...
  function saveClientQuick() {
      ...
      const normalized = codeRaw ? normalizeTcode(codeRaw) : '';
  ...
  async function handleContinue() {
      ...
      const payload = {
          id: oid, 
          code_str: officialOrderTcode, 
          code_n: transportCodeNumber(officialOrderTcode),
          ...
          data: {
            ...
            status: nextStatus,
            code_str: officialOrderTcode,
            order_code: officialOrderTcode,
            official_order_code: officialOrderTcode,
            ...
            client: {
              ...
              tcode: officialOrderTcode,
              code: officialOrderTcode,
              order_code: officialOrderTcode,
              official_order_code: officialOrderTcode,
              transport_client_tcode: clientBookTcode || null,
            ...
            created_by_role: isBaseWorkerBridgeEdit ? (editDataForTransport?.created_by_role || actor?.role || null) : (currentActorRole === 'TRANSPORT' ? 'TRANSPORT' : (actor?.role || null)),
            pickup_date: editDataForTransport?.pickup_date || pickupDate,
            pickup_slot: editDataForTransport?.pickup_slot || pickupSlot,
            pickup_window: editDataForTransport?.pickup_window || transportPickupWindow(pickupSlot),
            gps_lat: gpsLat || null,
            gps_lng: gpsLng || null
          }
      };
      ...
      if (isEdit) {
        ...
        delete payload.code_n;
        delete payload.code_str;
      ...
      try {
        if (isEdit) {
          ...
          const verifiedOrder = await createTransportOrderOnlineVerifiedOrQueue(
            ...
            {
              id: oid,
              codeStr: officialOrderTcode,
              phone: phoneFull,
              status: nextStatus,
              dataStatus: nextStatus,
              transportId: tid,
              assignedDriverId: payload?.data?.assigned_driver_id,
              transportPin: payload?.data?.transport_pin,
              driverPin: payload?.data?.driver_pin,
              createdByPin: payload?.data?.created_by_pin,
              createdByRole: payload?.data?.created_by_role,
              orderOrigin: payload?.data?.order_origin,
        ...
        if(autoMsgAfterSave) { setMsgKind('start'); setShowMsgSheet(true); } // ✅ HAP MESAZHIN
          ...
          try {
            if (queuedTransportOffline && isBrowserTransportOffline()) localStorage.setItem(OFFLINE_MODE_KEY, '1');
            else localStorage.removeItem(OFFLINE_MODE_KEY);
          } catch {}
          const saveErrorMessage = String(e?.message || '');
          alert(
  ...
  async function applyPayAndClose() {
    ...
    if (isTransportOrderPaymentBlocked(editRowStatus)) {
      alert('POROSIA ËSHTË E ANULUAR. PAGESA U BLLOKUA. KRIJO NJË VIZITË TË RE.');
      return;
    }
    if (Number(totalM2 || 0) <= 0 || Number(totalEuro || 0) <= 0) {
      alert('POROSIA S’KA MASA OSE TOTAL TË VLEFSHËM. PAGESA U BLLOKUA.');
      return;
    }
            ...
            <div className="card" style={{padding:'12px 14px', borderRadius:18}}>
              ...
              <select
                ...
                onChange={async (e) => {
                  ...
                  setAssignTid(v);
                  if (!isEdit) {
                    // Switching driver/admin scope starts a clean official order code.
                    setCodeRaw('');
                    resetAcceptedTransportClientIdentity();
                  }
                }}
                ...
                {clientHits.map((c, i) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setClientId(null);
                      setClientTcode('');
                      // Keep the official order code; a client hit must not replace or clear it.
                      setTransportClientMatchDecision({ matchKey: '', mode: '' });
              ...
              <div className="client-selected-main">
                ...
                <div className="client-selected-copy">
                  <div className="client-copy-topline">
                    <div className="client-code-pill">{(codeRaw || clientTcode) ? normalizeTcode(codeRaw || clientTcode) : 'I RI'}</div>
          ...
          <div className="modal-overlay" onClick={() => setShowAddClient(false)}>
            <div className="modal-content dark add-client-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="field-group transport-tcode-focus">
                ...
                <input className="input tcodeInput" value={(codeRaw || clientTcode) ? normalizeTcode(codeRaw || clientTcode) : 'MERRET PAS TEL.'} readOnly disabled placeholder="T123" style={{ opacity: 0.7, cursor: 'not-allowed' }} />
              </div>
