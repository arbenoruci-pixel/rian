...
function getTransportDisplayCode(row = {}) {
  const raw = String(row?.code_str || row?.client_tcode || row?.data?.client_tcode || row?.data?.client?.tcode || row?.data?.client?.code || '').trim();
  if (!raw) return '';
...
function buildTransportStatusPatch(current = {}, nextStatus = '', transportId = '', nowIso = new Date().toISOString(), sessionObj = {}, options = {}) {
  ...
  if (status === 'pastrim') {
    data.at_base_at = nowIso;
    data.pastrim_at = nowIso;
    data.unloaded_at = nowIso;
  ...
  if (status === 'pickup') data.pickup_at = data.pickup_at || nowIso;
