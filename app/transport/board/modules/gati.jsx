...
const OVERDUE_FOLLOWUP_DAY_THRESHOLD = 3;
const OVERDUE_FOLLOWUP_LOG_KEY = 'transport_overdue_followups';
const OVERDUE_FOLLOWUP_SOURCE = 'transport_gati_overdue_v1';
const OVERDUE_FOLLOWUP_REASONS = [
  {
    key: 'no_answer',
    label: 'NUK PO LAJMËROHET',
    helper: 'Hap mesazh që klienti të konfirmojë dorëzimin.',
    smsAction: 'transport_followup_no_answer',
  },
  {
    key: 'not_accepting',
    label: 'NUK PO I PRANON',
    helper: 'Hap mesazh për konfirmim/riplanifikim të dorëzimit.',
    smsAction: 'transport_followup_not_accepting',
  },
  {
    key: 'confirm_address',
    label: 'DUHET ADRESË/ORAR',
    helper: 'Hap mesazh për adresë të saktë dhe orar.',
    smsAction: 'transport_followup_confirm_address',
  },
  {
    key: 'general_followup',
    label: 'DUHET KONFIRMIM',
    helper: 'Hap mesazh të përgjithshëm për ta shtyrë klientin të përgjigjet.',
    smsAction: 'transport_followup_general',
  },
];
...
function safeTransportData(row) {
  const data = row?.data;
...
function localDateKey(value = new Date()) {
  const key = toLocalDateValue(value || new Date());
  if (key) return key;
  return toLocalDateValue(new Date());
}

function getPastrimStartAt(row) {
  const d = safeTransportData(row);
  return (
    row?.pastrim_at ||
    d?.pastrim_at ||
    d?.pastrim_started_at ||
    d?.at_base_at ||
    d?.brought_to_base_at ||
    d?.base_arrived_at ||
    d?.unloaded_at ||
    row?.ready_at ||
    d?.ready_at ||
...
function getOverdueFollowupLog(row) {
  const d = safeTransportData(row);
  const raw = d?.[OVERDUE_FOLLOWUP_LOG_KEY];
  return Array.isArray(raw) ? raw.filter(Boolean) : [];
}

function getLatestOverdueFollowup(row) {
  const list = getOverdueFollowupLog(row);
  return list.length ? list[list.length - 1] : null;
}

function hasOverdueFollowupForDay(row, dayKey) {
  const cleanDay = String(dayKey || '').trim();
  if (!cleanDay) return false;
  return getOverdueFollowupLog(row).some((entry) => {
    const entryDay = String(entry?.day_key || '').trim() || localDateKey(entry?.at || entry?.created_at || '');
    return entryDay === cleanDay;
  });
}
...
const overdueFollowupBadgeStyle = {
  ...readyMiniBadgeStyle,
...
const overdueHandledBadgeStyle = {
  ...readyMiniBadgeStyle,
...
function ReadyView({
  ...
  const [overdueFollowupActiveId, setOverdueFollowupActiveId] = useState('');
  const [overdueFollowupNote, setOverdueFollowupNote] = useState('');
  const [savingOverdueFollowup, setSavingOverdueFollowup] = useState(false);
  const [overdueFollowupPaused, setOverdueFollowupPaused] = useState(false);
  const [handledOverdueFollowupKeys, setHandledOverdueFollowupKeys] = useState(() => new Set());
  ...
  const todayFollowupKey = localDateKey(new Date());
  ...
  const itemView = useMemo(() => {
    ...
    const mapped = list.map((row) => {
      ...
      const pastrimStartAt = getPastrimStartAt(row);
      const pastrimStartTs = safeTs(pastrimStartAt);
      const pastrimStartDayTs = startOfDayTsFromTs(pastrimStartTs);
      const pastrimAgeDays = ageDaysSinceTs(pastrimStartDayTs, startOfTodayTs);
      const latestFollowup = getLatestOverdueFollowup(row);
      const followedUpToday = hasOverdueFollowupForDay(row, todayFollowupKey);
    ...
    return mapped;
  }, [deferredItems, getSmsCount, todayFollowupKey]);
  ...
  const overdueFollowupItems = useMemo(() => {
    return itemView.filter((item) => {
      if (!item?.id) return false;
      if (Number(item?.pastrimAgeDays || 0) < OVERDUE_FOLLOWUP_DAY_THRESHOLD) return false;
      const localKey = `${item.id}:${todayFollowupKey}`;
      if (handledOverdueFollowupKeys.has(localKey)) return false;
      return !item.followedUpToday;
    });
  }, [handledOverdueFollowupKeys, itemView, todayFollowupKey]);

  const activeOverdueFollowupItem = useMemo(() => {
    if (!overdueFollowupItems.length) return null;
    return overdueFollowupItems.find((item) => item.id === overdueFollowupActiveId) || overdueFollowupItems[0];
  }, [overdueFollowupActiveId, overdueFollowupItems]);

  useEffect(() => {
    if (savingOverdueFollowup || overdueFollowupPaused) return;
    if (!overdueFollowupItems.length) {
      if (showOverdueFollowup) setShowOverdueFollowup(false);
      if (overdueFollowupActiveId) setOverdueFollowupActiveId('');
      return;
    }
    const nextId = overdueFollowupItems[0]?.id || '';
    if (!showOverdueFollowup) setShowOverdueFollowup(true);
    if (!overdueFollowupActiveId || !overdueFollowupItems.some((item) => item.id === overdueFollowupActiveId)) {
      setOverdueFollowupActiveId(nextId);
    }
  }, [overdueFollowupActiveId, overdueFollowupItems, overdueFollowupPaused, savingOverdueFollowup, showOverdueFollowup]);
  ...
  const saveRiplan = useCallback(async (row) => {
    ...
    try {
      ...
      const currentData = (row?.data && typeof row.data === 'object' && !Array.isArray(row.data)) ? row.data : {};
      await updateTransportOrderById(row.id, {
        status: 'riplan',
        updated_at: nowIso,
        data: { ...currentData, status: 'riplan', riplan_at: nowIso },
        reschedule_at: iso,
  ...
  const saveOverdueFollowup = useCallback(async (item, reason) => {
    const row = item?.row || null;
    const reasonObj = reason || OVERDUE_FOLLOWUP_REASONS[0];
    if (!row?.id) return;
    const orderId = String(row.id);
    const localHandledKey = `${orderId}:${todayFollowupKey}`;
    const nowIso = new Date().toISOString();
    const note = cleanNoteText(overdueFollowupNote);
    const prevData = safeTransportData(row);
    const prevLog = getOverdueFollowupLog(row).slice(-24);
    const entry = {
      at: nowIso,
      day_key: todayFollowupKey,
      source: OVERDUE_FOLLOWUP_SOURCE,
      reason_key: String(reasonObj?.key || 'general_followup'),
      reason_label: String(reasonObj?.label || 'DUHET KONFIRMIM'),
      note: note || null,
      sms_action: reasonObj?.smsAction || null,
      client_phone_present: !!getClientPhone(row),
      code: getCode(row) || null,
      pastrim_started_at: item?.pastrimStartAt || getPastrimStartAt(row) || null,
      pastrim_age_days: Number(item?.pastrimAgeDays || 0),
      ready_at: item?.readyAt || getReadyAt(row) || null,
    };
    const nextLog = [...prevLog, entry];
    const nextData = {
      ...prevData,
      [OVERDUE_FOLLOWUP_LOG_KEY]: nextLog,
      transport_overdue_last_followup_at: nowIso,
      transport_overdue_last_followup_day: todayFollowupKey,
      transport_overdue_last_followup_reason: entry.reason_label,
      transport_overdue_last_followup_note: note || null,
      transport_overdue_requires_attention: true,
    };
    const nextRow = { ...row, updated_at: nowIso, data: nextData };

    setSavingOverdueFollowup(true);
    try {
      await updateTransportOrderById(orderId, { updated_at: nowIso, data: nextData });
    } catch (e) {
      try { console.warn('transport overdue followup saved local/queued but remote update failed', e); } catch {}
    } finally {
      setHandledOverdueFollowupKeys((prev) => {
        const next = new Set(prev || []);
        next.add(localHandledKey);
        return next;
      });
      setOverdueFollowupNote('');
      setOverdueFollowupPaused(true);
      setShowOverdueFollowup(false);
      setOverdueFollowupActiveId('');
      setSavingOverdueFollowup(false);
      try { window.dispatchEvent(new CustomEvent('transport:refresh')); } catch {}
    }

    if (reasonObj?.smsAction && onOpenSms) {
      runDeferred(() => onOpenSms(nextRow, reasonObj.smsAction), 120);
    }
    runDeferred(() => setOverdueFollowupPaused(false), 45000);
  }, [onOpenSms, overdueFollowupNote, runDeferred, todayFollowupKey]);
  ...
  return (
    <>
      <div style={{ padding: '0 12px 10px', display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap', width: '100%', maxWidth: '100%', boxSizing: 'border-box', overflow: 'hidden' }}>
        {selectionMode ? (
          <>
            <button style={topActionBtnStyle} onClick={() => runDeferred(() => { setSelectionMode(false); setSelectedIds(new Set()); })}>Anulo</button>
            <button style={topActionBtnStyle} onClick={() => runDeferred(selectAll)}>Zgjedh Krejt</button>
            {(selectedIds?.size || 0) > 0 && (
              <>
                <button style={topActionBtnStyle} onClick={() => runDeferred(() => { prepareActionItems(); setShowBulk(true); })}>DËRGO SMS KONFIRMIMI</button>
                <button style={topActionBtnStyle} onClick={() => runDeferred(() => { prepareActionItems(); setShowRoute(true); })}>OPTIMIZO RRUGËN</button>
              </>
            )}
          </>
        ) : (
          <button style={topActionBtnStyle} onClick={() => runDeferred(() => setSelectionMode(true))}>Selekto</button>
        )}
      </div>
      ...
      <div style={ui.listContainer}>
        ...
        {visibleReadyItems.map((item) => {
          ...
          const followupLocalHandled = handledOverdueFollowupKeys.has(`${item.id}:${todayFollowupKey}`);
          const needsOverdueFollowup = Number(item.pastrimAgeDays || 0) >= OVERDUE_FOLLOWUP_DAY_THRESHOLD && !item.followedUpToday && !followupLocalHandled;
          const followupWasHandled = Number(item.pastrimAgeDays || 0) >= OVERDUE_FOLLOWUP_DAY_THRESHOLD && !needsOverdueFollowup && (item.followedUpToday || followupLocalHandled);
          return (
            <div
              key={item.id}
              style={readyRowStyle(item)}
              ...
              <div style={{ minWidth: 0, flex: 1, display: 'grid', gap: 5 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', alignItems: 'start', gap: 6, minWidth: 0, maxWidth: '100%' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4, flexWrap: 'wrap', maxWidth: 158, minWidth: 0 }}>
                    ...
                    {smsCount > 0 ? <span style={readyMetaPillStyle}>SMS {smsCount}/3</span> : null}
                    {Number(item.pastrimAgeDays || 0) >= OVERDUE_FOLLOWUP_DAY_THRESHOLD ? <span style={readyMetaPillStyle}>{item.pastrimAgeDays}D PASTRIM</span> : null}
                    {item.ageDays > 0 ? <span style={readyMetaPillStyle}>{item.ageDays}D GATI</span> : null}
                  </div>
                </div>
                ...
                <div style={{ ...readyFooterStyle, gap: 6, minWidth: 0, maxWidth: '100%' }}>
                  <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.totals?.pieces} copë • {money(item.totals?.total)} €</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, marginLeft: 'auto', minWidth: 0, maxWidth: '100%' }}>
                    {item.rackLabel ? <span style={{ color: '#86efac', fontWeight: 950, maxWidth: 92, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>R: {item.rackLabel}</span> : null}
                    <span style={{ ...readyOpenPillStyle, minWidth: 56, height: 28, padding: '0 10px' }}>HAP</span>
                  </span>
                </div>
              </div>
            </div>
      ...
      {showOverdueFollowup && activeOverdueFollowupItem && (
        <div style={{ ...ui.modalOverlay, justifyContent: 'center', alignItems: 'center', padding: 16 }}>
          <div style={{ ...ui.toolsSheet, maxWidth: 560, width: '100%', border: '1px solid rgba(248,113,113,0.45)', boxShadow: '0 22px 70px rgba(0,0,0,0.55)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 950, letterSpacing: 0.8, color: '#fca5a5', textTransform: 'uppercase' }}>NDJEKJE E DETYRUESHME</div>
                <div style={{ fontWeight: 1000, fontSize: 20, marginTop: 4 }}>{activeOverdueFollowupItem.name}</div>
                <div style={{ fontSize: 13, opacity: 0.82, marginTop: 3 }}>{activeOverdueFollowupItem.code} • {activeOverdueFollowupItem.address || 'PA ADRESË'}</div>
              </div>
              <div style={{ textAlign: 'right', minWidth: 92 }}>
                <div style={{ ...overdueFollowupBadgeStyle, display: 'inline-flex' }}>{activeOverdueFollowupItem.pastrimAgeDays} DITË</div>
                <div style={{ fontSize: 11, opacity: 0.68, marginTop: 6 }}>{overdueFollowupItems.length} për ndjekje</div>
              </div>
            </div>

            <div style={{ borderRadius: 16, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(248,113,113,0.28)', padding: 12, marginBottom: 12 }}>
              <div style={{ fontSize: 15, fontWeight: 950, lineHeight: 1.35 }}>
                Kjo porosi ka {activeOverdueFollowupItem.pastrimAgeDays} ditë prej PASTRIM dhe ende është GATI për klient.
              </div>
              <div style={{ fontSize: 13, opacity: 0.86, lineHeight: 1.45, marginTop: 6 }}>
                Zgjedh arsyen pse nuk është marrë për dërgim. Pas zgjedhjes hapet Smart Message me tekst të gatshëm për klientin.
              </div>
            </div>

            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, opacity: 0.78, marginBottom: 6, fontWeight: 900, letterSpacing: 0.5 }}>SHËNIM OPSIONAL</div>
              <input
                type="text"
                value={overdueFollowupNote}
                onChange={(e) => setOverdueFollowupNote(e.target.value)}
                placeholder="p.sh. thirrur 2 herë / premtoi nesër / nuk hap telefonin"
                style={riplanInputStyle}
              />
            </div>

            <div style={{ display: 'grid', gap: 10 }}>
              {OVERDUE_FOLLOWUP_REASONS.map((reason) => (
                <button
                  key={reason.key}
                  type="button"
                  disabled={savingOverdueFollowup}
                  onClick={() => runDeferred(() => saveOverdueFollowup(activeOverdueFollowupItem, reason))}
                  style={{
                    width: '100%',
                    borderRadius: 16,
                    border: '1px solid rgba(255,255,255,0.12)',
                    ...
                    padding: '12px 12px',
                    textAlign: 'left',
                    opacity: savingOverdueFollowup ? 0.6 : 1,
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
                    gap: 10,
                ...
                >
                  <span style={{ minWidth: 0 }}>
                    <span style={{ display: 'block', fontWeight: 1000, fontSize: 14 }}>{reason.label}</span>
                    <span style={{ display: 'block', fontWeight: 800, fontSize: 12, opacity: 0.72, marginTop: 3 }}>{reason.helper}</span>
                  </span>
                  <span style={{ flexShrink: 0, fontWeight: 1000, color: '#bfdbfe', fontSize: 12 }}>{savingOverdueFollowup ? 'DUKE RUAJTUR…' : 'QO MESAZH'}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
              ...
              <button style={{ ...riplanBtn, opacity: savingKthim ? 0.6 : 1 }} disabled={savingKthim} onClick={() => runDeferred(async () => {
                ...
                try {
                  ...
                  await updateTransportOrderById(toolsRow.id, { status: 'pastrim', updated_at: nowIso, data: nextData });
                  try { window.dispatchEvent(new CustomEvent('transport:refresh')); } catch {}
                  if (kthimPhotoObjectUrlRef.current) {
                    try { URL.revokeObjectURL(kthimPhotoObjectUrlRef.current); } catch {}
                    kthimPhotoObjectUrlRef.current = '';
