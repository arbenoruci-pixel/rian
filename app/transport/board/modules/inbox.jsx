...
function rawOrderCode(order) {
  return String(
    order?.data?.linked_display_code ||
      ...
      order?.data?.order_tcode ||
      order?.data?.official_order_code ||
      order?.client_tcode ||
...
const TRANSPORT_BOARD_LOCAL_TZ = 'Europe/Belgrade';

function pad2(value) {
  return String(value || '').padStart(2, '0');
}

function inboxLocalYmd(date = new Date()) {
  try {
    const parts = new Intl.DateTimeFormat('en-CA', {
      timeZone: TRANSPORT_BOARD_LOCAL_TZ,
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
...
function inboxAddDaysYmd(ymd, days) {
  try {
    const [year, month, day] = String(ymd || '').split('-').map((v) => Number(v));
    if (!year || !month || !day) return inboxLocalYmd();
    const dt = new Date(Date.UTC(year, month - 1, day + Number(days || 0), 12, 0, 0));
    return dt.toISOString().slice(0, 10);
  } catch (_) {
    return inboxLocalYmd();
  }
}

function parseInboxPickupYmd(raw) {
  ...
  if (local) {
    ...
    return `${year}-${pad2(local[2])}-${pad2(local[1])}`;
  }

  try {
    const dt = new Date(text);
    if (!Number.isNaN(dt.getTime())) return inboxLocalYmd(dt);
  } catch (_) {}

  return '';
}

function getInboxPickupYmd(order) {
  const data = order?.data && typeof order.data === 'object' && !Array.isArray(order.data) ? order.data : {};
  return parseInboxPickupYmd(order?.pickup_date || data?.pickup_date || '');
}

function cleanInboxWindow(value) {
  return String(value || '')
    .trim()
    .replace(/\s+/g, ' ')
    .replace(/\s*[-–—]\s*/g, '–');
...
function getInboxPickupWindow(order) {
  const data = order?.data && typeof order.data === 'object' && !Array.isArray(order.data) ? order.data : {};
  const rawWindow = cleanInboxWindow(order?.pickup_window || data?.pickup_window || '');
  if (rawWindow) return rawWindow;

  const slot = String(order?.pickup_slot || data?.pickup_slot || '').trim().toLowerCase();
  if (!slot) return '';
  if (/morning|paradite|mengjes|mëngjes|am|09|9/.test(slot)) return '09:00–13:00';
  if (/evening|pasdite|mbrem|mbrëm|pm|18|6/.test(slot)) return '18:00–21:00';
  return cleanInboxWindow(slot);
}

function formatInboxOtherDate(ymd) {
  try {
    const [year, month, day] = String(ymd || '').split('-').map((v) => Number(v));
...
function inboxPickupDateBadge(order) {
  const injected = String(order?.__pickupDateBadge || '').trim();
  if (injected) return injected;

  const pickupYmd = getInboxPickupYmd(order);
  const windowLabel = getInboxPickupWindow(order);
  if (!pickupYmd) return windowLabel ? `PA DATË • ${windowLabel}` : 'PA DATË';

  const todayYmd = inboxLocalYmd();
  const tomorrowYmd = inboxAddDaysYmd(todayYmd, 1);
  let label = 'DATA TJETËR';
  if (pickupYmd === todayYmd) label = 'SOT';
  else if (pickupYmd === tomorrowYmd) label = 'NESËR';
  else {
    const formatted = formatInboxOtherDate(pickupYmd);
    if (formatted) label = `DATA TJETËR • ${formatted}`;
  }
