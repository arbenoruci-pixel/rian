...
function getConcreteReadySlots(row = {}) {
  return normalizeRackSlots([
    ...(Array.isArray(row?.ready_slots) ? row.ready_slots : []),
    ...(Array.isArray(row?.fullOrder?.ready_slots) ? row.fullOrder.ready_slots : []),
    row?.ready_location,
...
function hasConcreteReadyRack(row = {}) {
...
function buildConcreteRackRequiredMessage(prefix = 'Nuk lejohet me vazhdu.') {
  return `${prefix} Vendos raftin konkret: A1-A50, B1-B20, ose FURRA POSHT/NALT me A1-A40. Vetëm “FURRA POSHT” ose shënim pa raft nuk pranohet.`;
}

function mapBaseCacheRowToGati(row) {
  const data = row?.data && typeof row.data === 'object' ? row.data : {};
  const resolvedStatus = normalizeGatiStatus(getDbTruthStatus(row) || data?.status || '');
...
function mapLocalOrderToGatiRow(sourceRow = {}) {
  ...
  return {
    ...
    _local: order._local === true,
    _synced: order._synced,
    _syncPending: order._syncPending === true,
    _syncFailed: order._syncFailed === true,
    pending_ops: order.pending_ops || 0,
    readyNote: readyMeta.readyNote,
    ready_location: readyMeta.readyLocation,
    ready_note_text: readyMeta.readyText,
    ready_slots: readyMeta.readySlots,
    ...
    const key = id && isPersistedDbLikeId(id)
      ...
      : localOid
        ...
        : id
          ? `tmp:${id}`
          : code
            ? `display:${code}:${String(row?.readyTs || row?.ts || row?.updated_at || '')}:${String(row?.name || row?.fullOrder?.client_name || '')}`
            : '';
    if (!key) continue;
...
function GatiPageInner() {
  ...
  const [auditCleanupMsg, setAuditCleanupMsg] = useState('');
    ...
    try {
      ...
      if (isOffline) {
        ...
        const visibleOfflineRows = dedupeGatiSnapshotRows([
          ...
          .map((row) => ({
            ...(row || {}),
            source: String(row?.source || '').toUpperCase() === 'OUTBOX'
              ? 'LOCAL / JO I SYNC / OFFLINE'
              : 'LOCAL / OFFLINE',
          }))
      ...
      const shouldReadLocalWarm = !onlineAtStart && currentRows.length === 0 && (!Array.isArray(syncSnapshot) || syncSnapshot.length === 0);
      ...
      const dedupeOrders = (rows) => {
        ...
        const identityKeys = (row) => {
          ...
          if (!keys.length && code) keys.push(`display:${code}:${rowFreshness(row)}:${String(row?.name || '')}`);
      ...
      if (!data) {
        ...
        const localProblemCandidates = (Array.isArray(localRows) ? localRows : [])
          .filter((row) => isGatiRowLike(row))
          .map(mapLocalOrderToGatiRow);
        const problemRows = dedupeOrders([
          ...localProblemCandidates,
          ...(Array.isArray(warmRows) ? warmRows : []),
          ...(Array.isArray(syncSnapshot) ? syncSnapshot : []),
        ])
          .filter((row) => {
            const tokens = getGatiRowMatchTokens(row);
            if (tokens.some((token) => healthyDbTokenSet.has(token))) return false;
            if (isTerminalRecoveryGhostRow(row, recoveryIndex)) return false;
            const id = String(row?.id || row?.local_oid || row?.oid || '').trim();
            const source = String(row?.source || '').trim().toUpperCase();
            return (
              ...
              Number(row?.pending_ops || 0) > 0 ||
              !isPersistedDbLikeId(id)
            );
          })
          .map((row) => ({
            ...(row || {}),
            source: String(row?.source || '').trim().toUpperCase() === 'OUTBOX'
              ? 'LOCAL / JO I SYNC / PROBLEM'
              : 'LOCAL / NEEDS VERIFY',
          }))
          .sort((a, b) => (Number(b?.readyTs || b?.ts || 0) - Number(a?.readyTs || a?.ts || 0)));

        setLocalProblemRows(problemRows);
        finalRows = healthyDbRows.sort((a, b) => (Number(b?.readyTs || b?.ts || 0) - Number(a?.readyTs || a?.ts || 0)));
        scheduleDeferredCachePersist(rowsForCache, 4200);

        if (localPendingTimer.current) {
          clearTimeout(localPendingTimer.current);
  ...
  const auditCleanupSummary = useMemo(() => {
    ...
    for (const row of Array.isArray(orders) ? orders : []) {
      const audit = readAuditState(row?.fullOrder || row || {}, row || {});
      const status = normalizeAuditStatus(audit?.status || '');
      const auditedToday = status === AUDIT_STATUS.SEEN_IN_DEPOT && audit?.audited_at && sameCalendarDay(audit.audited_at, today);
      if (auditedToday) {
        out.verifiedToday += 1;
        continue;
      }

      if (status === AUDIT_STATUS.SEEN_IN_DEPOT) out.verifiedOld += 1;
      if (!status || status === AUDIT_STATUS.UNVERIFIED) out.noVerification += 1;
      out.targets.push({ ...row, audit, cleanup_reason: status === AUDIT_STATUS.SEEN_IN_DEPOT ? 'old_verification' : 'not_verified_today' });
    }
  ...
  async function runGatiAuditCleanup() {
    if (auditCleanupBusy) return;
    const targets = Array.isArray(auditCleanupSummary?.targets) ? auditCleanupSummary.targets : [];
    if (!targets.length) {
      setAuditCleanupMsg('Nuk ka rreshta për cleanup. Vetëm verifikimet e sotme mbesin në GATI.');
      return;
    }
    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
      alert('Cleanup i GATI duhet të bëhet online që statusi dhe audit trail të ruhen në DB.');
      return;
    }

    const conf = window.confirm(
      `Cleanup GATI: ${targets.length} rreshta që nuk janë verifikuar sot do kalohen në DORZIM.\n\n` +
      `Mbesin në GATI vetëm: ${auditCleanupSummary.verifiedToday} të verifikuara sot.\n` +
      `Çdo ndryshim ruan audit_cleanup + backup në data. Vazhdoni?`
    );
    if (!conf) return;

    const actor = readActor() || {};
    const byName = String(actor?.name || actor?.pin || actor?.transport_id || 'STAF').trim();
    const byPin = String(actor?.pin || actor?.transport_id || actor?.id || '').trim();
    ...
    setAuditCleanupMsg('Cleanup në progres...');
    try {
      for (const row of targets) {
        const id = String(row?.id || row?.fullOrder?.id || '').trim();
        if (!isPersistedDbLikeId(id)) { skipped += 1; continue; }

        let fresh = null;
        try {
          fresh = await fetchOrderByIdSafe('orders', Number(id), 'id,local_oid,code,status,client_name,client_phone,updated_at,created_at,data', { timeoutMs: GATI_DB_TIMEOUT_MS });
        } catch {
          skipped += 1;
          continue;
        }
        const currentData = unwrapGatiOrder(fresh?.data || row?.fullOrder || row || {});
        const currentStatus = String(fresh?.status || currentData?.status || row?.status || '').trim().toLowerCase();
        if (currentStatus !== 'gati') { skipped += 1; continue; }

        const freshAudit = readAuditState(currentData, fresh || row || {});
        const status = normalizeAuditStatus(freshAudit?.status || '');
        const verifiedToday = status === AUDIT_STATUS.SEEN_IN_DEPOT && freshAudit?.audited_at && sameCalendarDay(freshAudit.audited_at, nowIso);
        if (verifiedToday) { skipped += 1; continue; }

        const cleanupEntry = {
          id: `gati_cleanup_${Date.now()}_${id}`,
          action: 'GATI_AUDIT_CLEANUP_TO_DORZIM',
          reason: row?.cleanup_reason || (status === AUDIT_STATUS.SEEN_IN_DEPOT ? 'old_verification' : 'not_verified_today'),
          at: nowIso,
          by_name: byName,
          by_pin: byPin,
          previous_status: 'gati',
          next_status: 'dorzim',
          audit_snapshot: freshAudit,
          backup: {
            id: fresh?.id || id,
            code: normalizeCode(fresh?.code || currentData?.code || row?.code || ''),
            status: fresh?.status || 'gati',
            updated_at: fresh?.updated_at || null,
            data: currentData,
          },
        };

        const nextHistory = Array.isArray(currentData?.audit_cleanup_history) ? currentData.audit_cleanup_history : [];
        const nextData = {
          ...(currentData || {}),
          status: 'dorzim',
          state: 'dorzim',
          audit_cleanup: cleanupEntry,
          audit_cleanup_history: [cleanupEntry, ...nextHistory].slice(0, 50),
        };

        await transitionOrderStatus('orders', Number(id), 'dorzim', {
          updated_at: nowIso,
          data: nextData,
        });
        ...
        const localPatch = {
          ...fresh,
          id,
          local_oid: String(fresh?.local_oid || currentData?.local_oid || currentData?.oid || id),
          status: 'dorzim',
          updated_at: nowIso,
          data: nextData,
          table: 'orders',
      ...
      if (doneIds.size) {
        setOrders((prev) => (prev || []).filter((row) => !doneIds.has(String(row?.id || ''))));
      }
      setAuditCleanupMsg(`Cleanup GATI përfundoi: ${done} u kaluan në DORZIM, ${skipped} u lanë pa prekur.`);
      await refreshOrders('gati_audit_cleanup');
    } catch (e) {
      setAuditCleanupMsg(`Gabim gjatë cleanup: ${String(e?.message || e || '')}`);
    } finally {
  ...
  async function sendPickupSms(row) {
    if (!hasConcreteReadyRack(row)) {
      alert(buildConcreteRackRequiredMessage('SMS nuk lejohet.'));
  ...
  function showFastPayNotice(text, tone = 'ok', ttlMs = 2800) {
    ...
    setFastPayNotice({ id, text: msg, tone });
    try {
      window.setTimeout(() => {
        setFastPayNotice((current) => (current?.id === id ? null : current));
      }, Math.max(1200, Number(ttlMs || 2800)));
    } catch {}
  }

  function buildFastBasePaymentIdempotencyKey({ orderId, amount, actorPin } = {}) {
    return buildArkaIdempotencyKey(ARKA_ACTION.BASE_ORDER_PAYMENT, [
      orderId,
  ...
  function buildFastPaymentTransaction({ payload, amount, pinData, method, idempotencyKey } = {}) {
    const amt = Number(Number(amount || 0).toFixed(2));
    return {
      action: ARKA_ACTION.BASE_ORDER_PAYMENT,
      actorPin: String(pinData?.pin || '').trim(),
      actorName: pinData?.name ? String(pinData.name) : null,
      actorRole: pinData?.role ? String(pinData.role) : null,
      orderId: payload?.id || payload?.orderId || payload?.order_id || null,
      amount: amt,
      method: String(method || 'CASH').toUpperCase(),
      note: payload?.payment_note || `PAGESA ${amt}€ • #${payload?.code || payload?.order_code || ''} • ${payload?.client_name || payload?.client?.name || ''}`.trim(),
  ...
  async function queueFastPaymentTransaction(transaction) {
    if (!transaction?.action || !transaction?.idempotencyKey) return null;
    return queueOp('arka_transaction', {
      transaction: {
        ...transaction,
        _queued_at: new Date().toISOString(),
        _queue_reason: 'gati_fast_confirm',
      },
  ...
  function withOptimisticArkaRecordedPaid(payload, applied) {
    const amt = Number(Number(applied || 0).toFixed(2));
    if (!payload || !amt) return payload;
    const pay = (payload.pay && typeof payload.pay === 'object') ? payload.pay : {};
    const arkaRecordedPaid = Number((Number(pay.arkaRecordedPaid || 0) + amt).toFixed(2));
    return {
      ...payload,
      pay: {
        ...pay,
    ...
    <div className="wrap">
      {fastPayNotice ? (
        <div
          ...
          style={{
            ...
            padding: '13px 14px',
            borderRadius: 16,
            border: fastPayNotice.tone === 'warn' ? '1px solid rgba(245,158,11,0.45)' : '1px solid rgba(34,197,94,0.42)',
            background: fastPayNotice.tone === 'warn' ? 'rgba(120,53,15,0.96)' : 'rgba(6,78,59,0.96)',
            color: '#fff',
            fontWeight: 1000,
            fontSize: 14,
            textAlign: 'center',
      ...
      {auditMode && (
        <div className="card" style={{ padding: 12, marginTop: 10, display: 'grid', gap: 10 }}>
          ...
          <div style={{ border: '1px solid rgba(251,191,36,0.22)', borderRadius: 12, padding: 10, background: 'rgba(251,191,36,0.08)', display: 'grid', gap: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 1000, color: '#fde68a' }}>AUDITED CLEANUP</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.72)' }}>Mban vetëm të verifikuarat sot; të tjerat kalohen në DORZIM me audit trail.</div>
              </div>
              <button
                type="button"
                disabled={auditCleanupBusy || auditCleanupSummary.targets.length === 0}
                onClick={runGatiAuditCleanup}
                style={{ background: 'rgba(251,191,36,0.18)', border: '1px solid rgba(251,191,36,0.42)', color: '#fef3c7', padding: '8px 10px', borderRadius: 10, fontWeight: 1000, fontSize: 11 }}
              >{auditCleanupBusy ? 'DUKE PASTRUAR...' : `CLEANUP ${auditCleanupSummary.targets.length}`}</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 6 }}>
              <div style={{ fontSize: 11 }}>Verifikuar sot: <b>{auditCleanupSummary.verifiedToday}</b></div>
              <div style={{ fontSize: 11 }}>Verifikim i vjetër: <b>{auditCleanupSummary.verifiedOld}</b></div>
              <div style={{ fontSize: 11 }}>Pa verifikim: <b>{auditCleanupSummary.noVerification}</b></div>
              <div style={{ fontSize: 11 }}>Target cleanup: <b>{auditCleanupSummary.targets.length}</b></div>
            </div>
            {auditCleanupMsg ? <div style={{ fontSize: 11, color: '#fef3c7' }}>{auditCleanupMsg}</div> : null}
          </div>
        </div>
      )}
      ...
      {localProblemRows.length > 0 && (
        <section className="card" style={{ padding: 12, marginTop: 10, display: 'grid', gap: 8, border: '1px solid rgba(251,191,36,0.35)', background: 'rgba(120,53,15,0.18)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ fontSize: 13, fontWeight: 900, color: '#fde68a' }}>LOCAL / JO I SYNC / PROBLEM</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.68)', fontWeight: 800 }}>Nuk janë në listën normale GATI pa konfirmim nga DB.</div>
          </div>
          <div style={{ display: 'grid', gap: 8 }}>
            {localProblemRows.slice(0, 12).map((row) => (
              <div
                key={`local_problem_${row.id || row.local_oid || row.code || row.readyTs}`}
                style={{
                  width: '100%',
                  textAlign: 'left',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}>
                  <div style={{ fontWeight: 900 }}>NR {normalizeCode(row.code) || '—'} • {row.name || 'Pa emër'}</div>
                  <div style={{ fontSize: 11, color: '#fde68a', fontWeight: 900 }}>{row.source || 'LOCAL'}</div>
                </div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.72)', marginTop: 4 }}>
                  ID: {row.id || row.local_oid || 'pa ID'} • Telefoni/cache e ka GATI, DB nuk e ka konfirmuar në listën normale.
                </div>
              </div>
            ))}
          </div>
