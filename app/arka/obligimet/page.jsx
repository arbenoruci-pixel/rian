...
import { isPayrollEligibleWorker } from '@/lib/payrollMonthClose';
import {
...
function stableBudgetToken(v = '') {
  return String(v ?? '').trim().toUpperCase().replace(/[^A-Z0-9_.:-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 80) || 'X';
}
function makeBudgetClientActionId({ actor, title, amount, category }) {
  const base = [
    'MANUAL_BUDGET_SPEND',
    stableBudgetToken(actor?.pin || 'NO_PIN'),
    stableBudgetToken(category || 'SHPENZIM'),
    stableBudgetToken(Number(amount || 0).toFixed(2)),
    stableBudgetToken(title || 'NO_TITLE'),
  ].join(':');
...
function summaryFrom({ users = [], summaryRow = null, fixedExpenses = [], ledger = [] } = {}) {
  const payrollUsers = (users || []).filter((u) => isPayrollEligibleWorker(u) && n(u?.salary) > 0);
...
export default function ArkaObligimetPage() {
  ...
  const budgetActionRef = useRef(null);
  const budgetSubmitRef = useRef(false);
  ...
  useEffect(() => {
    budgetActionRef.current = null;
  }, [budgetTitle, budgetAmount, budgetCategory, actor?.pin]);
  ...
  async function addBudgetExpense() {
    if (budgetBusy || budgetSubmitRef.current) return;
    ...
    const clientActionId = budgetActionRef.current || makeBudgetClientActionId({
      actor,
      title: cleanTitle,
      amount: cleanAmount,
      category: budgetCategory || 'SHPENZIM',
    });
    budgetActionRef.current = clientActionId;
    budgetSubmitRef.current = true;
    try {
      setBudgetBusy(true);
      await spendFromCompanyBudget({
        ...
        sourceType: 'manual_budget_spend',
        clientActionId,
        ...
        <>
          ...
          <div className="arkaSplitGrid detailPage">
            <section className="arkaSectionCard">
              ...
              <div className="arkaSectionSub">KJO LEXOHET DIREKT NGA PAYROLL-I — VETËM STAFI AKTIV</div>
