...
function normalizeWorkerExpenseRequest({ requestKind, actorPin, actorName, beneficiaryPin, beneficiaryName }) {
  ...
  if (kind === 'PERSONAL_SELF') {
    if (!selfPin) return { error: '🔴 MUNGON PIN-I I PUNËTORIT PËR PERSONAL / AVANS.' };
...
function CashClientCompactRow({ row = {}, workerName = 'PUNTORI', mini = false }) {
  ...
  const name = String(row?.clientName || 'KLIENT').toUpperCase();
  const gross = n(row?.gross);
  const commission = n(row?.commission);
  const due = n(row?.dueToBase);
  const type = String(row?.type || 'CASH').toUpperCase();
  ...
  return (
    <details className={`arkaCashCompactRow ${mini ? 'mini' : ''}`}>
      ...
      <div className="arkaCashCompactDetails"><span>{type}</span><span>KLIENTI: {euro(gross)}</span>{commission > 0 ? <span>{String(workerName || 'PUNTORI').toUpperCase()}: {euro(commission)}</span> : null}<span>PËR BAZË: {euro(due)}</span></div>
    </details>
  );
}
...
function cashDueTransportDedupeKey(row = {}) {
  const raw = row?.raw || row;
  const id = String(raw?.transport_order_id || raw?.transportOrderId || row?.transport_order_id || row?.transportOrderId || '').trim();
  const code = extractTransportCodeCandidate(raw) || extractTransportCodeCandidate(row) || String(row?.code || '').trim().toUpperCase();
  if (id) return `transport_order_id:${id}`;
  if (/^T\d+$/.test(code)) return `transport_code:${code}`;
  return '';
}

function preferCashDueDedupeRow(current = {}, candidate = {}) {
...
function dedupeCashDueTransportRows(rows = []) {
  const output = [];
  const indexByKey = new Map();
  const duplicateKeys = new Set();
  for (const row of Array.isArray(rows) ? rows : []) {
    const key = String(row?.type || '').toUpperCase() === 'TRANSPORT' ? cashDueTransportDedupeKey(row) : '';
    if (!key) {
      output.push(row);
      continue;
    }
    ...
    output[idx] = preferCashDueDedupeRow(output[idx], row);
  }
  return {
    rows: output,
...
export default function ArkaWorkerDetailPage() {
  ...
  const cashAccount = useMemo(() => {
    ...
    const rawOpenRows = sortRowsDesc(allPayments.filter((row) => isOpenForDispatch(row, pin))).map((row) => buildCashDueRow(row, dueOptions));
    const openRowsDedupe = dedupeCashDueTransportRows(rawOpenRows);
    const openRows = openRowsDedupe.rows;
    const collectedOpenRows = openRows.filter((row) => safeUpper(row?.status) === 'COLLECTED');
    const dispatchOpenRows = openRows.filter((row) => OPEN_FOR_DISPATCH_STATUSES.has(safeUpper(row?.status)));
    ...
    const acceptedHistoryGrossTotal = historyRows.reduce((sum, row) => sum + n(row?.gross), 0);
    const acceptedHistoryBaseTotal = historyRows.reduce((sum, row) => sum + n(row?.dueToBase), 0);
    const acceptedHistoryCommissionTotal = historyRows.reduce((sum, row) => sum + n(row?.commission), 0);
    const visiblePaidHistoryTotal = +(openRows.reduce((sum, row) => sum + n(row?.gross), 0) + acceptedHistoryGrossTotal).toFixed(2);
    const visibleCommissionHistoryTotal = +(transportOpenRows.reduce((sum, row) => sum + n(row?.commission), 0) + acceptedHistoryCommissionTotal).toFixed(2);
    const collectedDueBeforeExpenses = collectedOpenRows.reduce((sum, row) => sum + n(row?.dueToBase), 0);
    ...
    return {
      ...
      collectedOpenRows,
      dispatchOpenRows,
      duplicateTransportCashCount: openRowsDedupe.duplicateCount,
      duplicateTransportCashKeys: openRowsDedupe.duplicateKeys,
      collectedDueBeforeExpenses,
      collectedDueToBase: Math.max(0, +collectedDueBeforeExpenses.toFixed(2)),
      baseOpenRows,
      transportOpenRows,
      historyRows,
      acceptedHistoryGrossTotal,
      acceptedHistoryBaseTotal,
      acceptedHistoryCommissionTotal,
  ...
  const cashRemainingToHandOver = cashAccount.totalDueToBase;
  ...
  function handleExpenseBeneficiaryPinChange(value) {
    ...
    const row = (staffOptions || []).find((item) => String(item?.pin || '').trim() === cleanPin);
    if (row) setExpenseBeneficiaryName(String(row?.name || cleanPin).trim());
  }
  ...
  async function addExpense() {
    ...
    try {
      ...
      await createExpenseEntry({
        actor,
        amount,
        note: buildExpenseRequestNote(expenseNote || 'SHPENZIM', request),
        workerPin: pin,
        workerName: worker?.name || pin,
        workerRole: worker?.role || 'WORKER',
      });
  ...
  async function addAdvance() {
    ...
    try {
      ...
      await arkaTransaction({
        ...
        actorPin: adminPin || workerPin,
        actorName: adminName || workerName,
        ...
        idempotencyKey: buildArkaIdempotencyKey(ARKA_ACTION.EXPENSE_REQUEST, [workerPin, 'ADVANCE', amount]),
      });
...
` : ''}${count} KLIENTË ME CASH I MARRË.`);
  ...
  const ownerDeliveredRows = sortRowsDesc((summary.deliveredRows || []).filter((row) => ACCEPTED_HISTORY_STATUSES.has(safeUpper(row?.status))));
  ...
  const openPaymentGroups = buildPaymentDisplayGroups(cashAccount.dispatchOpenRows);
        ...
        <>
          ...
          {canManage ? (
            <details className="arkaAdvancedDetails">
              <summary className="arkaAdvancedSummary">HAP ADMIN / AVANS</summary>
          ...
          <details className="arkaAdvancedDetails">
            ...
            <section className="arkaSectionCard">
              ...
              <div className="arkaSectionSub">KOMISIONI ËSHTË HESAP PAYROLL. NË EKRANIN KRYESOR SHFAQET VETËM SA MBAHET NGA CASH-I.</div>
              <div className="arkaWorkerStats adminTopGrid ownerTotalsGrid">
          ...
          {(canManage || sameWorker) ? (
            <div className="arkaActionPanel">
              ...
              <div className="arkaSimpleSub">PUNTORI PROPOZON TIPIN. ADMIN / DISPATCH E KONFIRMON VENDIMIN FINAL.</div>
              <div className="arkaInlineForm">
              ...
              {expenseRequestType === 'PERSONAL_OTHER' ? (
                <div className="arkaExpenseBeneficiaryGrid">
                  <div>
                    ...
                    <input className="arkaField" list="puntor_expense_beneficiaries" value={expenseBeneficiaryPin} onChange={(e) => handleExpenseBeneficiaryPinChange(e.target.value)} placeholder="P.SH. 2020" />
                    <datalist id="puntor_expense_beneficiaries">
                      {(staffOptions || []).map((row) => <option key={String(row?.pin || '')} value={String(row?.pin || '')}>{row?.name || row?.pin}</option>)}
                    </datalist>
                  </div>
                  <div>
                    <label>EMRI</label>
              ...
              {expenseRequestType === 'PERSONAL_SELF' ? <div className="arkaExpenseWarning warn">KËRKESA DO RUAHET SI PERSONAL / AVANS PËR KËTË PUNËTOR.</div> : null}
          ...
          <div className="arkaSplitGrid detailPage">
            <section className="arkaSectionCard">
              ...
              <div className="arkaSectionSub">COLLECTED/PENDING JANË CASH AKTIV. ACCEPTED_BY_DISPATCH SHFAQET SI HISTORI E DORËZUAR.</div>
              <div className="arkaWorkerStats arkaSectionTopStats">
                <Stat label="TOTAL COLLECTED" value={euro(summary.collectedTotal)} tone="ok" />
                <Stat label="TOTAL PENDING" value={euro(summary.pendingTotal)} tone="warn" />
                <Stat label="HISTORI PRANUAR" value={euro(cashAccount.acceptedHistoryGrossTotal)} tone="neutral" />
