...
async function dispatchTableHasCode(table, code, selector) {
  const c = normTCode(code);
  const n = String(tCodeNumber(c) || '').trim();
  if (!c || !n) return true;
  try {
...
async function dispatchCodeHasRealHistory(code) {
  ...
  const hasOrder = await dispatchTableHasCode('transport_orders', c, (row, codeStr, codeNum) => {
    ...
    return values.some((value) => {
      ...
      if (!raw) return false;
      return normTCode(raw) === codeStr || raw.replace(/\D+/g, '') === codeNum;
    });
  });
  if (hasOrder) return true;

  const hasPayment = await dispatchTableHasCode('arka_pending_payments', c, (row, codeStr, codeNum) => {
    if (String(row?.type || '').toUpperCase() !== 'TRANSPORT') return false;
    const values = [row?.transport_code_str, row?.order_code];
    return values.some((value) => {
      const raw = String(value ?? '').trim();
      if (!raw) return false;
      return normTCode(raw) === codeStr || raw.replace(/\D+/g, '') === codeNum;
  ...
  try {
    ...
    const { data: directClientHits, error: directClientErr } = await supabase
      .from('transport_clients')
      .select('id,tcode,code,client_code,code_str,name,phone,phone_digits')
      .or(clientFilters)
      .limit(1);
    if (!directClientErr && Array.isArray(directClientHits) && directClientHits.length > 0) {
      return true;
    }
  } catch {}

  const hasClient = await dispatchTableHasCode('transport_clients', c, (row, codeStr, codeNum) => {
    const data = row?.data && typeof row.data === 'object' && !Array.isArray(row.data) ? row.data : {};
    const values = [
      row?.tcode,
      row?.code,
...
async function dispatchClaimTransportCode(code, owner) {
  const c = normTCode(code);
  ...
  try {
    for (let startN = 1; startN <= maxProbe; startN += batchSize) {
      const codes = Array.from(
        { length: Math.min(batchSize, maxProbe - startN + 1) },
        (_, idx) => `T${startN + idx}`
      );
      ...
      const candidates = rows
        ...
        .filter((x) => x.code && x.n > 0)
        .sort((a, b) => a.n - b.n);

      for (const item of candidates) {
        const hasHistory = await dispatchCodeHasRealHistory(item.code);
        if (hasHistory) continue;

        const claimed = await dispatchClaimTransportCode(item.code, owner);
        if (!claimed) continue;

        try {
          const cacheKey = oid ? `transport_order_code_v1__${oid}` : '';
          if (cacheKey && typeof localStorage !== 'undefined') {
            localStorage.setItem(cacheKey, item.code);
          }
        } catch {}
        return item.code;
      }
    ...
    console.warn('DISPATCH_DIRECT_LOW_WINDOW_TCODE_FAILED', error);
  }

  // Last fallback only. Normal Dispatch should not reach here when low pool has safe available codes.
  return normTCode(await reserveTransportCode(owner || 'DISPATCH', opts));
}


const DISPATCH_CODE_BUFFER_TARGET = 3;
...
async function warmDispatchCodeBuffer(owner = 'DISPATCH', opts = {}) {
  ...
  try { await releaseExpiredDispatchBufferCodes(bufferOwner); } catch {}
  ...
  for (let i = 0; i < 8 && buffer.length < target; i += 1) {
    const holdOwner = dispatchCodeHoldOwner(bufferOwner);
    const code = normTCode(await reserveDispatchSmallestTransportCode(holdOwner, { oid: `dispatch_buffer_${bufferOwner}_${Date.now()}_${i}` }));
...
async function getDispatchBufferedTransportCode(owner = 'DISPATCH', opts = {}) {
  const bufferOwner = String(owner || 'DISPATCH').trim() || 'DISPATCH';
  const buffered = popDispatchBufferedCode(bufferOwner);
  if (buffered) return buffered;

  // Emergency path: only if background warm-up did not have a ready code.
  const holdOwner = dispatchCodeHoldOwner(bufferOwner);
  return normTCode(await reserveDispatchSmallestTransportCode(holdOwner, opts));
}

async function ensureDispatchTransportClientLink({ name, phone, address, existingPhoneClient, tcodeOwner, reservedOrderCode }) {
...
export default function DispatchPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try { void warmDispatchCodeBuffer(bufferOwner); } catch {}
    }, 450);
    return () => clearTimeout(t);
  }, [accessChecked, accessAllowed]);
  ...
  async function send() {
    ...
    try {
      ...
      try {
        await markTransportCodeUsed(officialOrderCode, poolOwner);
      } catch {}
      ...
      try {
        void warmDispatchCodeBuffer(dispatchBufferOwner);
      } catch {}
      try {
        void loadRows();
      } catch {}
