...
function chipStyleForVal(v, active) {
  ...
  let top = '#8b5cf6';
  let bottom = '#6d28d9';
  let edge = 'rgba(196,181,253,0.55)';
  let glow = 'rgba(139,92,246,0.28)';

  if (n >= 8) {
    top = '#f59e0b';
    bottom = '#d97706';
    edge = 'rgba(253,230,138,0.62)';
    glow = 'rgba(245,158,11,0.32)';
  } else if (n >= 5) {
    top = '#fb923c';
    bottom = '#ea580c';
    edge = 'rgba(254,215,170,0.62)';
    glow = 'rgba(249,115,22,0.30)';
  } else if (n >= 3.5) {
    top = '#ec4899';
    bottom = '#db2777';
    edge = 'rgba(251,207,232,0.60)';
    glow = 'rgba(236,72,153,0.28)';
  } else if (n >= 2.0) {
    top = '#3b82f6';
    bottom = '#007aff';
    edge = 'rgba(191,219,254,0.60)';
    glow = 'rgba(0,122,255,0.30)';
  } else {
    top = '#34c759';
    bottom = '#16a34a';
    edge = 'rgba(187,247,208,0.62)';
    glow = 'rgba(52,199,89,0.28)';
  }

  return {
    background: `linear-gradient(180deg, rgba(255,255,255,0.16), rgba(255,255,255,0.03)), linear-gradient(180deg, ${top} 0%, ${bottom} 100%)`,
    border: `1.5px solid ${edge}`,
    outline: active ? '2px solid rgba(255,255,255,0.18)' : 'none',
    boxShadow: active
      ? `0 0 0 2px ${glow}, 0 10px 22px rgba(0,0,0,0.34), inset 0 1px 0 rgba(255,255,255,0.24)`
      : `0 8px 18px rgba(0,0,0,0.30), inset 0 1px 0 rgba(255,255,255,0.16)`,
    color: '#ffffff',
  };
}
  ...
  return (
    <div className="wrap">
      ...
      <style jsx>{`
        ...
        .chip-row.modern { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-top:-4px; align-items:stretch; }
        @media (min-width: 400px) { .chip-row.modern { grid-template-columns:repeat(4,minmax(0,1fr)); } }
        .chip-modern {
          position: relative;
          min-height: 64px;
          padding: 0 10px;
        ...
        .chip-modern:active { transform: scale(.965); filter: saturate(1.06); }
