...
async function fetchRemoteDraftsSummary() {
  ...
  const tasks = files.map(async (f) => {
    ...
    out.push({
      id: d.id,
      ts: d.ts || 0,
      code: formatKod(normalizeCode(d.codeRaw || d.code || ''), __isOnlineUI),
      m2,
      euro,
    });
  });
...
function ensureCodePair(obj){
  // Appi duhet me punu vetem me `code` numerik. Mos e dergo `code_n` ne DB/queue.
...
function formatKod(v, isOnline){
  ...
  // if we're online, we may be waiting for pool reserve
  return isOnline ? "…" : "—";
}
...
useEffect(() => {
  try {
    ...
    setOfflineMode(init);
    // FORCE_RESERVE_ON_MOUNT_V1
    (async () => {
      try {
        if (typeof navigator !== 'undefined' && navigator.onLine) {
          // ✅ Always warm BASE code pool when online.
          // NOTE: reserveSharedCode() takes ONLY (oid). Using it here with (pin, n) was a bug.
          try {
...
setCodeRaw(c || '');
  ...
  async function commitDraftAndAdvanceCodeBestEffort() {
    try {
      ...
      const n = Number(formatKod(normalizeCode(codeRaw), __isOnlineUI));
    ...
    try {
      ...
      if (!normCodeNow || normCodeNow === '0') {
        ...
        if (!browserOffline && conn?.ok) {
          try { console.warn('[PRANIMI] Missing code while online. Blocking save.', { conn }); } catch {}
        ...
        // Offline / DB unreachable: save to offline queue so you don’t lose the client.
...
${msg}${details}`);
  ...
  const kod = formatKod(normalizeCode(codeRaw), __isOnlineUI);
...
<div className="code-badge">
          <span className="badge">{`KODI: ${formatKod(normalizeCode(codeRaw), __isOnlineUI)}`}</span>
        </div>
      </header>
  ...
  setClientHits([]);
  try {
    const newCodeWeDontNeed = formatKod(codeRaw, __isOnlineUI);
    if (newCodeWeDontNeed && newCodeWeDontNeed !== '—') {
      await releaseLocksForCode(newCodeWeDontNeed);
    }
  } catch(e) {}
}}
