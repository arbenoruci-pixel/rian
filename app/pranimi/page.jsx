...
function readBaseCodePoolVerdict(row = null) {
  ...
  const reservedBy = String(row?.reserved_by || '').trim();
...
function evaluateDraftCodePoolGuard(item = {}, pool = {}) {
  ...
  if (code == null) {
    if (!remote && snapshotHasMeaningfulWork(item)) return { allow: true, reason: 'MEANINGFUL_DRAFT_NEEDS_OFFICIAL_ASSIGNMENT', code: null, reassign: true };
    return { allow: false, reason: 'NO_CODE' };
  }
  if (pool?.uncertain) {
  ...
  if (verdict.state === 'used') {
    ...
    // A code reported as used cannot be displayed or reused by this draft.
    // Preserve meaningful local/DB draft data and force official reassignment after DB verification;
    // storage-only/remote ghosts remain hidden so a finalized code is never painted as active.
    if (dbDraft && !remote) return { allow: true, reason: 'DB_DRAFT_POOL_USED_REQUIRES_REASSIGNMENT', code, row, reassign: true };
    if (!remote && snapshotHasMeaningfulWork(item)) return { allow: true, reason: 'LOCAL_MEANINGFUL_POOL_USED_REQUIRES_REASSIGNMENT', code, row, reassign: true };
  ...
  if (verdict.state === 'available') {
    ...
    if (!remote && snapshotHasMeaningfulWork(item)) return { allow: true, reason: 'LOCAL_MEANINGFUL_POOL_AVAILABLE_REQUIRES_REASSIGNMENT', code, row, reassign: true };
  ...
  if (!remote && snapshotHasMeaningfulWork(item)) return { allow: true, reason: `LOCAL_MEANINGFUL_CODE_POOL_${String(verdict.state || 'missing').toUpperCase()}_REQUIRES_REASSIGNMENT`, code, row, reassign: true };
...
async function filterDraftSummariesAgainstDb(items = []) {
  ...
  for (const item of input) {
    ...
    if (codePoolVerdict?.reassign) {
      appendPranimiCodeDebug('meaningful_draft_retained_for_official_code_reassignment', {
        local_oid: item?.local_oid || item?.id || null,
        rejected_code: codePoolVerdict?.code || item?.code || item?.codeRaw || null,
        reason: codePoolVerdict?.reason || null,
      });
    }
    if (codePoolVerdict?.rehold && !codePoolVerdict?.reassign && codePoolVerdict?.code != null) {
      void renewPranimiCodeLease(
        resolvePranimiActorPin(),
        item?.id || item?.local_oid || '',
        codePoolVerdict.code,
        true
      ).catch(() => ({ ok: false }));
    }
...
function friendlyCodeReserveMessage(raw = '') {
  ...
  if (!text) return '';
  if (text.includes('PIN')) return 'PIN-I I PUNËTORIT NUK U GJET';
  if (text.includes('NO_BASE_CODES') || text.includes('POOL_EMPTY') || text.includes('POOL-I DUHET') || text.includes("S'KA KOD")) {
    return 'POOL-I I KODEVE ËSHTË BOSH — PREK PROVO';
  }
  if (text.includes('TIMEOUT') || text.includes('LOAD FAILED') || text.includes('FAILED TO FETCH') || text.includes('NETWORK') || text.includes('ABORT')) {
    return 'LIDHJA ME SERVERIN DËSHTOI — PREK PROVO';
  }
  return 'KODI S’U MOR — PREK PROVO';
}
...
export default function PranimiPage() {
  ...
  const [codeReserveUi, setCodeReserveUi] = useState({ status: 'idle', message: '', raw: '' });
  ...
  const codeReserveStateRef = useRef({ oid: '', running: false, attempt: 0, lastError: '', lastReason: '' });
  ...
  async function runBackgroundMetaSync(reason = 'manual') {
    try {
      const liveActor = getActor() || actor || null;
      const activePin = resolvePranimiActorPin(liveActor);
      if (!activePin) {
        try { logDebugEvent('pranimi_bg_sync_no_actor_pin', { reason: String(reason || 'manual') }); } catch {}
        return;
      }
      const isOnline = (() => {
      ...
      // Startup can request a code before login warm-up / pool refill finishes.
      // Once the pool is ready, retry the active blank code immediately.
      const activeOid = String(oidRef.current || '').trim();
      const activeCode = String(normalizeCode(codeRawRef.current) || '').trim();
      if (activeOid && !activeCode && (poolStatus?.ok || epochChanged)) {
        tryReserveCodeInBackground(activeOid, `pool_ready_after_${String(reason || 'sync')}`, { attempt: 0, delayMs: 0, force: true });
      }
    } catch {}
    finally {
  ...
  function tryReserveCodeInBackground(nextOid, reason = 'session_open', options = {}) {
    const id = String(nextOid || '').trim();
    if (!id) return;
    const attempt = Math.max(0, Number(options?.attempt || 0) || 0);
    const delayMs = Number.isFinite(Number(options?.delayMs))
      ? Math.max(0, Number(options.delayMs))
      : PRANIMI_CODE_RESERVE_RETRY_DELAYS_MS[Math.min(attempt, PRANIMI_CODE_RESERVE_RETRY_DELAYS_MS.length - 1)];
    const state = codeReserveStateRef.current || {};
    if (state.running && String(state.oid || '') === id) return;
    try { if (startupCodeReserveTimerRef.current) clearTimeout(startupCodeReserveTimerRef.current); } catch {}

    const restoredCandidate = normalizeCode(codeRawRef.current ?? getAssignedPranimiCode(id));
    // Never paint a restored/local candidate while DB/proof verification is pending.
    codeRawRef.current = '';
    setCodeRaw('');
    codeReserveStateRef.current = { oid: id, running: false, attempt, lastReason: String(reason || 'session_open'), lastError: '' };
    setCodeReserveUi({ status: attempt > 0 ? 'retrying' : 'reserving', message: attempt > 0 ? `DUKE PROVU PRAP (${attempt + 1})` : 'DUKE VERIFIKU KODIN…', raw: '' });
    ...
    startupCodeReserveTimerRef.current = setTimeout(() => {
      void (async () => {
        try {
          ...
          if (!activePin) {
            const e = new Error('PIN-I REAL I PUNËTORIT MUNGON');
            e.code = 'MISSING_REAL_ACTOR_PIN';
            throw e;
          }
          codeReserveStateRef.current = { oid: id, running: true, attempt, lastReason: reason, lastError: '' };
          const result = await allocatePranimiCode(activePin, id, restoredCandidate, hasStartedWork());
          if (String(oidRef.current || '') !== id) return;
          const reserved = normalizeCode(result?.code);
          if (reserved == null) throw new Error('PRANIMI_CODE_EMPTY_AFTER_ALLOCATOR');
          codeRawRef.current = String(reserved);
          setCodeRaw(String(reserved));
          writeDraftReservationLocal({ local_oid: id, code: reserved, created_by_pin: activePin, has_meaningful_work: hasStartedWork(), reason: result?.source || 'ONE_WAY_ALLOCATOR' });
          setCodeReserveUi({ status: 'ready', message: '', raw: '' });
          codeReserveStateRef.current = { oid: id, running: false, attempt: 0, lastError: '', lastReason: reason };
          appendPranimiCodeDebug('one_way_code_ready', { local_oid: id, final_code: reserved, actor_pin: activePin, source: result?.source || null, reason });
        } catch (error) {
          if (String(oidRef.current || '') !== id) return;
          const errorText = String(error?.message || error || 'UNKNOWN');
          const errorCode = String(error?.code || '');
          codeReserveStateRef.current = { oid: id, running: false, attempt, lastError: errorText, lastReason: reason };
          codeRawRef.current = '';
          setCodeRaw('');
          appendPranimiCodeDebug('one_way_code_not_ready', { local_oid: id, actor_pin: resolvePranimiActorPin(actor), error: errorText, error_code: errorCode, reason, attempt });
          const terminal = error?.terminal === true || ['DRAFT_ALREADY_FINALIZED','PIN_ACTIVE_DRAFT_EXISTS','MISSING_REAL_ACTOR_PIN','DB_ASSIGNMENT_IDENTITY_MISMATCH'].includes(errorCode) || /DRAFT_ALREADY_FINALIZED|PIN_ACTIVE_DRAFT_EXISTS/.test(errorText);
          const ambiguous = /AMBIGUOUS|TIMEOUT|LOAD FAILED|FAILED TO FETCH|NETWORK|ABORT/i.test(`${errorCode} ${errorText}`);
          const nextAttempt = attempt + 1;
          if (!terminal && ambiguous && nextAttempt < PRANIMI_CODE_RESERVE_RETRY_DELAYS_MS.length) {
            setCodeReserveUi({ status: 'retrying', message: `VERIFIKIMI S’U KONFIRMUA — PROVA ${nextAttempt + 1}`, raw: errorText });
            tryReserveCodeInBackground(id, `${reason}_retry_${nextAttempt}`, { attempt: nextAttempt, delayMs: PRANIMI_CODE_RESERVE_RETRY_DELAYS_MS[nextAttempt], force: true });
          } else {
            setCodeReserveUi({ status: 'error', message: terminal ? 'DRAFTI/KODI ËSHTË I BLOKUAR — LAJMËRO ADMININ' : friendlyCodeReserveMessage(errorText), raw: `${errorCode}:${errorText}` });
          }
        } finally {
          const latest = codeReserveStateRef.current || {};
          if (String(latest.oid || '') === id && latest.running) codeReserveStateRef.current = { ...latest, running: false };
        }
      })();
    }, delayMs);
  }
  ...
  function hydrateFromPastrimiEdit(payload) {
    ...
    setOid(nextOid);
    // Existing-order edit codes are DB facts, not allocator candidates. Keep the
    // badge blank until the exact order id/code pair is verified.
    codeRawRef.current = '';
    setCodeRaw('');
    setCodeReserveUi({ status: 'reserving', message: 'DUKE VERIFIKU KODIN E ORDERIT…', raw: '' });
    void (async () => {
      try {
        const verdict = await verifyExactPranimiOrderCode(nextOid, codeVal, editBridgeRef.current?.local_oid || '');
        if (!verdict?.ok || String(oidRef.current || '') !== nextOid) throw Object.assign(new Error(verdict?.reason || 'EDIT_ORDER_CODE_NOT_VERIFIED'), { code: verdict?.reason || 'EDIT_ORDER_CODE_NOT_VERIFIED' });
        codeRawRef.current = String(verdict.code);
        setCodeRaw(String(verdict.code));
        setCodeReserveUi({ status: 'ready', message: '', raw: '' });
      } catch (error) {
        if (String(oidRef.current || '') !== nextOid) return;
        codeRawRef.current = '';
        setCodeRaw('');
        setCodeReserveUi({ status: 'error', message: 'KODI I ORDERIT NUK U VERIFIKUA — RUAJTJA ËSHTË BLOKUAR', raw: String(error?.code || error?.message || error || '') });
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let localBootMeta = {};
      try {
        const activePin = resolvePranimiActorPin(actor);
        try {
    ...
    const onOnline = () => {
      ...
      const activeOid = String(oidRef.current || '').trim();
      if (activeOid && normalizeCode(codeRawRef.current) == null) {
        tryReserveCodeInBackground(activeOid, 'online_resume_blank_code', { attempt: 0, delayMs: 200, force: true });
      }
    };
    ...
    const onPageShow = () => {
      ...
      const activeOid = String(oidRef.current || '').trim();
      if (activeOid && normalizeCode(codeRawRef.current) == null) {
        tryReserveCodeInBackground(activeOid, 'pageshow_blank_code', { attempt: 0, delayMs: 250, force: true });
      }
    };
    ...
    const onVisible = () => {
      try {
        if (document.visibilityState === 'visible') {
          ...
          const activeOid = String(oidRef.current || '').trim();
          if (activeOid && normalizeCode(codeRawRef.current) == null) {
            tryReserveCodeInBackground(activeOid, 'visibility_blank_code', { attempt: 0, delayMs: 300, force: true });
          }
        }
      } catch {}
    };
    ...
    const onSessionChanged = () => {
      queueBackgroundMetaSync('session_changed', 50);
      const activeOid = String(oidRef.current || '').trim();
      if (activeOid && normalizeCode(codeRawRef.current) == null) {
        tryReserveCodeInBackground(activeOid, 'session_changed_blank_code', { attempt: 0, delayMs: 80, force: true });
      }
    };
    ...
    window.addEventListener('tepiha:session-changed', onSessionChanged);
  ...
  function getCurrentPranimiCodeNumber() {
    // Only the verified UI binding may participate in matching/saving. Raw storage
    // remains an allocator candidate and never overrides the badge.
    return normalizeCode(codeRawRef.current || codeRaw || null);
  }
  ...
  async function applyClientMatchChoice(mode, payload = clientMatchPrompt) {
    ...
    const codeNum = normalizeCode(cand?.code || null);
    const candPhoneRaw = String(cand?.phone || cand?.client_phone || '').trim();
    const candName = String(cand?.name || cand?.full_name || cand?.client_name || [cand?.first_name, cand?.last_name].filter(Boolean).join(' ') || '').trim();
    codeRawRef.current = '';
    setCodeRaw('');
    setSelectedClient(null);
    setCodeReserveUi({ status: 'reserving', message: 'DUKE VERIFIKU KODIN E KLIENTIT…', raw: '' });

    if (!candId || codeNum == null) {
      setCodeReserveUi({ status: 'error', message: 'KODI I KLIENTIT NUK U VERIFIKUA', raw: 'EXISTING_CLIENT_ID_OR_CODE_MISSING' });
      appendPranimiCodeDebug('existing_phone_client_rejected_unverified', { local_oid: String(oidRef.current || oid || ''), matchKey, selected_client_id: candId || null, selected_client_code: codeNum, final_code_reason: 'SELECTED_CLIENT_CODE_REJECTED_ID_OR_CODE_MISSING' });
      alert('Ky klient nuk u verifikua me ID dhe kod exact në DB. Zgjedhja u ndal për siguri.');
      return;
    }
  ...
  function buildDraftSnapshot() {
    ...
    const now = Date.now();
    const actorPin = resolvePranimiActorPin(actor, existingReservation?.created_by_pin);
    const base = {
      id: activeOid,
      local_oid: activeOid,
      ts: now,
  ...
  function markDraftReservationFromSnapshot(draft, reason = 'draft_activity') {
    try {
      ...
      if (meaningful && code != null) {
        void renewPranimiCodeLease(resolvePranimiActorPin(actor), id, code, true).catch(() => ({ ok: false }));
      }
      return meta;
    } catch {
      return null;
  ...
  async function releaseBlankDraftReservation(meta = {}, reason = 'blank_draft_release') {
    try {
      ...
      if (code != null) {
        await releasePranimiCodeLease(resolvePranimiActorPin(actor), id, code, reason).catch(() => ({ ok: false }));
      }
  ...
  function readLocalDraftSummaries() {
    return readAllDraftsLocal()
      .filter((d) => d?.id && !isAnyDraftAliasSuppressed(d, { id: d?.id, file_key: d?._draft_file_key || d?.remote_file_key || d?.file_key || '' }) && !isPranimiUnsafeRemoteDraftSummary(d) && snapshotHasMeaningfulWork(d))
      .map((d) => {
        let totalM2 = 0;
        (d?.tepihaRows || []).forEach((r) => { totalM2 += (Number(r?.m2) || 0) * (Number(r?.qty) || 0); });
        (d?.stazaRows || []).forEach((r) => { totalM2 += (Number(r?.m2) || 0) * (Number(r?.qty) || 0); });
  ...
  async function handleContinue() {
    ...
    if (!finalActorPin) {
      setCodeReserveUi({ status: 'error', message: 'PIN-I I PUNËTORIT NUK U GJET — DIL E HYR PRAP', raw: 'MISSING_REAL_ACTOR_PIN' });
      alert('PIN-i i punëtorit nuk u gjet. Dil nga aplikacioni dhe hyr prap me PIN.');
      return;
    }
    ...
    try {
      ...
      if (finalizingOid) suppressDraftId(finalizingOid);
      try { if (draftTimer.current) clearTimeout(draftTimer.current); } catch {}
      try { if (remoteDraftTimerRef.current) clearTimeout(remoteDraftTimerRef.current); } catch {}
    } catch {}
    ...
    setSavingContinue(false);
      ...
      const codeDraftId = String(stableLocalOid || shadowOrderId || oid || '').trim();
      ...
      try {
        if (codeLifecycleMode === 'EDIT_EXISTING_ORDER') {
          const candidate = normalizeCode(codeRawRef.current || codeRaw || editBridgeRef.current?.order?.code || editBridgeRef.current?.order?.data?.code || null);
          const verdict = await verifyExactPranimiOrderCode(editTargetId, candidate, stableLocalOid);
          if (!verdict?.ok) throw Object.assign(new Error(verdict?.reason || 'EDIT_ORDER_CODE_NOT_VERIFIED'), { code: verdict?.reason || 'EDIT_ORDER_CODE_NOT_VERIFIED' });
          resolvedCodeRaw = String(verdict.code);
          verifiedCodeResult = verdict;
          finalCodeReason = 'EDIT_EXISTING_ORDER_DB_VERIFIED';
        } else if (codeLifecycleMode === 'EXISTING_CLIENT_HISTORICAL_CODE') {
          const verdict = await verifyExistingPranimiClientCode(
            resolvedSelectedClientId,
            resolvedSelectedClientCode,
            resolvedSelectedClient?.phone || getCanonicalClientPhone(),
            resolvedSelectedClient?.name || name
          );
          if (!verdict?.ok) throw Object.assign(new Error(verdict?.reason || 'EXISTING_CLIENT_CODE_NOT_VERIFIED'), { code: verdict?.reason || 'EXISTING_CLIENT_CODE_NOT_VERIFIED' });
          resolvedCodeRaw = String(normalizeCode(verdict.code));
          verifiedCodeResult = verdict;
          ...
          finalCodeReason = result?.verdict?.offline ? 'OFFLINE_VERIFIED_ASSIGNMENT_PROOF' : 'ONE_WAY_DB_ASSIGNED_CODE';
        }

        normCodeNow = formatKod(normalizeCode(resolvedCodeRaw), true);
        if (!normCodeNow || normCodeNow === '0' || normCodeNow === '—' || normCodeNow === '…') throw new Error('FINAL_CODE_EMPTY_AFTER_ONE_WAY_VERIFY');
        ...
        appendPranimiCodeDebug('pranimi_final_code_one_way_verified', {
          ...
          offline_proof: !!verifiedCodeResult?.offline,
        });
      } catch (verifyErr) {
        const verifyMsg = String(verifyErr?.message || verifyErr || 'PRANIMI_CODE_VERIFY_FAILED');
        const verifyCode = String(verifyErr?.code || '');
        codeRawRef.current = '';
        setCodeRaw('');
        setCodeReserveUi({ status: 'error', message: friendlyCodeReserveMessage(verifyMsg), raw: `${verifyCode}:${verifyMsg}` });
    ...
    setSavingContinue(false);
      ...
      // Offline save may reuse only the allocator's recent verified proof. Network
      // ambiguity while online never becomes an offline fallback.
    ...
    setSavingContinue(false);
      ...
      const finishLocalOnlyWarning = (warning = {}) => {
        // Preserve the active draft, exact assigned code, proof, and form state. A
        // failed/ambiguous final save must retry the same code and must never look
        // like a completed order locally.
        const activeLocalOid = stableLocalOid || String(shadowOrderId || oid);
        appendPranimiCodeDebug('final_save_not_confirmed_binding_retained', {
          local_oid: activeLocalOid,
          code: persistedClientCode,
          status: warning?.status_label || 'LOCAL / NOT SYNCED',
          error: warning?.last_error || null,
        });
        setLocalSyncWarning({
          severity: warning?.severity || 'yellow',
          title: warning?.title || 'RUAJTJA NUK U KONFIRMUA — DRAFTI U RUAJT PËR RETRY',
          subtitle: warning?.subtitle || 'LOCAL / NOT SYNCED',
          status_label: warning?.status_label || warning?.subtitle || 'LOCAL / NOT SYNCED',
          message: warning?.message || 'KJO ORDER ENDE NUK KA HYRË NË DB. PROVO RETRY ME TË NJËJTIN KOD.',
    ...
    setSavingContinue(false);
      ...
      const finalizeCodeLifecycleForVerifiedOrder = async (verifiedRow = {}) => {
        const exactOrderId = String(verifiedRow?.id || '').trim();
    ...
    setSavingContinue(false);
        ...
        const smsOrderPayload = JSON.parse(JSON.stringify({
          ...
          order_id: verifiedOrderIdForDraftCleanup,
          public_order_id: verifiedOrderIdForDraftCleanup,
          db_id: verifiedOrderIdForDraftCleanup,
          local_oid: stableLocalOid || String(shadowOrderId || oid || ''),
      ...
      try {
        if (isBaseEdit && editTargetId) {
          applyPranimiFinalLifecycleToPayload(payload, {
            ...
            localOid: stableLocalOid || String(editTargetId),
            verifyState: 'DB_VERIFIED',
            source: 'DB_FINAL',
            draftSource: 'FINAL / EDIT SAVE',
          });
    ...
    setSavingContinue(false);
          ...
          try {
            applyPranimiFinalLifecycleToPayload(payload, {
              ...
              verifyState: 'DB_VERIFIED',
              source: 'DB_FINAL',
              draftSource: 'FINAL / DB VERIFIED',
              serverId: String(verifyRes?.row?.id || ''),
            });
    ...
    setSavingContinue(false);
          ...
          try {
            applyPranimiFinalLifecycleToPayload(payload, {
              status: payload.status || verifiedDbRow?.status || targetStatus,
              localOid: stableLocalOid || String(shadowOrderId || oid),
              saveAttemptId: payload?.data?.pranimi_code_lifecycle?.save_attempt_id || '',
              verifyState: 'DB_VERIFIED',
              source: 'DB_FINAL',
              draftSource: 'FINAL / DB VERIFIED',
              serverId: String(verifiedDbRow?.id || verifyRes?.row?.id || ''),
            });
            const verifiedDbId = String(verifiedDbRow?.id || verifyRes?.row?.id || '').trim();
            if (verifiedDbId) {
              await updateOrderRecord('orders', verifiedDbId, {
                status: payload.status || targetStatus,
                data: payload.data,
                updated_at: new Date().toISOString(),
              });
              verifiedDbRow = { ...(verifiedDbRow || {}), status: payload.status || targetStatus, data: payload.data };
            }
          } catch (verifyUpdateErr) {
            appendPranimiCodeDebug('db_verified_metadata_update_failed', {
              local_oid: stableLocalOid || String(shadowOrderId || oid),
              save_attempt_id: payload?.data?.pranimi_code_lifecycle?.save_attempt_id || null,
              error: String(verifyUpdateErr?.message || verifyUpdateErr || ''),
            });
          }
    ...
    setSavingContinue(false);
          ...
          try {
            ...
            codeLifecycleResult = { ok: false, reason: error?.code || error?.message || 'CODE_LIFECYCLE_THROW', error };
          }
          if (!codeLifecycleResult?.ok) {
            appendPranimiCodeDebug('final_order_code_lifecycle_not_confirmed', {
              local_oid: codeDraftId,
              order_id: String(verifiedDbRow?.id || ''),
              code: persistedClientCode,
              lifecycle_mode: codeLifecycleMode,
              reason: codeLifecycleResult?.reason || 'NOT_CONFIRMED',
            });
            setLocalSyncWarning({
              severity: 'red',
              title: 'ORDER U RUAJT, POR KODI NUK U FINALIZUA',
              subtitle: 'DB VERIFIED / CODE LIFECYCLE PENDING',
              status_label: 'DB VERIFIED / CODE PENDING',
              message: 'MOS E KRIJO ORDERIN PRAPË. PROVO RETRY ME TË NJËJTIN DRAFT/KOD OSE LAJMËRO ADMININ.',
              problem_title: 'CODE LIFECYCLE NOT CONFIRMED',
              last_error: codeLifecycleResult?.reason || 'CODE_LIFECYCLE_NOT_CONFIRMED',
              payload,
              local_oid: codeDraftId,
              server_id: String(verifiedDbRow?.id || ''),
            });
    ...
    setSavingContinue(false);
          ...
          finishSuccess(verifiedDbRow, codeLifecycleResult);
          return;
        }
  ...
  function applyDraftSnapshotToForm(d, fallbackId = '', options = {}) {
    ...
    if (restoredDraftCode != null) {
      // ONE-WAY ALLOCATOR — verify-on-read.
      // Never paint a restored code before it is verified. We keep the badge
      // blank ("verifying") so a stale/used code is not shown even for a single
      // render, then either display the verified code or allocate a fresh one.
      codeRawRef.current = '';
      setCodeRaw('');
      setCodeReserveUi({ status: 'reserving', message: 'DUKE VERIFIKU KODIN…', raw: '' });
  ...
  async function deleteDraft(input) {
    ...
    if (releasedCode != null) {
      await releasePranimiCodeLease(resolvePranimiActorPin(actor), targetId, releasedCode, 'delete_incomplete_draft').catch(() => ({ ok: false }));
    }
  ...
  function getActivePranimiCodeForDisplay() {
    return normalizeCode(codeRaw || codeRawRef.current || null);
  }
