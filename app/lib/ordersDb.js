...
export async function saveOrderToDb(order, source = 'PRANIMI') {
  // orders: code, status, client_name, client_phone, data, total, paid
  // NOTE: some DB variants do NOT have `code_n` column. We only persist `code`.
  ...
  // IMPORTANT: local UUID must be stored as local_oid (DB id is BIGINT auto-increment)
  const localOrderId = String(order?.id || order?.oid || order?.local_id || '').trim() || (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `order_${Date.now()}_${Math.random().toString(16).slice(2)}`);
  ...
  const insertRow = {
    ...
    data: payload,
    total: Number(payload?.pay?.euro ?? payload?.pay?.total ?? 0) || 0,
    paid: Number(payload?.pay?.paid ?? 0) || 0,
  };

  const { data: ins, error: e1 } = await supabase
    .from('orders')
    // Idempotency by local_oid
    .upsert(insertRow, { onConflict: 'local_oid' })
    .select('id, code, status, created_at, local_oid')
    .single();
...
export async function fetchOrdersFromDb(limit = 5000) {
  const { data, error } = await supabase
    .from('orders')
    .select('id, code, status, client_name, client_phone, created_at, updated_at')
