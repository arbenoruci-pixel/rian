          ...
          return (
            <div className="payfs">
              <div className="payfs-top">
                <div><div className="payfs-title">PAGESA (ARKË)</div><div className="payfs-sub">KODI: {normalizeCode(codeRaw)} • {name}</div></div>
                <button className="btn secondary" onClick={() => setShowPaySheet(false)} style={{fontSize: 20}}>✕</button>
              </div>
              <div className="payfs-body">
                <div className="card" style={{ marginTop: 0, padding: 18 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, opacity: 0.9 }}>
                    <span>TOTALI I POROSISË:</span> <strong>{totalEuro.toFixed(2)} €</strong>
                  </div>
                  {clientPaid > 0 && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, color: '#10b981', marginTop: 10 }}>
                      <span>PAGUAR MË HERËT:</span> <strong>{Number(clientPaid).toFixed(2)} €</strong>
                    </div>
                  )}
              ...
              <div className="payfs-footer">
                <button className="btn secondary" onClick={() => setShowPaySheet(false)} style={{ fontWeight: 900, flex: 1 }}>ANULO</button>
                <button className="btn primary" onClick={applyPayAndClose} style={{ fontWeight: 900, background: '#10b981', color: '#000', flex: 2 }} disabled={saving}>KRYEJ PAGESËN</button>
              </div>
            </div>
          );
        })()}
