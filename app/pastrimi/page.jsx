...
function getTransportBaseSummary(raw) {
  const order = unwrapOrderData(raw);
  const transport = unwrapPayload(order?.transport || order?.transport_meta || order?.transportOrder || {});
  const broughtBy = String(
...
function mapExactDbRowToPastrim(row, source = 'orders') {
  ...
  if (status !== 'pastrim' && status !== 'pastrimi') return null;
    ...
    try {
      ...
      (transportData || []).forEach(row => {
        ...
        allOrders.push(normalizeRenderableOrderRow({
          ...
          cope, total, paid, isPaid: paid >= total && total > 0, isReturn: false, fullOrder: localOid && !String(order?.local_oid || '').trim() ? { ...order, local_oid: localOid } : order
        }));
      });
