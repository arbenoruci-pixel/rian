...
function PastrimiPageInner() {
  ...
  const [paymentPurposeWizard, setPaymentPurposeWizard] = useState(null);
  const [paymentPurposeBusy, setPaymentPurposeBusy] = useState(false);
  const paymentPurposeBusyRef = useRef(false);
...
👉 SHKRUAJ PIN-IN TËND PËR TË KRYER PAGESËN:`;
  ...
  async function applyRowPayAndClose(purpose = '') {
    ...
    // PASTRIMI_PREPAYMENT_PRESERVE_STATUS_V3: superseded by the explicit purpose wizard.
    ...
    const paymentTransaction = {
      ...
      amount: applied,
      method: 'CASH',
      note: `PAGESË NË PASTRIMI ${applied.toFixed(2)}€ • #${rowPayOrder.code} • ${rowPayOrder.name || ''} • ${paymentOutcome}`,
  ...
  async function choosePaymentPurpose(purpose) {
    if (paymentPurposeBusyRef.current) return;
  ...
  return (
    <div className="wrap">
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, flexShrink:0 }}>
                    <div
                      ...
                      style={{
                        ...
                        color: '#fff', ...getOrderCodeBadgeStyle(codeLabel), display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, fontWeight: 800, flexShrink: 0
                      }}>
                      {codeLabel}
                    </div>
      ...
      <PastrimiPaymentPurposeWizard
        open={paymentPurposeWizard?.source === 'row'}
        code={paymentPurposeWizard?.code}
        clientName={paymentPurposeWizard?.clientName}
        due={paymentPurposeWizard?.due}
        cashGiven={paymentPurposeWizard?.cashGiven}
