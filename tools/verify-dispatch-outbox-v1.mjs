...
// Offline submit survives reload with exact original customer, schedule, note and UUID.
{
  ...
  const input = structuredClone(payload); await queue.enqueue(input); input.data.note = 'Changed outside queue';
  await queue.drain(); assert.equal(state.calls.length, 0);
  const restored = createDispatchOutbox(args);
  assert.equal((await restored.list())[0].payload.data.note, 'Original');
  assert.equal((await restored.enqueue({ ...payload, id: otherActor, data: { note: 'Changed form' } })).id, id);
  ...
  assert.equal((await restored.list())[0].state, 'sent');
...
// Repeated failures are retried by the runtime, never by clicking/creating another UUID.
{
  ...
  assert.equal((await queue.list())[0].state, 'sent'); assert.equal(new Set(bodies).size, 1); assert.equal(state.committed, 1);
}
...
// A committed write whose response was lost is replayed under the same key.
{
  ...
  await queue.enqueue(payload); await queue.drain(); state.time += 30000; await queue.drain();
  assert.equal(inserts, 1); assert.equal((await queue.list())[0].state, 'sent');
}

// Concurrent drains are serialized; separate tabs preserve the same request.
{
  let release; let started; const entered = new Promise(resolve => { started = resolve; }); let calls = 0;
  const { queue, args } = setup({ submit: async () => { calls++; started(); await new Promise((resolve) => { release = resolve; }); return { ok: true, data: { id, code_str: 'T1' } }; } });
  await queue.enqueue(payload); const first = queue.drain(); const second = queue.drain();
  await entered;
  assert.equal(calls, 1); release(); await Promise.all([first, second]);
  assert.equal((await createDispatchOutbox(args).list())[0].state, 'sent');
}

// Another login cannot view or send pending work; server checks the expected actor too.
{
  const { queue, state } = setup(); await queue.enqueue(payload); state.actor = otherActor;
  await assert.rejects(() => queue.enqueue({ ...payload, expected_actor_id: actor }), /ACTOR_SESSION_MISMATCH/);
  assert.equal((await queue.list()).length, 0); await queue.drain(); assert.equal(state.calls.length, 0);
...
for (const error of ['DEVICE_NOT_APPROVED', 'AUTH_REQUIRED', 'DISPATCH_ORDER_ACTOR_SESSION_MISMATCH', 'TRANSPORT_ORDER_IDEMPOTENCY_FINGERPRINT_CONFLICT', 'TRANSPORT_PHONE_INVALID']) {
  ...
  await queue.enqueue(payload); await queue.drain(); state.time += 60000; await queue.drain();
  assert.equal(calls, 1); assert.equal((await queue.list())[0].state, 'blocked'); assert.equal((await queue.list())[0].error, error);
}
...
for (const storage of [
  ...
  const { queue, state } = setup({ storage }); await assert.rejects(() => queue.enqueue(payload)); assert.equal(state.calls.length, 0);
}

// Old pending work remains recoverable; explicit review reuses its frozen UUID/payload.
{
  const { queue, state } = setup(); await queue.enqueue(payload); state.time += 25 * 60 * 60 * 1000;
  await queue.drain(); assert.equal(state.calls.length, 0); assert.equal((await queue.list())[0].state, 'blocked');
  await queue.retry(id); await queue.drain(); assert.equal(state.calls[0].id, id);
}
