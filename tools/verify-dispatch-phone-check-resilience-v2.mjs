...
check(runtime.includes('submit: insertTransportOrder'), 'outbox must retain approved-device atomic create');
check(transport.includes('assertDeduplicatedActiveDispatchOrder(body.data, expected)'), 'active-order identity verification missing');
check(transport.includes('body?.recoveredStaleIntent === true'), 'server phone-conflict recovery verification missing');
check(dispatch.includes('DËRGIMI VAZHDON AUTOMATIKISHT'), 'durable send guidance missing');
