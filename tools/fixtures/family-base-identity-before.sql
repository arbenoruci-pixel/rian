CREATE OR REPLACE FUNCTION public.normalize_kosovo_phone_v1(p_phone text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
declare
  v_raw text := upper(trim(coalesce(p_phone,'')));
  v_digits text;
  v_local text;
begin
  if v_raw='' or v_raw like 'PA NUMER%' or v_raw like 'PA TELEFON%' or v_raw like 'PA PHONE%' then
    return null;
  end if;
  v_digits := regexp_replace(v_raw,'\D','','g');
  if length(v_digits)<8 or v_digits like '000%' then return null; end if;
  v_local := right(v_digits,8);
  if v_local in ('00000000','12121212','30000000','44000000','45454545','54545454','83000000') then return null; end if;
  if v_local ~ '^([0-9])\1{7}$' then return null; end if;
  return '383'||v_local;
end;
$function$;

CREATE OR REPLACE FUNCTION public.client_name_is_placeholder_v1(p_name text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
AS $function$
  select
    ...
    or lower(trim(coalesce(p_name,''))) in ('pa emer','pa emër','unknown','test','x')
    or lower(trim(coalesce(p_name,''))) ~ '(^|\s)(test|unknown)(\s|$)'
    or lower(trim(coalesce(p_name,''))) ~ '^([a-zçë]+)\s+\1$'
    or lower(trim(coalesce(p_name,''))) ~ '\s+x$';
$function$;

CREATE OR REPLACE FUNCTION public.norm_client_name(v text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
AS $function$
  select nullif(
    regexp_replace(lower(trim(coalesce(v, ''))), '\s+', ' ', 'g'),
    ''
  );
$function$;
