...
{
 const storage=memory(), set=storage.setItem; let interleave=true;let concurrent;
 const a=createDispatchOutbox(common(storage)),b=createDispatchOutbox(common(storage));
 storage.setItem=(key,value)=>{if(interleave){interleave=false;concurrent=b.enqueue(payload(2));}set(key,value);};
 await a.enqueue(payload(1));await concurrent;
 assert.deepEqual((await a.list()).map(x=>x.id).sort(),[uuid(1),uuid(2)]);
}
// All 100 independent offline orders survive reopening, commit once, and are not resent.
{
 ...
 for(let i=1;i<=100;i++)await q.enqueue(payload(i));
 ...
 await restored.drain();await restored.drain();assert.equal(calls,100);assert.equal((await restored.list()).filter(x=>x.state==='sent').length,100);
}
// 100 timeouts, with a reload halfway through, retain the identical payload and UUID.
{
 ...
 let q=createDispatchOutbox(options);await q.enqueue(payload(1));
 for(let i=0;i<=100;i++){await q.drain();time+=30001;if(i===50)q=createDispatchOutbox(options);}
 assert.equal((await q.list())[0].state,'sent');assert.equal(calls,101);assert.equal(new Set(bodies).size,1);
}
// Migration retains payloads, UUIDs and attempts; partial migration failure preserves v1.
{
 const storage=memory();const q=createDispatchOutbox(common(storage));await q.enqueue(payload(1));await q.enqueue(payload(2));
 const items=(await q.list());for(const i of items)storage.removeItem(DISPATCH_OUTBOX_ITEM_PREFIX+i.id);
 ...
 await assert.rejects(()=>q.list(),/quota/);assert.equal(storage.getItem(DISPATCH_OUTBOX_KEY),legacy);
 storage.setItem=set;assert.deepEqual((await q.list()),items);assert.equal(storage.getItem(DISPATCH_OUTBOX_KEY),null);
}
