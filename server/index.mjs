...
import runtimeIncidentHandler from '../api/runtime-incident.js';
...
app.post('/api/base-codes/reserve', async (req, res) => {
  try {
    const supabase = createAdminClientOrThrow();
    const result = await reserveBaseCodesForPin(req.body || {}, { supabase });
...
// Legacy alias used by older cached PWA bundles. Keep it on the exact same
// allocator so a stale client cannot follow a weaker/empty-pool code path.
app.post('/api/pranimi/reserve-code', async (req, res) => {
  try {
    const supabase = createAdminClientOrThrow();
    const result = await reserveBaseCodesForPin(req.body || {}, { supabase });
    return apiOk(res, {
...
app.post('/api/pranimi/draft', async (req, res) => {
  try {
    const supabase = createAdminClientOrThrow();
    const result = await runPranimiDraftDbAction(req.body || {}, { supabase });
...
app.post('/api/runtime-incident', runtimeIncidentHandler);
